...
{
public:
    FilterTest(const FirFilter &filter);
...
FilterTest::FilterTest(const FirFilter &filter) : filter(filter)
{
...
void SyllibTest::testFirFilter()
{
    const unsigned char dump[] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x40, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x08, 0x40, 0xa4, 0xd1, 0xc5, 0xe4, 0x2d, 0xd1, 0x51, 0x3f,
        0xac, 0x3d, 0xfa, 0xe1, 0x09, 0xcc, 0x51, 0xbf, 0x6d, 0x16, 0x76, 0x23,
        0x23, 0x20, 0x52, 0x3f, 0xc6, 0xad, 0x60, 0x61, 0x45, 0x0d, 0x53, 0x3f,
        0x93, 0xd9, 0x1e, 0x39, 0xfc, 0x84, 0x52, 0xbf, 0xc4, 0xc5, 0x5e, 0xb4,
        0x44, 0x51, 0x53, 0x3f, 0x5d, 0x7f, 0xef, 0x64, 0x58, 0x5b, 0x55, 0x3f,
        0xf9, 0xe7, 0x1b, 0xb9, 0x3d, 0xcf, 0x56, 0xbf, 0x05, 0x28, 0x3b, 0x92,
        0xc3, 0xf7, 0x54, 0x3f, 0x87, 0x3f, 0xcc, 0x24, 0x5c, 0xc0, 0x55, 0x3f,
        0x4b, 0x4a, 0xeb, 0x4b, 0x26, 0xd7, 0x56, 0xbf, 0xf9, 0x8b, 0x66, 0xaa,
        0x24, 0xf7, 0x57, 0x3f};

    const double taps[] = {1.0874698810684280e-03, -1.0862442503912590e-03,
    ...
    try {
        { // Basic loading, rounding up to 4 taps
            ...
            Q_FOREACH (const QVector<double> &taps, FirFilter::load(&buffer))
                filters.append(FirFilter(taps, false));
        ...
        { // Basic loading, limited to 2
            QBuffer buffer(&array);
            QList<FirFilter> filters;
            Q_FOREACH (const QVector<double> &taps, FirFilter::load(&buffer, 2))
                filters.append(FirFilter(taps, false));
            QCOMPARE(filters.size(), 4);
        ...
        { // synchronous
            ...
            QList<FirFilter> filters;
            Q_FOREACH (const QVector<double> &taps, FirFilter::load(&buffer))
                filters.append(FirFilter(taps, true));
            FirFilter filter = filters.at(0);
            for (unsigned i = 0; i < 128;) {
                const unsigned length = filter.length();
                filter.process(data3 + i, length);
                i += length;
            }
        }
