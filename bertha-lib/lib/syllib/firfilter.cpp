...
// Emulate QSharedPointer with a custom delete function by using
...
QList<QVector<double>> FirFilterPrivate::load(QIODevice *device, unsigned limit)
{
    QList<QVector<double>> result;

    double doubleChannels;
    if (device->read(reinterpret_cast<char *>(&doubleChannels), 8) != 8)
        throw SylError(QLatin1String("Unable to read number of channels"));
    if (doubleChannels < 1)
        throw SylError(QLatin1String("Invalid number of channels"));
    unsigned channels = qRound(doubleChannels);

    double doubleLength;
    if (device->read(reinterpret_cast<char *>(&doubleLength), 8) != 8)
        throw SylError(QLatin1String("Unable to read filter length"));
    if (doubleLength < 1 || doubleLength >= INT_MAX / 8)
        throw SylError(QLatin1String("Invalid number of samples"));
    ...
    for (unsigned i = 0; i < channels; ++i) {
        ...
        if (device->read(reinterpret_cast<char *>(taps.data()), 8 * length) !=
            8 * length)
            throw SylError(QLatin1String("Unable to read filter coefficients"));
        if (limit)
    ...
    : d(new FirFilterPrivate)
{
    ...
    fftw_plan plan = fftw_plan_dft_r2c_1d(
        2 * d->length, d->taps.data(),
        reinterpret_cast<fftw_complex *>(d->fftTaps.data()), FFTW_ESTIMATE);
    fftw_execute(plan);
    fftw_destroy_plan(plan);
    ...
    d->fftPlan = new FftwPlan(fftw_plan_dft_r2c_1d(
        2 * d->length, d->delay.data(),
        reinterpret_cast<fftw_complex *>(d->buffer.data()), FFTW_ESTIMATE));
    d->ifftPlan = new FftwPlan(fftw_plan_dft_c2r_1d(
        2 * d->length, reinterpret_cast<fftw_complex *>(d->buffer.data()),
        d->output.data(), FFTW_ESTIMATE));
}
...
FirFilter::FirFilter(const FirFilter &other) : d(other.d)
{
}
...
void FirFilter::process(double *data, unsigned dataLength)
{
    ...
    while (dataLength) {
        ...
        if (position % length == 0) {
            fftw_execute_dft_r2c(fftPlan, delay,
                                 reinterpret_cast<fftw_complex *>(buffer));
            for (unsigned i = 0; i < length + 1; ++i)
                buffer[i] *= fftTaps[i];
            fftw_execute_dft_c2r(
                ifftPlan, reinterpret_cast<fftw_complex *>(buffer), output);
                                       ...
                                       unsigned maximumLength)
{
    ...
    if (!file.open(QFile::ReadOnly))
        throw SylError(QString::fromLatin1("Unable to read FIR coefficients "
                                           "from '%1': %2")
                                       ...
                                       unsigned maximumLength)
{
    if (!device->open(QFile::ReadOnly))
        throw SylError(QString::fromLatin1("Unable to read FIR"
                                           " coefficients: %1")
