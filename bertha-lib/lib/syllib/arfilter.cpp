...
QVector<double> ArFilterPrivate::load(QIODevice *device, unsigned limit)
{
    ...
    if (result[0] != 1.0)
        throw SylError(
            QLatin1String("Invalid AR filter coefficients (b0!=1.0)"));
    for (unsigned i = 1; i < length; ++i)
        if (result[i] != 0.0)
            throw SylError(
                QLatin1String("Invalid AR filter coefficients (bi!=0.0|i>0)"));
    ...
    if (result[0] != 1.0)
        throw SylError(
            QLatin1String("Invalid AR filter coefficients (a0!=1.0)"));
...
ArFilter::ArFilter(const QVector<double> &taps) : d(new ArFilterPrivate)
{
    Q_ASSERT(taps.size());
    Q_ASSERT(taps[0] == 1.0);
...
ArFilter::ArFilter(const ArFilter &other) : d(other.d)
{
}
...
QVector<double> ArFilter::load(const QString &filePath, unsigned maximumLength)
{
    ...
    if (!file.open(QFile::ReadOnly))
        throw SylError(QString::fromLatin1("Unable to read AR coefficients "
                                           "from '%1': %2")
...
QVector<double> ArFilter::load(QIODevice *device, unsigned maximumLength)
{
    if (!device->open(QFile::ReadOnly))
        throw SylError(QString::fromLatin1("Unable to read AR coefficients: %1")
