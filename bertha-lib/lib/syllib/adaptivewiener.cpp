 ...
 *                                                                            *
 * Based on code provided by ESAT/STADIUS (M. Moonen).                        *
 *                                                                            *
...
/*-----------------------------------------------------------------*/
void SubtractMatrix_data(double *mat1_data, double *mat2_data,
                         double *result_data, int N)
...
/*-----------------------------------------------------------------*/
void AddMatrix_data(double *mat1_data, double *result_data, int N,
                    double lambda)
...
/*-----------------------------------------------------------------*/
void ConjugateMatrix_data(double *mat_data_real, double *mat_data_imag,
                          double *result_data_real, double *result_data_imag,
                          int N)
...
/*-----------------------------------------------------------------*/
void MultiplyMatrix_cplx_data(double *mat1_data_real, double *mat1_data_imag,
                              double factor, int N)
...
/*-----------------------------------------------------------------*/
void AddMatrix_cplx_data(double *mat1_data_real, double *mat1_data_imag,
                         double *result_data_real, double *result_data_imag,
                         int N, double lambda)
...
/*-----------------------------------------------------------------*/
void SubtractMatrix_cplx_data(double *mat1_data_real, double *mat1_data_imag,
                              double *result_data_real,
                              double *result_data_imag, int N)
...
/*-----------------------------------------------------------------*/
void PointWiseDivisionSpecial_cplx_data(double *vec1_data,
                                        double *result_data_real,
                                        double *result_data_imag, double fact,
                                        double delta, int N)
/* Calculate pointwise division of 2 vectors (of equal length), with vec1 real
 * elements */
...
/*-----------------------------------------------------------------*/
void AddPointWiseProductH_cplx_data(
    double *vec1_data_real, double *vec1_data_imag, double *vec2_data_real,
    double *vec2_data_imag, double *result_data_real, double *result_data_imag,
    int N, double lambda1, double lambda2)
/* Add scaled pointwise product of 2 complex vectors (1 hermitian conjugate!) to
 * scaled complex vector */
/* out = lambda1*out + lambda2*conj(vec1).*vec2; */
/* COMPLEXITY = 4N ADD + 4N MULT + 2N MULT (lambda1 ~= 1) + 2N MULT (lambda2 ~=
 * 1) */
/* (vec1=vec2)= 2N ADD + 2N MULT + N MULT  (lambda1 ~= 1) + N MULT  (lambda2 ~=
 * 1) */
{
    int i;
...
/*-----------------------------------------------------------------*/
void PointWiseProduct_cplx2_data(double *vec1_data_real, double *vec1_data_imag,
                                 double *result_data, int N)
...
/*-----------------------------------------------------------------*/
void PointWiseProductSum_cplx_data(double *mat1_data_real,
                                   double *mat1_data_imag,
                                   double *mat2_data_real,
                                   double *mat2_data_imag,
                                   double *result_data_real,
                                   double *result_data_imag, int M, int N)
/* Calculate pointwise product of 2 complex matrices (of equal dimensions) + sum
 * over rows */
/* out = sum(mat1.*mat2,2) */
...
{
    /* Calculate inverse of 2D complex matrix (Hermitian + real elements on
     * diagonal) */
...
/*-----------------------------------------------------------------*/
void BlockInverse2_cplx_data(double *in_data_real, double *in_data_imag, int M,
                             double delta)
{
    /* Calculate inverse (inline) of block-diagonal complex matrix (dimensions
     * (2*M)*2) + regularisation delta*/
    ...
    for (i = 0; i < M; i++) {
        Inverse2_cplx_data((*in_data_real) + delta, *(in_data_real + M),
                           *(in_data_imag + M),
                           (*(in_data_real + 3 * M)) + delta, out);
...
/*-----------------------------------------------------------------*/
void PositiveDefiniteBlockMatrix2_cplx_data(double *in_data_real,
                                            double *in_data_imag, int M)
{
    /* Make block-diagonal complex matrix (dimensions (2*M)*2) positive definite
     * by adding regularisation */
    /* i.e. calculate smallest eigenvalue of 2x2 subblocks -> add regularisation
     * if smallest eigenvalue is negative */
    ...
    for (i = 0; i < M; i++) {
        /* Smallest eigenvalue of [ar      br-j*bi] is equal to
        ...
        delta =
            ((a + c) - sqrt((a - c) * (a - c) + 4 * (br * br + bi * bi))) / 2;
...
/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
void ConstrainedFreq2_cplx_data(double *fftR, double *fftI, double *real_data,
                                int M, int P, fftw_plan pFFT, fftw_plan pIFFT)
...
/* rank = 1;dims[0].n = M;dims[0].is = 1;dims[0].os = 1;howmany_rank =
 * 1;howmany_dims[0].n = P; */
/* howmany_dims[0].is = M;howmany_dims[0].os = N; */
/* pFFT  =
 * fftw_plan_guru_split_dft_r2c(rank,dims,howmany_rank,howmany_dims,in_data,fftR,fftI,FFTW_ESTIMATE);
 */
/* howmany_dims[0].is = N;howmany_dims[0].os = M; */
/* pIFFT =
 * fftw_plan_guru_split_dft_c2r(rank,dims,howmany_rank,howmany_dims,fftR,fftI,out_data,FFTW_ESTIMATE);
 */
 ...
 */
void SDW_MWF_freqlmsLP6b_stepFD(
    double *const *in_data, const double *const *in_dataSNROffset,
    double *const *in_delay, double *const *in_delaySNROffset, double *z_data,
    const double *speech_data, int nr_samples, int nr_channels, int nr_ref,
    int L, int delay, double rho, double mu_inv, double /*gamma*/,
    double lambda, double *W_data_real, double *W_data_imag, double *P_data,
    double *Sy_data_real, double *Sy_data_imag, double *Sn_data_real,
    double *Sn_data_imag, fftw_plan pFFT1_2L, fftw_plan pIFFT1_2L,
    double *fftreal, double *fftR, double *fftI, int step, int constr,
    int negstep, double delta)
{
    ...
    E_data_real =
        (double *)fftw_malloc(L1 * sizeof(double)); /* Data vector (size L1) */
    E_data_imag = (double *)fftw_malloc(L1 * sizeof(double));
    e_data = (double *)fftw_malloc(
        2 * L *
        sizeof(double)); /* Data vector (size 2L) with first half zeroes */
    InitialiseZero(e_data, L);
    e_data2 = e_data + L;
    fftreal2 = fftreal + L;

    data_real = (double *)calloc(
        L1tot, sizeof(double)); /* Data matrix for various purposes */
    data_imag = (double *)calloc(L1tot, sizeof(double));
    tmp_mat_real = (double *)calloc(
        L1tot * nr_ref,
        sizeof(double)); /* Data matrix (size L1tot) for various purposes */
    tmp_mat_imag = (double *)calloc(
        L1tot * nr_ref,
        sizeof(double)); /* Data matrix (size L1tot) for various purposes */
    ...
    for (k = 0; k < nr_blocks; k++) {
        ...
        PointWiseProductSum_cplx_data(data_real, data_imag, W_data_real,
                                      W_data_imag, E_data_real, E_data_imag, L1,
                                      nr_ref);
        memcpy(fftR, E_data_real, L1 * sizeof(double));
        memcpy(fftI, E_data_imag, L1 * sizeof(double));
        fftw_execute_split_dft_c2r(pIFFT1_2L, fftR, fftI,
                                   fftreal); /* FACTOR 2L */
        ...
        if (*speech_data == 0) {
            ...
            for (i = 0; i < nr_ref; i++) {
                for (j = 0; j < i; j++) {
                    AddPointWiseProductH_cplx_data(
                        data_real + i * L1, data_imag + i * L1,
                        data_real + j * L1, data_imag + j * L1,
                        Sn_data_real + j * L1tot + i * L1,
                        Sn_data_imag + j * L1tot + i * L1, L1, lambda,
                        (1 - lambda) / 2);
                    ConjugateMatrix_data(Sn_data_real + j * L1tot + i * L1,
                                         Sn_data_imag + j * L1tot + i * L1,
                                         Sn_data_real + i * L1tot + j * L1,
                                         Sn_data_imag + i * L1tot + j * L1, L1);
                }
                PointWiseProduct_cplx2_data(
                    data_real + i * L1, data_imag + i * L1, tmp_mat_real, L1);
                MultiplyMatrix_data(Sn_data_real + i * (L1tot + L1), lambda,
                                    L1);
                AddMatrix_data(tmp_mat_real, Sn_data_real + i * (L1tot + L1),
                               L1, (1 - lambda) / 2);
            }
            ...
            for (i = 0; i < nr_ref; i++) {
                for (j = 0; j < nr_ref;
                     j++) { /* Difference between speech and noise correlation
                               matrix */
                    SubtractMatrix_data(Sy_data_real + j * L1tot + i * L1,
                                        Sn_data_real + j * L1tot + i * L1,
                                        tmp_mat_real + j * L1, L1);
                    SubtractMatrix_data(Sy_data_imag + j * L1tot + i * L1,
                                        Sn_data_imag + j * L1tot + i * L1,
                                        tmp_mat_imag + j * L1, L1);
                }
                PointWiseProductSum_cplx_data(
                    tmp_mat_real, tmp_mat_imag, W_data_real, W_data_imag,
                    R_data_real + i * L1, R_data_imag + i * L1, L1, nr_ref);
            }
            MultiplyMatrix_cplx_data(R_data_real, R_data_imag, mu_inv, L1tot);

            if (step == 1) {
                /* Calculate step size */
                SubtractMatrix_data(Sy_data_real, Sn_data_real, tmp_mat_real,
                                    L1tot * nr_ref);
                SubtractMatrix_data(Sy_data_imag, Sn_data_imag, tmp_mat_imag,
                                    L1tot * nr_ref);
                MultiplyMatrix_cplx_data(tmp_mat_real, tmp_mat_imag, mu_inv,
                                         L1tot * nr_ref);
                if (negstep == 2) {
                    PositiveDefiniteBlockMatrix2_cplx_data(
                        tmp_mat_real, tmp_mat_imag,
                        L1); /* Make block matrix positive semi-definite */
                             /* Works only for 2 channels !!! */
                }
                AddMatrix_cplx_data(Sn_data_real, Sn_data_imag, tmp_mat_real,
                                    tmp_mat_imag, L1tot * nr_ref, 1);
                BlockInverse2_cplx_data(
                    tmp_mat_real, tmp_mat_imag, L1,
                    delta); /* Works only for 2 channels !!! */
                MultiplyMatrix_cplx_data(tmp_mat_real, tmp_mat_imag, fact,
                                         L1tot * nr_ref);

                /* Filter update */
                for (i = 0; i < nr_ref; i++) {
                    AddPointWiseProductH_cplx_data(
                        data_real + i * L1, data_imag + i * L1, E_data_real,
                        E_data_imag, R_data_real + i * L1, R_data_imag + i * L1,
                        L1, 1, -1);
                }
                for (i = 0; i < nr_ref; i++) {
                    for (j = 0; j < nr_ref; j++) {
                        memcpy(data_real + j * L1,
                               tmp_mat_real + i * L1 + j * L1tot,
                               L1 * sizeof(double));
                        memcpy(data_imag + j * L1,
                               tmp_mat_imag + i * L1 + j * L1tot,
                               L1 * sizeof(double));
                    }
                    PointWiseProductSum_cplx_data(
                        data_real, data_imag, R_data_real, R_data_imag,
                        E_data_real, E_data_imag, L1, nr_ref);

                    if (constr == 1) {
                        memcpy(fftR, E_data_real, L1 * sizeof(double));
                        memcpy(fftI, E_data_imag, L1 * sizeof(double));
                        ConstrainedFreq2_cplx_data(fftR, fftI, fftreal, 2 * L,
                                                   1, pFFT1_2L, pIFFT1_2L);
                        memcpy(E_data_real, fftR, L1 * sizeof(double));
                        memcpy(E_data_imag, fftI, L1 * sizeof(double));
                    } else {
                        MultiplyMatrix_cplx_data(E_data_real, E_data_imag, L,
                                                 L1); /* Factor 2L due to
                                                         fftw-routine * Factor
                                                         0.5 */
                    }

                    SubtractMatrix_cplx_data(E_data_real, E_data_imag,
                                             W_data_real + i * L1,
                                             W_data_imag + i * L1, L1);
                }
            }
            ...
            if (step == 2) {
                for (i = 0; i < nr_ref; i++) {
                    /* Calculate step size */
                    SubtractMatrix_data(Sy_data_real + i * L1tot + i * L1,
                                        Sn_data_real + i * L1tot + i * L1,
                                        P_data, L1);
                    ...
                    AddMatrix_data(Sn_data_real + i * L1tot + i * L1, P_data,
                                   L1, 1);
                    ...
                    PointWiseDivisionSpecial_cplx_data(
                        P_data, R_data_real + i * L1, R_data_imag + i * L1,
                        fact, delta, L1);
                    ...
                    if (constr == 1) {
                        ...
                        MultiplyMatrix_cplx_data(R_data_real + i * L1,
                                                 R_data_imag + i * L1, L,
                                                 L1); /* Factor 2L due to
                                                         fftw-routine * Factor
                                                         0.5 */
                    ...
                    SubtractMatrix_cplx_data(
                        R_data_real + i * L1, R_data_imag + i * L1,
                        W_data_real + i * L1, W_data_imag + i * L1, L1);
                }
            }

            if (step == 3 || step == 4) {
                /* Calculate step size */
                InitialiseZero(P2_data,
                               L1); /* Only noise (can be used 2 times) */
                for (i = 0; i < nr_ref; i++) {
                    AddMatrix_data(Sn_data_real + i * L1tot + i * L1, P2_data,
                                   L1, 1);
                }
                InitialiseZero(P_data, L1);
                for (i = 0; i < nr_ref; i++) {
                    AddMatrix_data(Sy_data_real + i * L1tot + i * L1, P_data,
                                   L1, 1);
                }
                ...
                if (step == 4) {
                    MultiplyMatrix_data(
                        P_data, invnr_ref,
                        L1); /* normalisation (same magnitude as step=3) */
                }
            ...
            for (i = 0; i < nr_ref; i++) {
                for (j = 0; j < i; j++) {
                    AddPointWiseProductH_cplx_data(
                        data_real + i * L1, data_imag + i * L1,
                        data_real + j * L1, data_imag + j * L1,
                        Sy_data_real + j * L1tot + i * L1,
                        Sy_data_imag + j * L1tot + i * L1, L1, lambda,
                        (1 - lambda) / 2);
                    ConjugateMatrix_data(Sy_data_real + j * L1tot + i * L1,
                                         Sy_data_imag + j * L1tot + i * L1,
                                         Sy_data_real + i * L1tot + j * L1,
                                         Sy_data_imag + i * L1tot + j * L1, L1);
                }
                PointWiseProduct_cplx2_data(
                    data_real + i * L1, data_imag + i * L1, tmp_mat_real, L1);
                MultiplyMatrix_data(Sy_data_real + i * (L1tot + L1), lambda,
                                    L1);
                AddMatrix_data(tmp_mat_real, Sy_data_real + i * (L1tot + L1),
                               L1, (1 - lambda) / 2);
            }
        }
        /* Next frame */
...
AdaptiveWienerFilter::AdaptiveWienerFilter(unsigned sampleRate,
                                           unsigned channels, unsigned length,
                                           double muInv, bool synchronous)
    : d(new AdaptiveWienerFilterPrivate)
{
    ...
    d->pFFT1_2L = fftw_plan_guru_split_dft_r2c(rank, dims, howmany_rank,
                                               howmany_dims, d->fftreal,
                                               d->fftR, d->fftI, FFTW_ESTIMATE);
    d->pIFFT1_2L = fftw_plan_guru_split_dft_c2r(rank, dims, howmany_rank,
                                                howmany_dims, d->fftR, d->fftI,
                                                d->fftreal, FFTW_ESTIMATE);
}
                                   ...
                                   const double *vad, unsigned dataLength)
{
    ...
    for (unsigned pos = 0; pos < dataLength; pos += d->length) {
        ...
        SDW_MWF_freqlmsLP6b_stepFD(
            dataPtr.data(), dataSnrPtr.data(), d->delayPtr.data(),
            d->delaySnrPtr.data(), output.data(), vadPtr, d->length,
            d->channels, d->refs, d->length, d->delayValue, d->rho, d->muInv,
            d->gamma, d->lambda, d->wDataReal.data(), d->wDataImag.data(),
            d->pData.data(), d->sData[0].data(), d->sData[1].data(),
            d->sData[2].data(), d->sData[3].data(), d->pFFT1_2L, d->pIFFT1_2L,
            d->fftreal, d->fftR, d->fftI,
        ...
        for (unsigned i = 0; i < d->channels; ++i) {
            memcpy(d->delay[i].data(), dataPtr[i], d->length * sizeof(double));
