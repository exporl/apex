...
namespace apex
{
  namespace editor
  {
    ...
    void ScreenEditor::selectWidget( QFrame* widget, ScreenElementEditorDelegate* del )
    {
      if ( selectedWidget )
      {
        // back to normal..
    ...
    void ScreenEditor::addWidgetClicked( ScreenElementCreator* r )
    {
      ...
      if ( !selectedWidget )
      {
        QMessageBox::warning(
          this, tr( "Error" ),
          tr( "First select a place for the new %1.." ).arg( r->getElementName() ) );
        delete r;
        return;
      }
      ...
      while ( true )
      {
        // continue until we find either a parentRep or notice that this is the rootWidget
      ...
      {
        ScreenElement* newElem =
          r->create( parentRep->getScreenElement(),
                     screen );
        screen->addElement( newElem );
        parentRep->replaceChild(
          selectedWidget, selectedDelegate,
          newElem );
        selectedDelegate = 0;
        selectedWidget = 0;
      }
    ...
    void ScreenEditor::setupObjectPropertiesWidget()
    {
      ...
      connect( propeditor, SIGNAL( modified() ),
               this, SLOT( screenModified() ) );
      dw->setAllowedAreas( Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea );
      dw->setFeatures( QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable );
      dw->setWidget( propeditor );
      dw->setSizePolicy( QSizePolicy( QSizePolicy::Preferred, QSizePolicy::Expanding ) );
    ...
    void ScreenEditor::setupActions()
    {
      ...
      QAction* saveExperimentAction = new QAction( tr( "Save &Experiment" ), fileMenu );
      connect( saveExperimentAction, SIGNAL( triggered() ),
               this, SIGNAL( saveAllScreens() ) );
      fileMenu->addAction( saveExperimentAction );

      QAction* saveExperimentAsAction = new QAction( tr( "Save Experiment &As" ), fileMenu );
      connect( saveExperimentAsAction, SIGNAL( triggered() ),
               this, SIGNAL( saveAllAs() ) );
      fileMenu->addAction( saveExperimentAsAction );

      QAction* saveScreenAction = new QAction( tr( "&Save Screen" ), fileMenu );
      connect( saveScreenAction, SIGNAL( triggered() ),
               this, SLOT( saveScreen() ) );
      fileMenu->addAction( saveScreenAction );

      QAction* newScreenAction = new QAction( tr( "&New Screen" ), fileMenu );
      connect( newScreenAction, SIGNAL( triggered() ),
               this, SIGNAL( newScreen() ) );
      fileMenu->addAction( newScreenAction );

      main->insertItem( tr( "&File" ), fileMenu );
      main->show();
    }
