...
bool HrtfFilter::prepare (unsigned numberOfFrames)
{
    ...
    try {
        if (filePath != newFilePath) {
            ...
            qDebug("Loading FIR filters from file %s", qPrintable(filePath));
            filters.clear();
...
{
    return QStringList() << "firfilter";
}

PluginFilterInterface *HrtfFilterCreator::createFilter
