...
Q_EXPORT_PLUGIN2 (amplifierfilter, FadeFilterCreator)
...
{
    if (name == "fadein" && channel == -1)
        return true;
    if (name == "fadeout" && channel == -1)
        return true;
    if (name == "disabled" && channel == -1)
        return true;
        ...
        const QString &value)
{
    ...
    if (name == "fadeout" && channel == -1) {
        ...
        if (!ok) {
            setErrorMessage (QString ("Unable to convert fadeout value %1")
                    .arg (value));
            return false;
        }
