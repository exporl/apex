...
int main (int argc, char *argv[])
{
    ...
    const unsigned channels = QString (argv[3]).toUInt();
    ...
    // Setup module
    std::auto_ptr<PluginFilterInterface> hrtfFilter (hrtfCreator->createFilter
        ("hrtf", channels, blockSize, sampleRate));
    if (!hrtfFilter.get()) {
        fprintf (stderr, "Unable to instantiate filter instance.\n");
        exit (6);
    }
    hrtfFilter->resetParameters();
    if (!hrtfFilter->setParameter ("uri", -1, argv[4])) {
        fprintf (stderr, "Unable to set coefficient file path: %s\n",
            qPrintable (hrtfFilter->errorMessage()));
        exit (7);
    }
    if (!hrtfFilter->setParameter ("angle", -1, argv[5])) {
        fprintf (stderr, "Unable to set angle: %s\n",
            qPrintable (hrtfFilter->errorMessage()));
        exit (8);
    }
    if (!hrtfFilter->setParameter ("limit", -1, argv[6])) {
