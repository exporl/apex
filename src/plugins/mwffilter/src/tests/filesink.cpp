...
void TestSyl::fileSink()
{
    ...
    Q_FOREACH (PluginFilterCreator *creator,
        ...
        if (creator->availablePlugins().contains ("filesink")) {
    ...
    std::auto_ptr<PluginFilterInterface> fileSinkFilter
        (fileSinkCreator->createFilter ("filesink", channels, blockSize,
                                        sampleRate));
    if (!fileSinkFilter.get())
        QFAIL ("Unable to instantiate fileSink filter instance.");
    fileSinkFilter->resetParameters();
    if (!fileSinkFilter->setParameter ("uri", -1, tempFile.fileName()))
        QFAIL (qPrintable (fileSinkFilter->errorMessage()));
    if (!fileSinkFilter->setParameter ("format", -1, format))
        QFAIL (qPrintable (fileSinkFilter->errorMessage()));
