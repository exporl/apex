...
// needs to come after Qt/windows.h
#include <iowkit.h>

Q_LOGGING_CATEGORY(APEX_LEDFEEDBACKPLUGIN, "apex.ledfeedbackplugin")
...
LedController::LedController() : d(new LedControllerPrivate)
{
    d->eString = "";
    ...
    if (numIows != 1)
        qCDebug(APEX_LEDFEEDBACKPLUGIN)
            << "Warning: multiple IOWarriors found, using the first one";
    ...
    /*for (int i=0; i<32; ++i)
    {
        ...
        write(value);
        qCDebug(APEX_LEDFEEDBACKPLUGIN, "%d, %x", i, value);
        usleep (500000);
    }*/

    qCDebug(APEX_LEDFEEDBACKPLUGIN, "Sending test pattern to leds");
    for (int i = 0; i < 13; ++i) {
        setLed(i + 1);
#ifdef Q_OS_WIN32
        Sleep(20);
...
void LedController::setLed(int number)
{
    ...
    // qCDebug(APEX_LEDFEEDBACKPLUGIN, "** writing %x", bits);
    write(~bits);
}
...
bool LedController::write(unsigned long value)
{
    ...
    case IOWKIT_PRODUCT_ID_IOW40:
        ...
        return IowKitWrite(devHandle, IOW_PIPE_IO_PINS, (PCHAR)&rep,
                           IOWKIT40_IO_REPORT_SIZE) == IOWKIT40_IO_REPORT_SIZE;
    ...
    case IOWKIT_PRODUCT_ID_IOW56:
        ...
        return IowKitWrite(devHandle, IOW_PIPE_IO_PINS, (PCHAR)&rep56,
