...
struct ParameterAdapterPrivate
{
    ...
    /**
     * @brief The Direction enum, direction has the value INIT when no answer is processed yet.
     ...
     * REMARK: this is different from the terminology used in literature, where UP is when the subject provides a wrong answer.
     */
    enum Direction {
        INIT,
        UP,
...
ParameterAdapter::ParameterAdapter(const data::AdaptiveProcedureData* parameters) :
...
void ParameterAdapter::updateParameter(const QVariant& answer,
                                       int trialNb,
...
void ParameterAdapter::updateStepsize(int trialNb)
{
    ...
    // stepsizes does. So, when the direction is UP we need to use the 'down stepsizes' and vice versa.
    ...
    qCDebug(APEX_RS) << "Updated stepsize for trial" << trialNb << "to" << d->currentStepsize;
}
