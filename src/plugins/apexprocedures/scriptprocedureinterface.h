...
public:
    ScriptProcedureInterface(QObject *parent = 0) : QObject(parent)
    {
    }
...
public slots:
    ...
    {
        ShowError(
            "Error: setupNextTrial should be implemented in Script procedure");
        return apex::data::Trial();
    }

    //! Prepare everything so the first trial can be started when StartOutput is
    //! called
    virtual QString firstScreen()
    {
        ShowError(
            "Error: firstScreen should be implemented in Script procedure");
        return QString();
    }
    ...
    processResult(const apex::ScreenResult *) const
    {
        ShowError(
            "Error: processResult should be implemented in script procedure.");
