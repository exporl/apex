...
void ProcedureDataParser::Parse(const QDomElement &p_base,
    ...
    if (p_name == "presentations") {
            ...
            for (QStringList::const_iterator constIterator = slist.constBegin();
                ...
                if (!ok)
                    throw ApexStringException(
                        "Part of choices/select not integer");
                //                if (pos>param->m_choices.nChoices)
                //                    throw(ApexStringException("Element of
                //                    choices/select can not be > choices
                //                    itself"));
                data->addStimulusInterval(pos - 1);
            }
        }

        for (QDomElement alternative = d.firstChildElement();
             !alternative.isNull();
        ...
        if (!data->choices().isValid()) {
            throw ApexStringException(tr("procedure/choices is invalid. "
                                         "Check whether each interval has an "
                                         "associated screen element"));
        }
    } else if (p_name == "corrector") {
        if (data->choices().hasMultipleIntervals()) {
            throw ApexStringException(
                tr("Corrector specified with >= 2 alternatives. This is "
                   "probably unwanted."));
        }
        ...
        if (p_value == "random")
            ...
            throw ApexStringException(
                "ProcedureDataParser::SetParameter: unknown order");
