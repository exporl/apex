    ...
    if (++trialsDone <= totalTrials) {
        if (lastAnswer.isEmpty()) {
            ...
            if (trials.isEmpty()) {
                api->stopWithError(
                    "TrainingProcedure",
                    QObject::tr("Didn't find a trial that corresponds with "
                                "button \"%1\".")
                        .arg(lastAnswer));
                return trial;
            } else {
                ...
                double waitAfter =
                    lastTrial ? 0.0 : data->pauseBetweenStimuli() / 1000.0;

                if (trialsDone != totalTrials)
                    Q_FOREACH (QString stimulus,
                               api->makeOutputList(data, trialData))
                        trial.addStimulus(stimulus, QMap<QString, QVariant>(),
                                          "", waitAfter);
            }
        }
    }
...
ResultHighlight
TrainingProcedure::processResult(const ScreenResult *screenResult)
{
...
QString TrainingProcedure::resultXml() const
{
    ...
    QStringList result;
    if (!previousAnswer.isEmpty()) { // answer and corresponding stimulus should
                                     // be happily together
        if (data->GetID().isEmpty()) {
            result.append(
                QLatin1String("<procedure type=\"apex:trainingProcedure\">"));
        } else {
            result.append(
                QString("<procedure type=\"apex:trainingProcedure\" id=\"%1\">")
                    .arg(data->GetID()));
        }
        ...
        result.append(QSL("<%1>%2</%1>")
                          .arg(QSL("correct"))
                          .arg(ApexTools::boolToString(lastTrial.answer() ==
                                                       previousAnswer)));
        result.append(QSL("<%1>%2</%1>")
                          .arg(QSL("answer"), xmlEscapedText(previousAnswer)));
        result.append(QSL("<%1>%2</%1>")
                          .arg(QSL("correct_answer"),
                               xmlEscapedText(lastTrial.answer())));
