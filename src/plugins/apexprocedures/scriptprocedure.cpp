...
namespace apex
{
ScriptProcedure::ScriptProcedure(ProcedureApi *api,
      ...
      data(dynamic_cast<const data::ScriptProcedureData *>(data))
{
    ...
    QString newName =
        Paths::searchFile(this->data->script(),
    ...
    if (newName.isEmpty())
        throw Exception(tr("Cannot find procedure plugin file: %1")
                            .arg(this->data->script()));

    qCDebug(APEX_RS, "Looking for %s", qPrintable(newName));

    qCDebug(APEX_RS, "Adding ScriptProcedureInterface object to QtScript");
    QScriptValue baseClass =
        m_scriptEngine.scriptValueFromQMetaObject<ScriptProcedureInterface>();
    m_scriptEngine.globalObject().setProperty("ScriptProcedureInterface",
                                              baseClass);
    ...
    // Expose Trial class
    QScriptValue trial =
        m_scriptEngine.scriptValueFromQMetaObject<data::Trial>();
    m_scriptEngine.globalObject().setProperty("Trial", trial);

    /*QScriptValue trialPValue = m_scriptEngine.newQObject(&trialProto);
    m_scriptEngine.setDefaultPrototype(qMetaTypeId<data::Trial*>(),
    trialPValue);

    data::Trial *trial= new data::Trial();

    QScriptValue trialObj =
    m_scriptEngine.newVariant(qVariantFromValue<data::Trial*>(trial));
    ...
    QScriptValue resultHighlight =
        m_scriptEngine
            .scriptValueFromQMetaObject<data::ScriptResultHighlight>();
    m_scriptEngine.globalObject().setProperty("ResultHighlight",
                                              resultHighlight);
    ...
    // load script, script is supposed to define scriptProcedure, which inherits
    // from scriptProcedureInterface
    QFile file(newName);
    if (!file.open(QIODevice::ReadOnly))
        throw ApexStringException("Cannot open plugin procedure script: " +
                                  newName);
    QString scriptdata(file.readAll());

    // attach the main script library to the current script
    int nApiLines = 0; // number of lines in the API, used to calculate correct
                       // error message line
    if (!api->pluginScriptLibrary().isEmpty()) {
        QFile file(Paths::searchFile(
            api->pluginScriptLibrary(),
            QStringList() << ApexPaths::GetNonBinaryPluginPath()));
        if (!file.open(QIODevice::ReadOnly))
            throw Exception(tr("Cannot open main script library file: %1")
                                .arg(api->pluginScriptLibrary()));
    ...
    const data::ScriptProcedureData *scriptProcedureData =
        dynamic_cast<const data::ScriptProcedureData *>(data);
    const data::tScriptProcedureParameterList &pp =
        scriptProcedureData->customParameters();

    for (data::tScriptProcedureParameterList::const_iterator it = pp.begin();
         it != pp.end(); ++it) {
        dataObject.setProperty(it->name,
                               QScriptValue(&m_scriptEngine, it->value));
    }

    dataObject.setProperty(
        "choices", QScriptValue(&m_scriptEngine,
                                (scriptProcedureData->choices()).nChoices()));
    dataObject.setProperty(
        "presentations",
        QScriptValue(&m_scriptEngine, scriptProcedureData->presentations()));
    QString orderString =
        (scriptProcedureData->order() == data::ProcedureData::RandomOrder)
            ? (QString("random"))
            : (QString("sequential"));
    ...
    if (m_scriptEngine.hasUncaughtException()) {
        ...
        if (lineNo < nApiLines)
            throw Exception(QString("Error in scriptProcedure API line %1: %2")
                                .arg(lineNo)
                                .arg(m_classname.toString()));
        throw Exception(QString("Error in scriptProcedure line %1: %2")
                            .arg(lineNo - nApiLines)
                            .arg(m_classname.toString()));
    }
    ...
    m_scriptEngine.globalObject().setProperty(
        ScriptProcedureWrapper::INSTANCENAME, m_classname);
