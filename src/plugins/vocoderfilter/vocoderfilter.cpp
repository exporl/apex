...
private:
    ...
    QVector<double> carriers; // sample at <frame,vocoderChannel>: carriers[frame*nrVocoderChannels + vocoderChannel]

    QVector<unsigned int> carrierPositions; // monitor position in time to avoid onset clicks for each sound block (carriers are loopable)
...
bool VocoderFilter::prepare (unsigned numberOfFrames)
{
    ...
    // Read filter coefficients for filter bank from file and store in memory
        qCDebug(APEX_VOCODERFILTER, "Loading IIR filters from file %s", qPrintable(filterBankFilename));
        qDeleteAll(filterBank);
        filterBank.clear();
    ...
    // Initialize vocoderChannelBuffers
        ...
        for ( int vocoderChannel = 0; vocoderChannel < vocoderChannelBuffers.size(); ++vocoderChannel) {
            delete[] vocoderChannelBuffers[vocoderChannel];
        }
        vocoderChannelBuffers.clear();
        for ( int vocoderChannel = 0; vocoderChannel < vocoderChannels; ++vocoderChannel) {
            vocoderChannelBuffers.append(new double[blockSize]);
        }

    // Read low-pass filter coefficients from file and store in memory
        qCDebug(APEX_VOCODERFILTER, "Loading lowpass IIR filter from file %s", qPrintable(lowpassFilterFilename));
        qDeleteAll(lowpassBank);
        lowpassBank.clear();
        for ( int vocoderChannel = 0; vocoderChannel < vocoderChannels; ++vocoderChannel) {
            // beter zou zijn om niet telkens dezelfde file te moeten inlezen (met copy constructor of zo)...
            IirFilter * lowpassFilter = IirFilter::load(lowpassFilterFilename)[0];
            lowpassBank.append(lowpassFilter);
        }
    // Read carrier samples from file and store in memory
        qCDebug(APEX_VOCODERFILTER, "Loading carrier samples from file %s", qPrintable(carriersFilename));
        QByteArray filePath =
            QFile::encodeName(carriersFilename);
        ...
        if (unsigned(readSfinfo.samplerate) != sampleRate) {
            setErrorMessage(QString("Source file %1 has wrong sample rate (%2 instead of %3)")
                    .arg(QFile::decodeName(filePath))
        ...
        if (readSfinfo.channels != vocoderChannels) {
            setErrorMessage(QString("Source file %1 has wrong number of channels (%2 instead of %3)")
                            .arg(QFile::decodeName(filePath))
...
void VocoderFilter::process (double * const *data)
{
    ...
    for ( int vocoderChannel = 0 ; vocoderChannel < vocoderChannels ; ++vocoderChannel ) {
    // Filter bank
        memcpy (vocoderChannelBuffers[vocoderChannel], *data, blockSize*sizeof(double));
        filterBank[vocoderChannel]->process(vocoderChannelBuffers[vocoderChannel],blockSize);

    // Half-wave rectification
        for ( unsigned int sample = 0 ; sample < blockSize ; ++sample ) {
            if ( vocoderChannelBuffers[vocoderChannel][sample] < 0 )
                vocoderChannelBuffers[vocoderChannel][sample] = - vocoderChannelBuffers[vocoderChannel][sample];
        }

    // Low-pass filter
        lowpassBank[vocoderChannel]->process(vocoderChannelBuffers[vocoderChannel],blockSize);

    // Loop carriers and modulate with envelope
        for ( unsigned int sample = 0 ; sample < blockSize ; ++sample ) {
            unsigned int carrierSample = (carrierPositions[vocoderChannel])*vocoderChannels + vocoderChannel;
            vocoderChannelBuffers[vocoderChannel][sample] *= carriers[carrierSample];
            carrierPositions[vocoderChannel] = (carrierPositions[vocoderChannel]+1) % carrierLength;
        }
...
PluginFilterInterface *VocoderFilterCreator::createFilter
