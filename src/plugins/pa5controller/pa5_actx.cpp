...
CLSID const &PA5actx::GetClsid()
{
    static CLSID const clsid = {
        ...
        0x300D,
        0x4CE3,
        {0x97, 0x74, 0xA4, 0xC3, 0x77, 0x50, 0x7B, 0xD7}};
    return clsid;
}
BOOL PA5actx::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
                     DWORD dwStyle, const RECT &rect, CWnd *pParentWnd,
...
BOOL PA5actx::Create(LPCTSTR lpszWindowName, DWORD dwStyle, const RECT &rect,
                     CWnd *pParentWnd, UINT nID, CFile *pPersist, BOOL bStorage,
                     BSTR bstrLicKey)
{
    return CreateControl(GetClsid(), lpszWindowName, dwStyle, rect, pParentWnd,
...
BOOL PA5actx::SetUser(long ParCode, float Val)
{
    ...
    InvokeHelper(0x5, DISPATCH_METHOD, VT_BOOL, (void *)&result, parms, ParCode,
                 Val);
    return result;
}
...
BOOL PA5actx::Display(LPCTSTR Text, long Position)
{
    ...
    InvokeHelper(0x7, DISPATCH_METHOD, VT_BOOL, (void *)&result, parms, Text,
                 Position);
    return result;
}
...
long PA5actx::ConnectPA5(LPCTSTR IntName, long DevNum)
{
    long result;
    static BYTE parms[] = VTS_BSTR VTS_I4;
    InvokeHelper(0x8, DISPATCH_METHOD, VT_I4, (void *)&result, parms, IntName,
                 DevNum);
    return result;
}
...
public:
    ...
    bool mf_bCreate(CRect *a_pRect, HWND hParent)
    {
        ...
        if (!CWnd::CreateEx(0, _T("STATIC"), "fl", WS_CHILD | WS_VISIBLE, 0, 0,
                            a_pRect->right, a_pRect->bottom, hParent,
                            (HMENU)1234))
            return false;

        CWnd::GetClientRect(a_pRect);

        if (!m_PA5.CreateControl(m_PA5.GetClsid(), "", WS_VISIBLE, *a_pRect,
                                 this, 5000, 0, FALSE, 0))
            return false;
...
Pa5ActX::Pa5ActX() : m_pContainer(new QWidget()), m_pObj(new PA5Wnd())
{
    CRect rect(0, 0, 5, 5);
    if (!m_pObj->mf_bCreate(&rect, m_pContainer->winId()))
        throw(ApexStringException(
            "Pa5ActX::Pa5ActX(): couldn't initialize ActiveX for Pa5"));
}
