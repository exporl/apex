...
class ScrambleSpectrumFilterCreator : public QObject, public PluginFilterCreator
{
    Q_OBJECT
...
class ScrambleSpectrumFilter : public QObject, public PluginFilterInterface
{
    Q_OBJECT
public:
...
ScrambleSpectrumFilter::ScrambleSpectrumFilter(unsigned channels,
                                               unsigned blockSize, double fs)
    : channels(channels), blockSize(blockSize), fs(fs), disabled(false)
{
    resetParameters();
}
...
bool ScrambleSpectrumFilter::isValidParameter(const QString &type,
                                              int channel) const
{
                                          ...
                                          const QString &value)
{
    ...
    if (type == "disabled" && channel == -1) {
        disabled = value == "true" || value == "1" || value == "yes" ||
                   value == "disabled";
        return true;
    }
...
bool ScrambleSpectrumFilter::prepare(unsigned numberOfFrames)
{
    ...
    while (newfreq < maxfreq) {
        ...
        apex::PeakFilter::peakFilter(B, A, gain, (oldfreq + newfreq) / 2,
                                     newfreq - oldfreq, fs);
...
ScrambleSpectrumFilterCreator::createFilter(const QString &name,
