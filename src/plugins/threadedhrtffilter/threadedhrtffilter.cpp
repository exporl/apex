...
class ThreadedHrtfFilterCreator : public QObject, public PluginFilterCreator
{
    Q_OBJECT
...
class ThreadedHrtfFilter : public QObject, public PluginFilterInterface
{
    Q_OBJECT
public:
...
ThreadedHrtfFilter::ThreadedHrtfFilter(unsigned channels, unsigned blockSize)
    : channels(channels), blockSize(blockSize), buffered(false)
{
...
void ThreadedHrtfFilter::process(double *const *data)
{
    ...
    for (unsigned channel = 0; channel < channels; ++channel)
        QMetaObject::invokeMethod(
            filters[channel], "process", Q_ARG(double *, data[channel]),
...
ThreadedHrtfFilterCreator::createFilter(const QString &name, unsigned channels,
                                        unsigned size,
