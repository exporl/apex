...
Q_EXPORT_PLUGIN2 (slm2250, SLM_2250Creator)
...
{
    if (name == "slm_2250") {
        qDebug("Returning slm_2250()");
        SoundLevelMeter* slm = new SLM_2250();
        if (!slm) {
            qDebug("Invalid slm pointer");
            return 0;
        }
        if(slm->errorString().size() > 1) {
            qDebug("slm error: %s", qPrintable(slm->errorString()));
            throw ApexStringException(slm->errorString());
        } else
            return slm;
    } else {
        qDebug("Invalid sound level meter name requested");
        return 0;
    }
}
...
{
    qDebug() << "The instrument:" << instrument;
    if(instrument.isEmpty()) qDebug("no instrument");

    try {
        connectInterface->Connect(instrument);
    }  catch (std::exception &e)
    {
        qDebug("exception in SLM_2250::connect: %s" , e.what());
    }
    catch (...)
    {
        qDebug("unknown exception in SLM_2250::connect" );
    }
