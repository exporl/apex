...
Q_EXPORT_PLUGIN2 (ledfeedback, LedFeedbackCreator)
...
private:
    ...
    std::auto_ptr<LedController> controller;
...
void LedFeedback::highLight(const QString& element)
{
    ...
    if ( mapping.contains(element) ) {
        ...
        qDebug() << "LedFeedback:: error cannot find response " << element;
    }
}
