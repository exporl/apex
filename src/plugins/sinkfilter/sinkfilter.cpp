...
class FileSinkFilterCreator : public QObject, public PluginFilterCreator
{
    Q_OBJECT
...
class FileSinkFilter : public QObject, public PluginFilterInterface
{
    Q_OBJECT
public:
                                  ...
                                  const QString &value)
{
    ...
    if (type == QLatin1String("useBertha") && channel == -1) {
        useBertha =
...
bool FileSinkFilter::prepare(unsigned numberOfFrames)
{
    ...
    try {
        ...
        if (useBertha)
            filePath.replace(QLatin1String("${count}"),
                             QString::number(berthaCount));
        else
        ...
        if (!formats.contains(format)) {
            setErrorMessage(
                QString::fromLatin1("Unknown file format %1").arg(format));
            return false;
        }
...
FileSinkFilterCreator::createFilter(const QString &name, unsigned channels,
