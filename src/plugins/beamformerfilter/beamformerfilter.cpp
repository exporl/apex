...
class BeamformerFilterCreator : public QObject, public PluginFilterCreator
{
    Q_OBJECT
    Q_INTERFACES(PluginFilterCreator)
#if QT_VERSION >= 0x050000
    Q_PLUGIN_METADATA(IID "apex.beamformerfilter")
#endif
public:
...
class BeamformerFilter : public QObject, public PluginFilterInterface
{
    Q_OBJECT
public:
...
{
    return QStringList() << QLatin1String("beamformer");
}

PluginFilterInterface *
BeamformerFilterCreator::createFilter(const QString &name, unsigned channels,
                                      unsigned size, unsigned sampleRate) const
{
    Q_UNUSED(sampleRate);

    try {
        if (name == QLatin1String("beamformer"))
