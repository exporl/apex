...
int upgraderCore::majorVersion =
    currentVersionString.section(".", 0, 0).toInt();
int upgraderCore::minorVersion =
    currentVersionString.section(".", 1, 1).toInt();
...
void upgraderCore::determineVersion()
{
    ...
    QString versionMessage1 = tr("Original version: %1.%2.%3")
                                  .arg(oldVersion[0])
                                  .arg(oldVersion[1])
                                  .arg(oldVersion[2]);
    QString versionMessage2 = tr("Current version: %1.%2.%3")
                                  .arg(majorVersion)
                                  .arg(minorVersion)
                                  .arg(subVersion);
...
void upgraderCore::startUpgrade()
{
    ...
    QScopedPointer<apex::ExperimentParser> parser(
        new apex::ExperimentParser(sourceFile));
    try {
        parser->parse(false, false);
    } catch (const std::exception &e) {
        QString message =
            QString("Could not convert experiment file: %1").arg(e.what());
        emit displayMessage(message);
    } catch (...) {
        QString message =
            QString("Unknown exception while converting %1").arg(sourceFile);
        emit displayMessage(message);
    }
...
void upgraderCore::saveOutputFile()
{
    ...
    if (convertedText.isEmpty()) {
        emit displayMessage(
            "Unknown error during conversion, no output written");
        return;
    }
