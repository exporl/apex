...
namespace editor
{
ScreenElement *FlashPlayerEditorDelegate::getScreenElement()
{
    return element;
}

FlashPlayerEditorDelegate::FlashPlayerEditorDelegate(FlashPlayerElement *e,
                                                     QWidget *parent,
                                                     ScreenWidget *w)
...
int FlashPlayerEditorDelegate::getPropertyCount()
{
    return ScreenElementEditorDelegate::getPropertyCount() + 5;
}

QString FlashPlayerEditorDelegate::getPropertyName(int nr)
{
...
QVariant FlashPlayerEditorDelegate::getPropertyData(int nr, int role)
{
    if (nr < ScreenElementEditorDelegate::getPropertyCount())
...
PropertyType FlashPlayerEditorDelegate::getPropertyType(int nr)
{
    if (nr < ScreenElementEditorDelegate::getPropertyCount())
...
bool FlashPlayerEditorDelegate::setProperty(int nr, const QVariant &v)
{
    if (nr < ScreenElementEditorDelegate::getPropertyCount())
...
void FlashPlayerEditorDelegate::mouseReleaseEvent(QMouseEvent *ev)
{
    handleMouseReleaseEvent(ev, this);
}

const QString FlashPlayerEditorDelegate::getText() const
{
    QString def = element->getDefault();
    if (def.isEmpty())
        def = tr("Not set");
    def = shortenString(def, 30);
    QString overrideFeedback =
        element->getOverrideFeedback() ? tr("yes") : tr("no");
    QString highlight = element->getHighlight();
    highlight = shortenString(highlight, 30);
    if (highlight.isEmpty())
        highlight = tr("Not set");
...
FlashPlayerEditorDelegate::~FlashPlayerEditorDelegate()
{
