...
MediaSelector::MediaSelector(QWidget *parent) : QWidget(parent)
{
    ...
    button->setIcon(QIcon(QLatin1String("fileopen")));
...
class FlashFileSelector : public MediaSelector
{
    Q_OBJECT
public:
...
{
    return QFileDialog::getOpenFileName(this, tr("Select Flash File"),
                                        QString(), "Flash files (*.swf)");
}

FlashFileSelector::FlashFileSelector(QWidget *parent) : MediaSelector(parent)
{
    populateCombo();
}
...
class ImageFileSelector : public MediaSelector
{
    Q_OBJECT
public:
...
ImageFileSelector::ImageFileSelector(QWidget *parent) : MediaSelector(parent)
{
    populateCombo();
}
                                                 ...
                                                 const QModelIndex &index) const
{
    ...
    const ObjectPropertiesModel *model =
        dynamic_cast<const ObjectPropertiesModel *>(m);
    Q_ASSERT(model);
    ...
    case PositiveIntPropertyType:
        val = v.toInt();
        Q_ASSERT(dynamic_cast<QSpinBox *>(editor));
...
void ObjectPropertiesItemDelegate::setModelData(QWidget *editor,
                                                ...
                                                const QModelIndex &index) const
{
    ...
    Q_ASSERT(dynamic_cast<ObjectPropertiesModel *>(m));
    ...
    case FlashPropertyType:
        ...
        Q_ASSERT(ffs);
        text = ffs->getValue();
        qCDebug(APEX_RS, "setting flash prop text: %s", qPrintable(text));
        rep->setProperty(row, text);
        break;
    case ImagePropertyType:
        ifs = dynamic_cast<ImageFileSelector *>(editor);
        Q_ASSERT(ifs);
        text = ifs->getValue();
        qCDebug(APEX_RS, "setting image prop text: %s", qPrintable(text));
        rep->setProperty(row, text);
        break;
    default:
        break;
    ...
    const QModelIndex &index) const
{
    ...
    case ArcLayoutTypePropertyType:
        ...
        connect(comboBox, SIGNAL(activated(int)), this, SLOT(valueChanged()));
        return comboBox;
    case TwoPartLayoutDirectionPropertyType:
        comboBox = new QComboBox(parent);
    ...
    case FlashPropertyType:
        ...
        connect(flashFileSelector, SIGNAL(valueChanged()), this,
                SLOT(valueChanged()));
        return flashFileSelector;
    case ImagePropertyType:
        ifs = new ImageFileSelector(parent);
        connect(ifs, SIGNAL(valueChanged()), this, SLOT(valueChanged()));
        return ifs;
    default:
        return 0;
    }
...
QVariant ObjectPropertiesModel::headerData(int section,
...
PropertiesEditor::PropertiesEditor(QWidget *parent)
    : QTreeView(parent), selectedRep(0), model(new ObjectPropertiesModel(this))
{
...
ObjectPropertiesItemDelegate::ObjectPropertiesItemDelegate(
    ObjectPropertiesModel *m, QObject *parent)
    : QItemDelegate(parent), model(m), invaluechanged(false)
{
}

QSize ObjectPropertiesItemDelegate::sizeHint(const QStyleOptionViewItem &option,
                                             const QModelIndex &index) const
{
    Q_UNUSED(option);
    Q_UNUSED(index);
