                                  ...
                                  const QString& filePattern)
{
    ...
    {
        qCDebug(APEX_STATEMACHINEVISUALIZER) << "Filename should contain" << REPLACE_PATTERN;
        return;
    }
...
void StateMachineVisualizer::drawChildStates(const QAbstractState* abstractParent,
                                             const QString& filePattern)
{
    ...
    QString fileName = QString(filePattern).replace(REPLACE_PATTERN, parentLabel);
    QFile file(fileName);

    if (!file.open(QFile::WriteOnly | QFile::Text | QFile::Truncate))
    {
        qCDebug(APEX_STATEMACHINEVISUALIZER) << "Failed to open file" << fileName << "for writing";
        return;
    }
    ...
    Q_FOREACH (const QAbstractState* abstractState, states)
    {
        ...
        Q_FOREACH (const QAbstractTransition* transition, state->transitions())
        {
            Q_FOREACH (const QAbstractState* target, transition->targetStates())
            {
                if (!idMap.contains(target))
                {
                    qCDebug(APEX_STATEMACHINEVISUALIZER) << "While drawing state" << parentLabel
                             << "transition from state" << label(state)
                             << "to state" << label(target)
