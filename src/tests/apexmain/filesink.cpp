...
void ApexMainTest::fileSink_data()
{
...
void ApexMainTest::fileSink()
{
    ...
    QScopedPointer<PluginFilterInterface> fileSinkFilter(
        fileSinkCreator->createFilter(QLatin1String("filesink"), channels,
                                      blockSize, sampleRate));
    if (!fileSinkFilter)
        QFAIL("Unable to instantiate fileSink filter instance.");
    fileSinkFilter->resetParameters();
    if (!fileSinkFilter->setParameter(QLatin1String("file"), -1,
                                      tempFile.fileName()))
    ...
    fileInfo.format = 0;
    SNDFILE *dataFile =
        sf_open(QFile::encodeName(tempFile.fileName()), SFM_READ, &fileInfo);
    QVERIFY(dataFile);
