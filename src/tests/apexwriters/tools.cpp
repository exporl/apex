...
//    qCDebug(APEX_RS, "text content: %s", qPrintable(QString::fromUtf16(node->getFirstChild()->getTextContent())));
...
bool CompareDocuments ( QString file1, QString file2 )
{
        ...
        qCDebug(APEX_RS, "Differences between file %s and %s", qPrintable ( file1 ),
                 qPrintable ( file2 ) );
        qCDebug(APEX_RS, "-------------------------------------------------------" );
        ...
        if ( result )
                qCDebug(APEX_RS, "Differences found for file %s", qPrintable ( file1 ) );
        else
                qCDebug(APEX_RS, "No differences" );
