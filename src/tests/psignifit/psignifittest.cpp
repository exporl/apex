...
void PsignifitTest::initTestCase()
{
...
void PsignifitTest::testPsignifitWrapper_data()
{
    ...
    QTest::newRow("Default example")
        << QString("1.00000000     0.50000000     40\n"
        ...
        << (QList<double>() << 2.11401479565334149413 << 2.78304545915526491129
                            << 3.45207612265718832845)
        << (QList<double>() << 0.33153502504380333304 << 0.51802347663094272523
                            << 0.33153502504380327753);

    QTest::newRow("Single data point") << QString("1 0.2 20\n"
                                                  "#shape logistic\n"
                                                  "#n_intervals 2\n"
                                                  "#runs 99\n")
...
void PsignifitTest::testPsignifitWrapper()
{
    ...
    PsignifitResult *result = w.psignifitS(data);
    ...
    for (int i = 0; i < result->thresholds().length(); ++i) {
        // qCDebug(APEX_RS, "%1.20f", result->thresholds()[i]);
    ...
    qCDebug(APEX_RS) << "TH_LIMS: " << result->thresholdsLims();
    qCDebug(APEX_RS) << "SL_LIMS: " << result->slopesLims();
