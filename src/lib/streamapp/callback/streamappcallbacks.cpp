...
void Channel::mf_Callback()
{
  ...
  {
    if( m_pProcessor )
      m_pPlaySink->Write( m_pProcessor->mf_DoProcessing( m_pPlaySource->Read() ) );
    else
...
{
  mt_OwnedDetail( const bool ac_bSerialOutput ) :
    m_Outputs( ac_bSerialOutput )
  {
    m_pInput = &m_Inputs;
    m_pOutput = &m_Outputs;
    m_pProc = &m_Procs;
...
struct StreamAppChannel::mt_NotOwnedDetail : public StreamAppChannel::mt_Detail
{
  mt_NotOwnedDetail( const bool ac_bSerialOutput ) :
    m_Outputs( ac_bSerialOutput )
  {
    m_pInput = &m_Inputs;
    m_pOutput = &m_Outputs;
...
StreamAppChannel::StreamAppChannel( const bool ac_bDeleteContent, const bool ac_bSerialOutput )
{
...
bool StreamAppChannel::mf_bCheckValidConfig() const
{
  ...
  {
    if( ( c_nStrIchan >= m_pDetail->m_pProc->mf_nGetNumInputChannels() ) &&  ( m_pDetail->m_pProc->mf_nGetNumOutputChannels() >= c_nStrOchan ) )
      return true;
  }
  else
  {
