...
namespace streamapp
{
  ...
  public:
      /**
        * Constructor.
        * @param ac_bDeleteContent if true, deletes all items when killed or when removed
        */
  ...
  private:
    ...
    /**
      ...
      *   multiple input streams are read parallel, bundeled into one stream and serve
      *   as input for any number of sequential processors. The output of last processor,
      *   or the input stream if no processors present, is distributed over the outputstreams
  ...
  public:
      /**
        * Constructor.
        */
    StreamAppChannel( const bool ac_bDeleteContent = true, const bool ac_bSerialOutput = false );
    ...
    INLINE const unsigned& mf_nGetNumProcessors() const;
      ...
      /**
        ...
        * Better assert this true before running Callback or nasty things will happen.
