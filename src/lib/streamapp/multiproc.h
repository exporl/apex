...
/**
  * SequentialProcessor
  *   combines any number of subsequent IStreamProcessors into one
  *IStreamProcessor.
  *   Any processor added must have the same number of inputs as the numkber of
  *outputs
  *   of the one before it. Buffersize must be the same for all processors
  *added.
  *************************************************************************************
  */
...
public:
    /**
      * Constructor.
      * @param ac_bDeleteProcsWhenDeleted if true, deletes the items added when
     * destructing or removing an item.
      */
...
public:
    ...
    /**
      ...
      * @param ac_bDeleteWhenDone if true, deletes the processor when it's
     * plugged out
    ...
    void mp_SetItem(const unsigned ac_nItem, IStreamProcessor *const ac_pItem,
                    const bool ac_bDeleteWhenDone = true,
                    const bool ac_bBypassNow = false);
