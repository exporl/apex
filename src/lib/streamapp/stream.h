...
/**
  ...
  * -# The Processor, which takes data and puts it out again ( Write() followed
 * by Read() == Process() )
  *
  * With these three one can describe the entire audio/video/whatever data
 * operations: I->P->O.
  * Off course I->O is also possible, but there aren't anymore schemes to be
 * found on this world.
  * Unless when you think of an Input as a Processor that only outputs stuff
 * without needing input,
  * and of an Output as a Processor that only takes stuff without giving
 * anything back,
  * then theres just P, which looks quite boring and uncreative to me, and that
 * is probably
...
public:
    ...
    /**
      ...
      * anywhere, yet. This will be documented however so there are no
     * surprises.
      * @param ac_Input the Stream to process
      * @return the resulting Stream, can be same ac ac_Input if nothing is done
     * with it
      */
...
public:
    ...
    StreamProcessor(const unsigned ac_nChannels, const unsigned ac_nBufferSize)
        : mc_nChan(ac_nChannels), mc_nSize(ac_nBufferSize)
    {
    }
...
public:
    ...
    /**
      ...
      * any garbage (you don't want to hear the last few samples over and over
     * again, do you?)
      * @return
      */
    ...
    /**
      * The number of samples contained in one channel of the Stream returned by
     * Read().
      * This must not change during streaming.
      * @return the number
      */
    ...
    /**
      ...
      *   s.Read();
      * std::cout << "The last call to Read() only had " <<
     * mf_lGetNumSamplesRead() << " new samples.";
      * std::cout << " The remaining " << s.mf_nGetBufferSize() -
     * s.mf_lGetNumSamplesRead() << " were all zero.\n";
      * @endcode
...
/**
  ...
  *   beforehand when the stream will be exhausted. Moreover, it probably never
  *will,
  ...
  *   is not, it's just paused, and the last Read() call did contain all new
  *samples.
  ...
  *   Combining these easily leads to the fact you can loop it, hereby making it
  *act
  *   like it's parent, the great infinite InputStream.
  *
  *   When a PositionableInputStream is looped, it will automatically put the
  *first
  *   sample after the mf_lTotalSamples()th one, independent of the position in
  *the
  *   buffer where this should happen. Eg with num loops == 2, for the outside
  *world
  ...
  *   Btw the reason for putting looping in this class although it might seem
  *more
  *   logical to supply a utility class to do it, using this class'
  *mf_lSamplesLeft()
  *   and other methods, is that it is way faster because the implementation can
  *   use the same as it uses for reading the samples, without having to copy
  *anything.
  *************************************************************************************
  */
...
public:
    ...
    /**
      * Get the position of next sample ( = first one of next block ) going to
     * be read.
    ...
    /**
      ...
      * @param ac_nPosition the position, must be between 0 and
     * mf_lTotalSamples()
      */
    ...
    /**
      ...
      * Setting num loops while playing will only succeed if ac_nLoops is
     * greater
...
public:
    ...
    /**
      ...
      * but it's no problem to pass a lot more in. Depending on the
     * implementation,
      * it's also no problem to pass in less: it will just write as much as put
     * in.
    ...
    /**
      ...
      * By my knowledge, no class implements this, it's here just for
  decoration.
      * TODO
      * @return the number
