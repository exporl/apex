...
namespace dataconversion
{
  ...
  { return ( ac_dGainIndB <= streamapp::sc_dMixerMin ) ? 0.0 : pow( 10.0 , ac_dGainIndB / 20.0 ); }
  ...
  { return ( ac_dGainInLin <= 0.0 ) ? streamapp::sc_dMixerMin : ( 20.0 * log10( ac_dGainInLin ) ); }
  ...
  /**
    ...
    * This is about 6% faster than using the normal c++ cast to convert a double to an int, and
    * it will round the value to the nearest integer, instead of rounding it down like the normal cast does.
    * The constant used is 6755399441055744 = 2^52 + 2^51. The method is machine dependent as it relies on the
  ...
  { return (((int)bytes[0]) << 16) | (((unsigned int)(unsigned char)bytes[1]) << 8) | ((unsigned int)(unsigned char)bytes[2]); }
    ...
    /**
      ...
      */
  INLINE void littleEndian24BitToChars( const int value, char* const destBytes ) throw()
  { destBytes[0] = (char)(value & 0xff); destBytes[1] = (char)((value >> 8) & 0xff); destBytes[2] = (char)((value >> 16) & 0xff); }
    ...
    /**
      ...
      */
  INLINE void bigEndian24BitToChars( const int value, char* const destBytes ) throw()
  { destBytes[0] = (char)((value >> 16) & 0xff); destBytes[1] = (char)((value >> 8) & 0xff); destBytes[2] = (char)(value & 0xff); }
    ...
    /**
      ...
      */
  template< class tIntegerType >
  INLINE tIntegerType gf_nSamplesFromMsec( const double ac_dFs, const double ac_dMilliSeconds )
  {
