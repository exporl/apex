...
public:
    ...
    /**
      ...
      */
    static const unsigned tempBufSize =
        1440 * 8; // optimum for 1024 ac_nSamples, 2 channels  ??

    /**
      * The header size.
      */
    static const unsigned long waveHeaderSize =
        sizeof(RIFF) + sizeof(FMT) + sizeof(DATA);
...
/**
  * InputWaveFile
  *   PositionableAudioFormatReader implementation for linear pcm wavefiles
  *(Microsoft).
  *   @see OutputWaveFile
  **************************************************************************************
  */
class InputWaveFile : public PositionableAudioFormatReader, private WaveFileData
{
public:
    /**
      * Constructor.
    ...
    /**
      * Close the file.
      * @return false if the file is not open, or if there was an error closing
     * it.
      */
    bool mp_bClose();
...
private:
    ...
    unsigned long
        totalSampleCount; // keep as member to avoid calculating it everytime
...
/**
  ...
  *   @see OutputWaveFile
  ************************************************************** */
class OutputWaveFile : public AudioFormatWriter, private WaveFileData
{
public:
    OutputWaveFile();
    ~OutputWaveFile();
    ...
    /**
      ...
      * @return false if the file cannot be opened or if the parameters are not
     * supported by the format
      */
    bool
    mp_bOpen(const QString &fileName, const unsigned channelCount,
             const unsigned long ac_lFs,
             const AudioFormat::mt_eBitMode ac_eMode = AudioFormat::MSBint16);
