...
AudioFormatReaderStream::AudioFormatReaderStream(
    AudioFormatReader *const ac_cpSource, const unsigned ac_nBufferSize,
...
void AudioFormatReaderStream::mp_ReplaceReader(
    AudioFormatReader *const ac_pSource, const bool ac_bDeleteReader)
{
    PtrCheck(ac_pSource);
...
AudioFormatWriterStream::AudioFormatWriterStream(
    AudioFormatWriter *const ac_cpSource, const unsigned ac_nBufferSize,
    const bool ac_bDeleteWriter)
    : OutputStream(),
      m_pSource(ac_cpSource),
      m_Output(ac_cpSource->mf_nChannels(),
               ac_nBufferSize), // allocates buffer but is not always used
      mv_bEOF(false),
...
void AudioFormatWriterStream::mp_ReplaceWriter(
    AudioFormatWriter *const ac_cpSource, const bool ac_bDeleteWriter)
{
...
void PositionableAudioFormatReaderStream::mp_ReplaceReader(
    PositionableAudioFormatReader *const ac_pSource,
    const bool ac_bDeleteReader)
{
    PtrCheck(ac_pSource);
...
const Stream &PositionableAudioFormatReaderStream::Read()
{
    ...
    if (mf_bIsLooping()) {
        if (nLeft < nSamples && // loop end passed
            ...
            m_Output.mp_ClearRegion(nLeft, nSamples - nLeft);
...
PositionableAudioFormatReaderStream::mf_lSamplesLeft() const
{
    return mf_lTotalSamples() - mf_lCurrentPosition();
}

INLINE unsigned long
PositionableAudioFormatReaderStream::mf_lTotalSamples() const
{
    return mv_nLoops * m_pSource->mf_lTotalSamples();
}

INLINE unsigned long
PositionableAudioFormatReaderStream::mf_lCurrentPosition() const
{
    if (mf_bIsLooping())
        return (mv_nLooped * m_pSource->mf_lTotalSamples()) +
               m_pSource->mf_lCurrentPosition();
    return m_pSource->mf_lCurrentPosition();
}

void PositionableAudioFormatReaderStream::mp_SeekPosition(
...
INLINE void
PositionableAudioFormatReaderStream::mp_SetNumLoops(const unsigned ac_nLoops)
{
    const Lock L(mc_ReadLock);
