...
namespace streamapp
{
                  ...
                  const PaStreamCallbackTimeInfo* timeInfo, PaStreamCallbackFlags, void* userData )
  {
    ...
    assert( p->m_nBufferSize == framesPerBuffer );
  ...
  class PortAudioInitter : public appcore::Singleton< PortAudioInitter >
  {
      ...
      mv_bError( false )
    {
      ...
      /**
        ...
        */
    void mp_GetDriverInfos( std::string& a_sError ) throw()
    {
  ...
  public:
    ...
    bool mf_bSeriousError() const throw()
    {
      ...
      /**
        ...
        * @return string array
        */
    tStringVector mf_GetDriverNames( std::string& a_sError ) throw()
    {
      if( m_saDrivers.empty() )
    ...
    const tDrvrInfos& mf_GetDriverInfos() throw()
    {
      ...
      /**
        ...
        */
    PaDeviceIndex mf_nGetInputDeviceID( const std::string& ac_sDriverName ) throw()
    {
      ...
      {
        if( ac_sDriverName == m_saDrivers.at( i ) && m_DriverInfos.at( i )->maxInputChannels )
      ...
      /**
        ...
        */
    PaDeviceIndex mf_nGetOutputDeviceID( const std::string& ac_sDriverName ) throw()
    {
        std::cout << "getting device id for driver " << ac_sDriverName << std::endl;
      if( ac_sDriverName == sc_sDefault.toStdString() )
      ...
      for( tStringVector::size_type i = 0 ; i < nNames ; ++i )
      {
        ...
        if( ac_sDriverName == m_saDrivers.at( i ) && m_DriverInfos.at( i )->maxOutputChannels )
          return (PaDeviceIndex) i;
      }
      return paNoDevice;
    }
      ...
      unsigned& a_nInChannels, unsigned& a_nOutChannels, std::vector< unsigned long >& a_SampleRates )
    {
      ...
      for( unsigned i = 0 ; i < nRatesToTry ; ++i )
        if( Pa_IsFormatSupported( &in, &out, RatesToTry[ i ] ) == paFormatIsSupported )
  ...
  public:
    ...
    {
      assert( ac_nSamples == m_Host.mf_nGetBufferSize() );
      assert( a_pBuf );
...
PortAudioWrapper::PortAudioWrapper( const std::string& ac_sDriverName ) :
  m_pPaStream( 0 ),
  m_pUserData( new PaUserData() ),
  mv_bOpen( false ),
  mv_nBuffers( sc_nNumBuffers )
{
	std::cout << "PortAudioWrapper::PortAudioWrapper " << ac_sDriverName << std::endl;
  ...
  m_iInputID = PortAudioInitter::sf_pInstance()->mf_nGetInputDeviceID( ac_sDriverName );
  m_iOutputID = PortAudioInitter::sf_pInstance()->mf_nGetOutputDeviceID( ac_sDriverName );

  if( m_iInputID == paNoDevice && m_iOutputID == paNoDevice )
  {
	  std::cout <<  "Portaudio problems" << std::endl;
    std::string sDummy;
    if( PortAudioInitter::sf_pInstance()->mf_GetDriverNames( sDummy ).empty() )
      throw StringException( "Portaudio: no soundcards found" );
    else
      throw StringException( "Portaudio: couldn't open driver:" + ac_sDriverName );
  }
}
