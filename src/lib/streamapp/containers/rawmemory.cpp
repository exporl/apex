...
RawMemoryAccess::RawMemoryAccess( void* const ac_pMemory, const unsigned ac_nSize, const bool ac_bInitToZero ) :
  m_pData( (char*) ac_pMemory ),
  mv_nSize( ac_nSize )
{
  if( ac_bInitToZero )
    mp_FillWith( 0 );
...
{
  return ( mv_nSize == ac_Rh.mf_nGetSize() ) && memcmp( m_pData, ac_Rh.mf_pGetMemory(), mv_nSize );
}
...
void RawMemoryAccess::mp_CopyFrom( const void* const ac_pToCopy, const unsigned ac_nDestOffset, const unsigned ac_nSize )
{
  const unsigned nToCopy = s_min( mv_nSize - ac_nDestOffset, ac_nSize );
  memcpy( m_pData + ac_nDestOffset, ac_pToCopy, nToCopy );
}

void RawMemoryAccess::mf_CopyTo( void* const ac_pToCopy, const unsigned ac_nSourceOffset, const unsigned ac_nSize ) const
{
...
namespace
{
  char* f_Allocate( const unsigned ac_nInitialSize, const bool ac_bInitToZero = false )
  {
...
RawMemoryAccess RawMemory::mf_GetAccess( const unsigned ac_nOffset, const unsigned ac_nSize )
{
...
void RawMemory::mp_ResizeIfSmaller( const unsigned ac_nNewSize, const bool ac_bInitToZero )
{
  if( ac_nNewSize > mv_nSize )
    mp_Resize( ac_nNewSize, ac_bInitToZero );
}

void RawMemory::mp_RemovePart( const unsigned ac_nStartOffset, const unsigned ac_nSize )
{
  ...
  {
    memmove( m_pData + ac_nStartOffset, m_pData + ac_nStartOffset + ac_nSize, mv_nSize - ( ac_nStartOffset + ac_nSize ) );
    mp_Resize( mv_nSize - ac_nSize );
  }
}

void RawMemory::mp_ShiftDown( const unsigned ac_nStartOffset, const unsigned ac_nSize, const bool ac_bClear /*= true*/ )
{
...
void RawMemory::mp_Append( const void* const ac_pToAppend, const unsigned ac_nSize )
{
  if( ac_nSize > 0 )
  {
