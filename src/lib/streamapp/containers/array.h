...
public:
    /**
      ...
      */
    ArrayAccess(tType *const ac_pBuffer, const unsigned ac_nBufferSize)
        : mc_pArray(ac_pBuffer), mc_nBufferSize(ac_nBufferSize)
    {
    }
    ...
    /**
      ...
      */
    INLINE void mp_Set(const unsigned ac_nPos, const tType ac_dfVal)
    {
...
/**
  ...
  */
template <class tType>
tType *f_InitArrayMem(const unsigned ac_nBufferSize, const bool ac_bInitMem)
{
    if (ac_nBufferSize != 0) {
...
public:
    /**
      ...
      */
    ArrayStorage(const unsigned ac_nBufferSize, const bool ac_bInitMem = false)
        : ArrayAccess<tType>(f_InitArrayMem<tType>(ac_nBufferSize, ac_bInitMem),
                             ac_nBufferSize)
    {
    }
    ...
    ArrayStorage(const ArrayStorage &ac_Rh)
        : ArrayAccess<tType>(
              f_InitArrayMem<tType>(ac_Rh.mf_nGetBufferSize(), false),
...
/**
  ...
  *   Use this for arrays of pointers if you want mem to be cleaned up
  *automatically.
  ***********************************************************************************
  */
template <class tType>
...
/**
  ...
  * @param ac_nIndexToStartSearch the index to start searching at, use to
 * improve speed
  * @return the index, or -1 if the item wasn't found
  */
template <class tType>
int sf_nFindInArray(const tType *ac_pToFind,
                    const ArrayAccess<tType *> &ac_ToSearch,
                    const int ac_nIndexToStartSearch = 0)
{
