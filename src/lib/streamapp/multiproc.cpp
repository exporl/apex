...
SequentialProcessor::SequentialProcessor(
    const bool ac_bDeleteProcsWhenDeleted /* = true*/)
    : DynProcList(4), mc_bDeleteContent(ac_bDeleteProcsWhenDeleted)
{
}
SequentialProcessor::~SequentialProcessor()
{
...
void SequentialProcessor::mp_AddItem(IStreamProcessor *const ac_pItem)
{
    ...
    if (mf_nGetNumItems() != 0) {
        Q_ASSERT(
            mf_GetItem(mf_nGetNumItems() - 1)->mf_nGetNumOutputChannels() >=
            ac_pItem->mf_nGetNumInputChannels()); // exception? voiD?
    }
...
void SlotProcessor::mp_SetItem(const unsigned ac_nItem,
                               IStreamProcessor *const ac_pItem,
                               const bool ac_bDeleteWhenDone,
                               const bool ac_bBypassNow)
{
