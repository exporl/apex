...
void MultiInputStream< tOwned >::mp_UpdateMemory()
{
  if( !m_pRead || ( m_pRead->mf_nGetBufferSize() != this->mv_nChan ) )
  {
    ...
    m_pResult = new Stream( m_pRead->mf_pGetArray(), this->mv_nChan, this->mv_nSize );
  }
}
...
template< bool tOwned >
void MultiPosInputStream< tOwned >::mp_SeekPosition( const unsigned long ac_nPosition )
{
...
void MultiOutputStream< tOwned >::Write( const Stream& ac_ToWrite )
{
  ...
  else
  {
    ...
    const unsigned nChanToWrite = ( nChanCanWrite >= nChanShouldWrite ) ? nChanShouldWrite : nChanCanWrite;

      //try to write all output streams..
    for( unsigned i = 0 , nChan = 0 ; i < nStrToWrite && nChan < nChanToWrite ; ++i )
    {
      const unsigned c_nCurNumOchan = this->mf_GetItem( i )->mf_nGetNumChannels();
      ...
      const Stream curStr( write.mf_pGetArray( ), c_nCurNumOchan, this->mv_nSize );
