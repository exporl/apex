...
namespace
{
  typedef std::pair<std::string,ConnectItem*>     tConnectablePair;
  typedef std::pair<std::string,StreamCallback*>  tUnConnectablePair;
...
ConnectItem::ConnectItem(const unsigned ac_nBufferSize, const unsigned ac_nSockets ) :
...
void ConnectItem::mp_AddInput( StreamCallback* const ac_pItem, const unsigned ac_nItemOutChannel )
{
  int nIndex = mf_nGetFirstFreeChannel();
  if( nIndex != -1 )
...
void ConnectItem::mp_AddInput( StreamCallback* const ac_pItem, const unsigned ac_nItemOutChannel, const unsigned ac_nThisInChannel )
{
  ...
  //std::cout << toString(this) + " mp_AddInput()" + toString( ac_pItem ) << std::endl;
  //bTrig = true;
}
...
void ConnectItem::mp_RemoveInput( const unsigned ac_nThisInChannel )
{
  ...
  //std::cout << toString(this) + " mp_RemoveInput()" + toString( ac_nThisInChannel ) << std::endl;
}
...
void ConnectItem::mp_RemoveInput( StreamCallback* const ac_pItem, const unsigned ac_nItemOutChannel )
{
  int nIndex = sf_nFindInArray( ac_pItem, m_Inputs );
  while( nIndex != -1 )
  {
...
void ConnectItem::mp_RemoveInput( StreamCallback* const ac_pItem, const unsigned ac_nItemOutChannel, const unsigned ac_nThisInChannel )
{
  int nIndex = sf_nFindInArray( ac_pItem, m_Inputs );
  while( nIndex != -1 )
  {
    if( ac_nItemOutChannel == (unsigned) m_InputChans( nIndex ) && (unsigned) nIndex == ac_nThisInChannel )
    {
      mp_RemoveInput( nIndex );
      nIndex = -1;
    }
...
DeleteAtDestructionArray<ConnectItem::mt_eConnectionInfo>* ConnectItem::mf_pGetConnections() const
{
  DeleteAtDestructionArray<ConnectItem::mt_eConnectionInfo>* pRet = new DeleteAtDestructionArray<ConnectItem::mt_eConnectionInfo>( mc_nChan );
  for( unsigned i = 0 ; i < mc_nChan ; ++i )
    pRet->mp_Set( i, new ConnectItem::mt_eConnectionInfo( m_Inputs( i ), m_InputChans( i ), i ) );
  return pRet;
}
...
void ConnectItem::mf_CallbackFunc()
{
  for( unsigned i = 0 ; i < mc_nChan ; ++i )
  {
    ...
    /*if( bTrig )
      std::cout << toString( pToCall ) << std::endl;*/
    if( pToCall )
    {
      ...
      {
        m_Result.ResetChannel( i );     //must be done everytime if connecting from other thread??
      }
    }
  }
  //bTrig = false;
...
int ConnectionManager::mf_nGetItemIndex( const std::string& ac_sID ) const
{
...
ConnectItem* ConnectionManager::mf_pGetConnectable( const std::string& ac_sID ) const
{
  tConCIt CoIt = m_Connectable.find( ac_sID );
  if( CoIt != m_Connectable.end() )
    return (*CoIt).second;
...
StreamCallback* ConnectionManager::mf_pGetUnConnectable( const std::string& ac_sID ) const
{
  tUConCIt UCoIt = m_UnConnectable.find( ac_sID );
  if( UCoIt != m_UnConnectable.end() )
    return (*UCoIt).second;
...
StreamCallback* ConnectionManager::mf_pGetAny( const std::string& ac_sID ) const
{
...
std::string ConnectionManager::mf_sGetAny( StreamCallback* const ac_pID ) const
{
...
void ConnectionManager::mp_RegisterItem( const std::string& ac_sId, InputStream* const ac_pInput, const bool ac_bPlaying )
{
...
void ConnectionManager::mp_RegisterItem( const std::string& ac_sId, OutputStream* const ac_pOutput )
{
  if( mf_nGetItemIndex( ac_sId ) != -1 )
    return;
  const Lock L( *mc_pLock );
  m_Connectable.insert( tConnectablePair( ac_sId, new OutputStreamCallback( ac_pOutput ) ) );
  //std::cout << "mp_RegisterItem" + ac_sId << std::endl;
}

void ConnectionManager::mp_RegisterItem( const std::string& ac_sId, IStreamProcessor* const ac_pProcessor )
{
  if( mf_nGetItemIndex( ac_sId ) != -1 )
    return;
  const Lock L( *mc_pLock );
  m_Connectable.insert( tConnectablePair( ac_sId, new ProcessorCallback( ac_pProcessor ) ) );
  //std::cout << "mp_RegisterItem" + ac_sId << std::endl;
}

void ConnectionManager::mp_RegisterItem( const std::string& ac_sId, ConnectItem* const ac_pInput )
{
  if( mf_nGetItemIndex( ac_sId ) != -1 )
    return;
...
void ConnectionManager::mp_Connect( const std::string& ac_sFromID, const std::string& ac_sToID )
{
...
void ConnectionManager::mp_Connect( const std::string& ac_sFromID, const std::string& ac_sToID, const unsigned ac_nFromChannel, const unsigned ac_nToChannel )
{
  ConnectItem*      pTo = mf_pGetConnectable( ac_sToID );
  StreamCallback* pFrom = mf_pGetAny( ac_sFromID );
...
void ConnectionManager::mp_RemoveConnection( const std::string& ac_sFromID, const std::string& ac_sToID )
{
  ConnectItem*      pTo = mf_pGetConnectable( ac_sToID );
  StreamCallback* pFrom = mf_pGetAny( ac_sFromID );
  Q_ASSERT( pTo && pFrom );
...
void ConnectionManager::mp_RemoveConnection( const std::string& ac_sFromID, const std::string& ac_sToID, const unsigned ac_nFromChannel, const unsigned ac_nToChannel )
{
  ConnectItem*      pTo = mf_pGetConnectable( ac_sToID );
  StreamCallback* pFrom = mf_pGetAny( ac_sFromID );
  Q_ASSERT( pTo && pFrom );
...
void ConnectionManager::mp_UnregisterItem( const std::string& ac_sID )
{
...
void ConnectionManager::mp_PlayItem( const std::string& ac_sItem, const bool ac_bPlay /* = true  */ )
{
...
bool ConnectionManager::mf_bIsRegistered( const std::string& ac_sItem ) const
{
...
bool ConnectionManager::mf_bAllInputsConnected() const throw()
{
  ...
  while( itV != itE )
  {
    ...
    DeleteAtDestructionArray<ConnectItem::mt_eConnectionInfo>* p = cur.mf_pGetConnections();
    infos.mp_AddItem( p );
    ++itV;
  }
  ...
  while( itUV != itUE )
  {
    ...
    {
      DeleteAtDestructionArray<ConnectItem::mt_eConnectionInfo>* pCur = infos.mf_GetItem( i );
...
tStringVector ConnectionManager::mf_saGetRoutes() const
{
  ...
  while( itV != itE )
  {
    const std::string& sCurO = (*itV).first;
    DeleteAtDestructionArray<ConnectItem::mt_eConnectionInfo>* cur = (*itV).second->mf_pGetConnections();
    ...
    for( unsigned j = 0 ; j < nCurItems ; ++j )
    {
      ...
      std::string sCur = mf_sGetAny( rCur->m_pFrom );
      ret.push_back( sCur + " channel " + toString( rCur->m_nFromChannel ) + " -> " + sCurO + " channel " + toString( rCur->m_nToChannel ) );
    }
