...
void ConnectItem::mp_RemoveInput(const unsigned ac_nThisInChannel)
{
    ...
    //std::cout << toString(this) + " mp_RemoveInput()" + ac_nThisInChannel << std::endl;
}
...
StreamCallback* ConnectionManager::mf_pGetUnConnectable(const QString& ac_sID) const
{
    tUConCIt UCoIt = m_UnConnectable.find(ac_sID);
    if (UCoIt != m_UnConnectable.end())
        return (*UCoIt).second;
...
void ConnectionManager::mp_RegisterItem(const QString& ac_sId, InputStream* const ac_pInput, const bool ac_bPlaying)
{
...
void ConnectionManager::mp_RegisterItem(const QString& ac_sId, OutputStream* const ac_pOutput)
{
    if (mf_nGetItemIndex(ac_sId) != -1)
        return;
    const Lock L(*mc_pLock);
...
void ConnectionManager::mp_RegisterItem(const QString& ac_sId, IStreamProcessor* const ac_pProcessor)
{
    if (mf_nGetItemIndex(ac_sId) != -1)
        return;
    const Lock L(*mc_pLock);
...
void ConnectionManager::mp_RegisterItem(const QString& ac_sId, ConnectItem* const ac_pInput)
{
    if (mf_nGetItemIndex(ac_sId) != -1)
        return;
...
void ConnectionManager::mp_Connect(const QString& ac_sFromID, const QString& ac_sToID)
{
...
void ConnectionManager::mp_Connect(const QString& ac_sFromID, const QString& ac_sToID, const unsigned ac_nFromChannel, const unsigned ac_nToChannel)
{
    ConnectItem*      pTo = mf_pGetConnectable(ac_sToID);
    StreamCallback* pFrom = mf_pGetAny(ac_sFromID);
...
void ConnectionManager::mp_RemoveConnection(const QString& ac_sFromID, const QString& ac_sToID)
{
    ConnectItem*      pTo = mf_pGetConnectable(ac_sToID);
    StreamCallback* pFrom = mf_pGetAny(ac_sFromID);
    Q_ASSERT(pTo && pFrom);
...
void ConnectionManager::mp_RemoveConnection(const QString& ac_sFromID, const QString& ac_sToID, const unsigned ac_nFromChannel, const unsigned ac_nToChannel)
{
    ConnectItem*      pTo = mf_pGetConnectable(ac_sToID);
    StreamCallback* pFrom = mf_pGetAny(ac_sFromID);
    Q_ASSERT(pTo && pFrom);
...
void ConnectionManager::mp_PlayItem(const QString& ac_sItem, const bool ac_bPlay /* = true  */)
{
...
tStringVector ConnectionManager::mf_saGetRoutes() const
{
    ...
    while (itV != itE) {
        ...
        for (unsigned j = 0 ; j < nCurItems ; ++j) {
            ...
            ret.push_back(QString(sCur + " channel " + rCur->m_nFromChannel + " -> " + sCurO + " channel " + rCur->m_nToChannel).toStdString());
        }
