                                    ...
                                    const unsigned ac_nSize)
{
    if (!a_pTarget) // first time new
        ...
        if (a_pTarget->mf_nGetChannelCount() !=
            ac_nChan) // new if different nChan
        {
            delete a_pTarget;
            return (new StreamBuf(ac_nChan, ac_nSize, true));
        } else // zeroize if ok
        {
            for (unsigned i = 0; i < ac_nChan; ++i) {
                memset(a_pTarget->mf_pGetArray()[i], 0,
                       ac_nSize * sizeof(GainType));
            }
            return a_pTarget;
        }
    }
...
public:
    /**
      ...
      */
    MatrixMixer(const unsigned ac_nBufSize, const unsigned ac_nChan = 64)
    ...
    /**
      ...
      */
    void mp_SetMatrixNode(const unsigned ac_nIChan, const unsigned ac_nOChan,
                          const GainType ac_dGaindB)
    {
    ...
    /**
      ...
      */
    GainType mf_dGetMatrixNode(const unsigned ac_nIChan,
                               const unsigned ac_nOChan) const
    {
    ...
    {
        const unsigned nIChanToCopy =
            s_min(ac_MatrixToCopy.mf_nGetNumInputChannels(), m_nOChan);
        const unsigned nOChanToCopy =
            s_min(ac_MatrixToCopy.mf_nGetNumOutputChannels(), m_nIChan);
    ...
    {
        const unsigned nIChan =
            s_min(ac_dInput.mf_nGetChannelCount(), m_nIChan);
