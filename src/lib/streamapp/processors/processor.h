...
namespace streamapp
{
      ...
      else
      {
        for( unsigned i = 0 ; i < nChan ; ++i )
        {
          for( unsigned j = 0 ; j < nSize ; ++j )
          {
            ...
            const StreamType dNewv = ( ac_Source( i, j ) + dPrev ) / mv_dCount;
  ...
  {
  public:
    IOProcessor( IStreamProcessor* const ac_pProc, InputStream* const ac_pStream, const bool ac_bDeleteContent = false ) :
  ...
  {
  public:
    IOProcessor2( OutputStream* const ac_pOStream, InputStream* const ac_pIStream, const bool ac_bDeleteContent = false ) :
      mc_bDeleteContent( ac_bDeleteContent ),
  ...
  {
  public:
    Amplifier( const unsigned ac_nChan, const unsigned ac_nSize, const bool ac_bInit = true ):
        StreamProcessor( ac_nChan, ac_nSize ),
      mv_dGains( ac_nChan )
    {
      if( ac_bInit )
    ...
    virtual const Stream& mf_DoProcessing( const Stream& ac_Output )
    {
      ...
      for( unsigned i = 0 ; i < StreamProcessor::mf_nGetNumInputChannels() ; ++i )
  ...
  {
  public:
    AmplifierNR( const unsigned ac_nChan, const unsigned ac_nSize, const bool ac_bInit = true ) :
        Amplifier( ac_nChan, ac_nSize, ac_bInit ),
      m_Buf( ac_nChan, ac_nSize )
    {
    }
