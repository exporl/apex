...
Compare::mt_eResult Compare::sf_eCompare(AudioFormatReader &a_Input0,
                                         AudioFormatReader &a_Input1) throw()
{
    ...
    while (nRead == sc_nReadSize) {
        ...
        for (unsigned i = 0; i < nChan; ++i) {
            if (memcmp(buf0.mf_pGetArray()[i], buf1.mf_pGetArray()[i],
                       nRead * nBytesPerSample))
                return mc_eNotIdentical;
        }
    }
...
Compare::mt_eResult Compare::sf_eCompare(Compare::mt_Readers &a_Input0,
                                         AudioFormatReader &a_Input1) throw()
{
...
Compare::mt_eResult Compare::sf_eCompareWave(const QString &ac_sFile0,
                                             const QString &ac_sFile1) throw()
{
    try {
        InputWaveFile fin0;
...
Compare::mt_eResult Compare::sf_eCompareWave(const Compare::mt_Files &ac_sFile0,
                                             const QString &ac_sFile1) throw()
{
    try {
