...
struct tLoopLogicBase {
    s_static_assert_inclass(types::tIsReference<tType>::value == false,
                            AlreadyAReference)

        typedef tType tExtType;
    typedef tType &mt_RefType;
    typedef
        typename mpl::tIf<tUseReferences, mt_RefType, tExtType>::type tIntType;
};
...
public:
    ...
    LoopLogic(const tExtType ac_nLength)
        : mv_nRead(0), mv_nLoopStart(0), mv_nLoopEnd(0), mc_nLength(ac_nLength)
    {
    }
    ...
    /**
      ...
      */
    LoopLogic(tIntType ac_nLength, tIntType a_nLoopStart, tIntType a_nLoopEnd,
              tIntType a_nRead)
        : mv_nRead(a_nRead),
    ...
    tType mf_nGetPiece(const tType ac_nRequested) const
    {
        ...
        else if (this->mv_nLoopEnd < this->mv_nLoopStart &&
                 this->mv_nRead >= this->mv_nLoopStart)
            nMoved = this->mc_nLength - this->mv_nRead;
        else if (this->mv_nLoopEnd < this->mv_nLoopStart &&
                 this->mv_nRead <= this->mv_nLoopEnd)
    ...
    tType mf_nGetNewPos(const tType ac_nMoved)
    {
        ...
        if (this->mv_nLoopEnd > this->mv_nLoopStart &&
            ...
            return this->mv_nLoopStart;
                 ...
                 this->mv_nRead == this->mc_nLength)
            return 0;
        else if (this->mv_nLoopEnd < this->mv_nLoopStart &&
                 this->mv_nRead == this->mv_nLoopEnd + 1)
            return this->mv_nLoopStart;
        else
    ...
    s_track_mem_leaks(LoopLogic)
        ...
        private : LoopLogic(const LoopLogic &);
