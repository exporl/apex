...
void ScrollZoomer::rescale()
{
    ...
    /*double d = verticalScrollBar()->minSliderValue();
    scrollBarMoved( Qt::Vertical, d, 0.0 );*/

    /*QSize cv = mf_CanvasSize();
    QSize onePixelBigger( 1, 1 );
    cv += onePixelBigger;
    QResizeEvent evt( cv, cv );
    this->eventFilter( canvas(), &evt );*/
    //mp_DoProperResize( &evt );
}
...
void ScrollZoomer::mp_DoProperResize( QResizeEvent* e )
{
...
QSize ScrollZoomer::mf_CanvasSize() const
{
  return ((QwtPlotCanvas *)canvas())->geometry().size();
}
...
bool ScrollZoomer::eventFilter(QObject *o, QEvent *e)
{
    if (  o == canvas() )
    {
        switch(e->type())
        {
            case QEvent::Resize:
            {
              mp_DoProperResize( (QResizeEvent*) e );
              break;
            }
