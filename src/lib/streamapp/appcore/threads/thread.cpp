...
void IThread::sf_ThreadEntryPoint(IThread *const ac_pThreadToRun)
{
    ...
    if (ac_pThreadToRun->mc_StartWaiter.mf_eWaitForSignal(10000) ==
        WaitableObject::wait_ok) {
...
void IThread::mp_Stop(const int ac_nTimeoutInMSec)
{
    ...
    if (mf_bIsRunning()) {
        ...
        if (mf_bIsRunning()) {
            ...
            // std::cout << "Thread " << mc_sName.c_str() << " killed without
            // exiting thread function!" << std::endl;
...
void IThread::sf_SetCurrentThreadPriority(
    const IThread::mt_ePriority ac_Priority)
{
...
{
    return mc_Waiter.mf_eWaitForSignal(ac_nTimeoutInMSec) ==
           WaitableObject::wait_ok;
}
