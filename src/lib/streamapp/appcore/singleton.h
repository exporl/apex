        ...
        if (sm_pInstance == 0) {
            ...
            if (sm_pInstance == 0) {
                ...
                if (!bInside) {
                    ...
                    Q_ASSERT(0 && "two ore more threads tried to create the "
                                  "Singleton's insatnce at the same time!");
                }
            }
        }
        return sm_pInstance;
...
protected:
    ...
    virtual ~Singleton()
    {
        mf_ClearInstance(); // if only one can be born, there will be nothing
                            // left when that one dies
    }
...
public:
    /**
      * Constructor.
      */
    SingletonDestroyer() : m_pDoomed(0)
    {
    }
