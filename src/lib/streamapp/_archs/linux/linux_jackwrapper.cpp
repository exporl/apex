    ...
    bool mp_bMakePorts()
    {
        if (m_nIChan) {
            ...
            for (unsigned i = 0; i < m_nIChan; ++i) {
                ...
                jack_port_t *p = jack_port_register(
                    m_pClient, portname, JACK_DEFAULT_AUDIO_TYPE,
                    JackPortIsInput, m_nBufferSize);
                if (p)
                    m_pInputPorts[i] = p;
                else
                    return false;
        ...
        if (m_nOChan) {
            ...
            for (unsigned i = 0; i < m_nOChan; ++i) {
                ...
                jack_port_t *p = jack_port_register(
                    m_pClient, portname, JACK_DEFAULT_AUDIO_TYPE,
                    JackPortIsOutput, m_nBufferSize);
                if (p)
                    m_pOutputPorts[i] = p;
                else
                    return false;
...
{
public:
    JackFormatReader(tJackUserData &a_Parent) : m_Source(a_Parent)
    {
    }
...
{
public:
    JackFormatWriter(tJackUserData &a_Parent) : m_Source(a_Parent)
    {
    }
    ~JackFormatWriter()
    {
    ...
    unsigned long Write(const void **a_pBuf, const unsigned ac_nSamples)
    {
        ...
        for (unsigned i = 0; i < m_Source.m_nOChan; ++i)
            for (unsigned j = 0; j < m_Source.m_nBufferSize; ++j)
                m_Source.m_pOutput[i][j] =
                    (float)(pSrc[i][j]) / sc_f32BitMinMax;
#endif
        return ac_nSamples;
    }
    ...
    if (p->m_bRunning) {
        ...
        if (p->m_pInput) {
            for (unsigned i = 0; i < p->m_nOChan; ++i)
                p->m_pInput[i] =
                    (float *)jack_port_get_buffer(p->m_pInputPorts[i], nFrames);
        }
        if (p->m_pOutput) {
            for (unsigned i = 0; i < p->m_nOChan; ++i)
                p->m_pOutput[i] = (float *)jack_port_get_buffer(
                    p->m_pOutputPorts[i], nFrames);
#ifdef TEST_JACK_OUTPUT
        ...
        for (unsigned i = 0; i < p->m_nOChan; ++i) {
            float *pDest =
                (float *)jack_port_get_buffer(p->m_pOutputPorts[i], nFrames);
...
/**
  ...
  */
void sf_GetDriverNames(const char **ac_pcJackNames,
                       const std::string &ac_sDrvName, tStringVector &a_Dest)
{
    if (ac_pcJackNames) {
        const char *cSep = ":";
        const char *cOtherSep = "_";
      ...
      mv_bOpen(0)
{
    ...
    sf_GetDriverNames(jack_get_ports(m_pData->m_pClient, 0, 0,
                                     JackPortIsPhysical | JackPortIsInput),
                      ac_sDrvName, m_JackPortsIn);
    sf_GetDriverNames(jack_get_ports(m_pData->m_pClient, 0, 0,
                                     JackPortIsPhysical | JackPortIsOutput),
                      ac_sDrvName, m_JackPortsOut);
    if (m_JackPortsIn.empty() && m_JackPortsIn.empty()) {
        delete m_pData;
        throw(utils::StringException("JackWrapper: driver " + ac_sDrvName +
                                     " not found"));
    }
}
...
tStringVector JackWrapper::sf_saGetDriverNames(std::string &a_sError)
{
    ...
    sf_AddDrvrNames(
        jack_get_ports(pClient, 0, 0, JackPortIsPhysical | JackPortIsInput),
        ret);
    sf_AddDrvrNames(
        jack_get_ports(pClient, 0, 0, JackPortIsPhysical | JackPortIsOutput),
        ret);
                                 ...
                                 const unsigned ac_nBufferSize)
{
    ...
    for (unsigned i = 0; i < m_pData->m_nIChan; ++i) {
        if (jack_connect(m_pData->m_pClient, m_JackPortsIn.at(i).c_str(),
                         jack_port_name(m_pData->m_pInputPorts[i])))
            return false;
    }
