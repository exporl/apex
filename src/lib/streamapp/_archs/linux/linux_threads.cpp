...
WaitableObject::mt_eWaitResult
WaitableObject::mf_eWaitForSignal(const int ac_nTimeOut) const
{
    EventStruct *const es = (EventStruct *)m_hHandle;
    ...
    if (!es->triggered) {
        if (ac_nTimeOut < 0) {
            ...
            while (!timeout) {
                timeout =
                    pthread_cond_timedwait(&es->condition, &es->mutex, &time);
                if (!timeout)
                    // Success
                    break;
...
// TODO ANDROID pthread_cancel not supported on android
        ...
        qCCritical(
            APEX_RS,
            "Attempt to kill thread, android doesn't support pthread_cancel");
        return;
#else
...
{
    // a bit complicated on linux.. needs pid which is hard to get + priority
    // rules are weird
    // FIXMEEEEEE this is really needed, I have no idea what priority the
    // IThreads run at now,
    // but it could as well be super-high.. face it, windows threads are better
    // lol
}
