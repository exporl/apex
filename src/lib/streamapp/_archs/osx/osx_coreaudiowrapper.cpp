...
namespace streamapp
{
  ...
  struct tCoreAudioUserData
  {
    ...
    void mp_FillChannelInfos( const bool ac_bAsInput )
    {
      ...
      if( AudioDeviceGetPropertyInfo( m_DevID, 0, ac_bAsInput, kAudioDevicePropertyStreamConfiguration, &nSize, 0 ) == noErr )
      {
        AudioBufferList* const pBufList = (AudioBufferList*) ::malloc( nSize );
        if( AudioDeviceGetProperty( m_DevID, 0, ac_bAsInput, kAudioDevicePropertyStreamConfiguration, &nSize, pBufList ) == noErr )
        {
          ...
          for( int i = 0 ; i < nStreams; ++i )
          {
            ...
            for( unsigned j = 0 ; j < b.mNumberChannels ; ++j )
            {
              if( ac_bAsInput )
              {
                ...
                m_InputInfo[ nChannels ].m_nStride    = b.mNumberChannels;
              }
              else
              {
                m_OutputInfo[ nChannels ].m_nStreamNum = i;
                m_OutputInfo[ nChannels ].m_nOffset    = j;
                m_OutputInfo[ nChannels ].m_nStride    = b.mNumberChannels;
              }
              ++nChannels;
            }
          }
      ...
      ac_bAsInput ? m_nMaxInputChan = nChannels : m_nMaxOutputChan = nChannels;
    }
  ...
  OSStatus sf_CoreAudioCB( AudioDeviceID,
                           const AudioTimeStamp*,
                           const AudioBufferList* a_pInputData,
                           const AudioTimeStamp*,
                           AudioBufferList* a_pOutputData,
                           const AudioTimeStamp*,
                           void* a_pData )
  {
    ...
    if( p->m_pCallback )
    {
      ...
      for( unsigned i = 0 ; i < p->m_nIChan ; ++i )
      {
        ...
        const float* pSrc = ( (const float*) a_pInputData->mBuffers[ tInfo.m_nStreamNum ].mData ) + tInfo.m_nOffset;
      ...
      for( unsigned i = 0 ; i < p->m_nOChan ; ++i )
      {
        ...
        float* pDest = ( (float*) a_pOutputData->mBuffers[ tInfo.m_nStreamNum ].mData ) + tInfo.m_nOffset;
        const int nStride = tInfo.m_nStride;
        for( unsigned j = 0 ; j < p->m_nBufferSize ; ++j )
        {
          *pDest = *pSrc++;
  ...
  {
  public:
    CoreAudioFormatReader( tCoreAudioUserData& a_Parent ) :
      m_Source( a_Parent )
    {
    }
  ...
  {
  public:
    CoreAudioFormatWriter( tCoreAudioUserData& a_Parent ) :
      m_Source( a_Parent )
    {
    }
    ~CoreAudioFormatWriter()
  ...
  mv_bOpen( 0 )
{
  if( ac_sDrvName == sc_sDefault )
  {
    ...
    if( AudioHardwareGetProperty( kAudioHardwarePropertyDefaultOutputDevice, &nSize, &m_pData->m_DevID ) != noErr )
      throw( utils::StringException( "CoreAudioWrapper: couldn't get default device ID" ) );
  }
  else
  {
    std::string sDummy;
    ...
    if( !nDevs )
      throw( utils::StringException( "CoreAudioWrapper: no audio devices found" ) );
    ...
    if( !bGotIt )
      throw( utils::StringException( "CoreAudioWrapper: couldn't find driver for " + ac_sDrvName ) );
  }
...
tSoundCardInfo CoreAudioWrapper::mf_GetInfo() const
{
  ...
  if( AudioDeviceGetProperty( m_pData->m_DevID, 0, false, kAudioDevicePropertyNominalSampleRate, &nSize, &sr ) == noErr )
  {
  ...
  UInt32 bs;
  nSize = sizeof( bs );
  if( AudioDeviceGetProperty( m_pData->m_DevID, 0, false, kAudioDevicePropertyBufferFrameSize, &nSize, &bs ) == noErr )
  {
    Ret.m_BufferSizes.push_back( bs );
    m_pData->m_nBufferSize = bs;
  }

  if( AudioDeviceGetPropertyInfo( m_pData->m_DevID, 0, false, kAudioDevicePropertyUsesVariableBufferFrameSizes, &nSize, 0 ) == noErr )
    std::cout << "CoreAudioWrapper: warning buffersize might change and this is not implemented" << std::endl;
...
tStringVector CoreAudioWrapper::sf_saGetDriverNames( std::string& a_sError )
{
  ...
  UInt32 nSize;
  if( AudioHardwareGetPropertyInfo( kAudioHardwarePropertyDevices, &nSize, 0 ) == noErr )
  {
    AudioDeviceID* const pDevs = (AudioDeviceID*) ::malloc( nSize );
    if( AudioHardwareGetProperty( kAudioHardwarePropertyDevices, &nSize, pDevs ) == noErr )
    {
      ...
      for( unsigned i = 0 ; i < nDevs ; ++i )
      {
        char name[ 1024 ];
        nSize = sizeof( name );
        if( AudioDeviceGetProperty( pDevs[ i ], 0, false, kAudioDevicePropertyDeviceName, &nSize, name ) == noErr )
        {
        ...
        {
          a_sError = "CoreAudioWrapper: kAudioDevicePropertyDeviceName failed";
        }
      }
      if( !nDevs )
  ...
  {
    a_sError = "CoreAudioWrapper: kAudioHardwarePropertyDevices info failed";
  }
...
bool CoreAudioWrapper::mp_bStart( Callback& a_Callback )
{
  if( mv_bOpen )
  {
    if( AudioDeviceAddIOProc( m_pData->m_DevID, sf_CoreAudioCB, m_pData ) == noErr )
    {
