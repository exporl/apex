...
WaitableObject::WaitableObject() : m_hHandle(CreateEvent(0, 0, 0, 0))
{
}
...
void *f_CreateThread(void *a_pUserData)
{
    ...
    return (void *)_beginthreadex(0, 0, &stdf_nThreadEntryPoint, a_pUserData, 0,
                                  &threadId);
}
...
void IThread::sf_Sleep(const int ac_nMilliSeconds)
{
    if (ac_nMilliSeconds >= 10) {
        ...
        // unlike Sleep() this is guaranteed to return to the current thread
        // after
        // the time expires, so we'll use this for short waits, which are more
        // likely
