...
public:
    /**
      ...
      */
    AudioFormatConvertor(const unsigned ac_nChannels,
    ...
    /**
      * Read from a source.
      * After a read, the class' array(s) contains new samples which can be
     * accessed
      * using all normal MatrixAccess methods.
      * If the source is finished, the method returns less samples then
     * ac_nSamples.
      ...
      * @param ac_nSourceOffset the offset to start reading samples , must be <=
     * buffersize
      * @return the number of samples actually read
      */
    unsigned ReadFromSource(AudioFormatReader *const ac_pSource,
                            const unsigned ac_nSamples,
                            const unsigned ac_nSourceOffset);

    /**
     * Write to a source.
     * Before writing, place new samples into the class' array(s) using
     * MatrixAccess' methods,
     ...
     */
    unsigned WriteToSource(AudioFormatWriter *const a_cpSink,
                           const unsigned ac_nSamples);
    ...
    /**
      ...
      * @warning if this is for the soundcard stream, make sure not to block too
     * long,
