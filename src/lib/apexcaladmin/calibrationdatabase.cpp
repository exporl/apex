...
HardwareSetups CalibrationDatabase::QStringList2HardwareSetups(const QStringList& names, bool isLocal) const {
...
HardwareSetups CalibrationDatabase::globalHardwareSetups() const {
    return QStringList2HardwareSetups(hardwareSetupNames(globalSettings), false);
}
...
void CalibrationDatabase::makeGlobal(HardwareSetup hardwareSetup) {
    if(hardwareSetup.isGlobal()) {
        throw std::domain_error(tr("The hardware setup %1 is already global").arg(hardwareSetup.name()).toStdString());
    }
...
void CalibrationDatabase::makeLocal(HardwareSetup hardwareSetup) {
    if(hardwareSetup.isLocal()) {
        throw std::domain_error(tr("The hardware setup %1 is already local").arg(hardwareSetup.name()).toStdString());
    }

    move(hardwareSetup.name(), globalSettings, localSettings);
}

void CalibrationDatabase::move(const QString& hardwareSetupName, QSettings* from, QSettings* to) {
    if(!isWritable()) {
        throw std::domain_error(tr("Can't write the settings, are you running this program as administrator?").toStdString());
    }

    //Add setup
    QStringList newHardwareSetupsTo = to->value(DatabaseNames::hardwareSetup).toStringList();
    newHardwareSetupsTo.append(hardwareSetupName);
    to->setValue(DatabaseNames::hardwareSetup, newHardwareSetupsTo);

    //Remove setup
    QStringList newHardwareSetupsFrom = from->value(DatabaseNames::hardwareSetup).toStringList();
