...
The .bin-file can be generated with the MATLAB function a3iirfilter_write_bin.m,
...
*/
    ...
    for (unsigned i = 0; i < channels; ++i) {
        QVector<double> B (P);
        QVector<double> A (Q);
        if (file.read (reinterpret_cast<char*> (B.data()), 8 * P) !=
                8 * P)
            throw ("Unable to read filter coefficients");
        if (maximumLength)
            B.resize (qMin (unsigned (B.size()), maximumLength));

        if (file.read (reinterpret_cast<char*> (A.data()), 8 * Q) !=
                8 * Q)
            throw ("Unable to read filter coefficients");
        if (maximumLength)
