...
int ArcLayoutPrivate::doLayout (const QRect &rect, bool activate) const
{
    ...
    const double arc = items.count() < 3 ? ARC_PI :
        arcType == ArcLayout::ARC_FULL ? 2 * ARC_PI / (items.count() - 1) :
        ARC_PI / (items.count() - 2);

    QSize minimumSize;
    Q_FOREACH (QLayoutItem *item, items)
        minimumSize = minimumSize.expandedTo (item->minimumSize());
    const double ratio = arcLayoutFlags & ArcLayout::ARC_RECTANGULAR ?
            double (minimumSize.width()) / minimumSize.height() : 1.0;

    const double radiusFromWidth = qMax
       (rect.width() / ((arcType & (ArcLayout::ARC_LEFT | ArcLayout::ARC_RIGHT)
            ? 1 : 2) + 2 / sqrt (1 + 1 / (ratio * ratio)) * sin (arc / 2)),
        rect.width() / ((arcType & (ArcLayout::ARC_LEFT | ArcLayout::ARC_RIGHT)
            ? 1 : 2) + 1 / sqrt (1 + 1 / (ratio * ratio))));

    const double radiusFromHeight = qMax
       (rect.height() / ((arcType & (ArcLayout::ARC_TOP | ArcLayout::ARC_BOTTOM)
            ? 1 : 2) + 2 / sqrt (1 + (ratio * ratio)) * sin (arc / 2)),
        rect.height() / ((arcType & (ArcLayout::ARC_TOP | ArcLayout::ARC_BOTTOM)
            ? 1 : 2) + 1 / sqrt (1 + (ratio * ratio))));
    ...
    if (radiusFromHeight < radiusFromWidth && activate) {
        ...
        height = rect.height() - (arcType & (ArcLayout::ARC_TOP |
                    ArcLayout::ARC_BOTTOM) ? 1 : 2) * radius;
        width = unsigned (ratio * height);
    } else {
        radius = unsigned (ceil (radiusFromWidth));
        width = rect.width() - (arcType & (ArcLayout::ARC_LEFT |
                    ArcLayout::ARC_RIGHT) ? 1 : 2) * radius;
    ...
    for (int i = 0; i < items.count(); ++i) {
        ...
        if (i == 0 && arcLayoutFlags & ArcLayout::ARC_IRREGULAR_CENTER) {
            r = QRect ((rect.width() - centerWidth) / 2,
                       (rect.height() - centerHeight) / 2,
                       centerWidth, centerHeight);
        } else {
            r = QRect ((rect.width() - width) / 2,
        ...
        if (arcType != ArcLayout::ARC_FULL)
            r.translate (QPoint
                (qRound (radius / 2.0 * sin (minimumArc / 2.0 * ARC_PI)),
                 qRound (radius / 2.0 * -cos (minimumArc / 2.0 * ARC_PI))));
        if (i == 0) {
            // do nothing
        } else if (items.count() == 2) {
            r.translate (QPoint
               (qRound (radius * cos (((arcType == ArcLayout::ARC_FULL ? 1 :
                                0.5) + minimumArc / 2.0) * ARC_PI)),
                qRound (radius * sin (((arcType == ArcLayout::ARC_FULL ? 1 :
                                0.5) + minimumArc / 2.0) * ARC_PI))));
        } else {
            r.translate (QPoint (
                qRound (radius * cos (((i - 1) / (items.count() - (arcType ==
                    ArcLayout::ARC_FULL ? 1.0 : 2.0)) + minimumArc / 2.0) *
                        (arcType == ArcLayout::ARC_FULL ? 2 : 1) * ARC_PI)),
                qRound (radius * sin (((i - 1) / (items.count() - (arcType ==
                    ArcLayout::ARC_FULL ? 1.0 : 2.0)) + minimumArc / 2.0) *
                        (arcType == ArcLayout::ARC_FULL ? 2 : 1) * ARC_PI))));
        }
        item->setGeometry (r);
...
ArcLayout::ArcLayout (ArcType arcType, QWidget *parent) :
    QLayout (parent),
    d (new ArcLayoutPrivate (arcType))
{
}
...
QSize ArcLayout::minimumSize() const
{
    ...
    const double arc = d->items.count() < 3 ? ARC_PI : d->arcType == ARC_FULL ?
        2 * ARC_PI / (d->items.count() - 1) : ARC_PI / (d->items.count() - 2);

    QSize minimumSize;
    Q_FOREACH (QLayoutItem *item, d->items)
        minimumSize = minimumSize.expandedTo (item->minimumSize());
    if ((d->arcLayoutFlags & ArcLayout::ARC_RECTANGULAR) == 0)
        minimumSize = minimumSize.expandedTo (QSize (minimumSize.height(),
                minimumSize.width()));
    ...
    const unsigned minRadius = unsigned (ceil (qMax (minRadiusFromHeight,
                    minRadiusFromWidth)));

    return QSize
       (minRadius * (d->arcType & (ARC_LEFT | ARC_RIGHT) ? 1 : 2) +
            minimumSize.width(),
