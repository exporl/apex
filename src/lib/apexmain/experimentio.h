#ifndef _APEX_SRC_LIB_APEXMAIN_EXPERIMENTIO_H_
#define _APEX_SRC_LIB_APEXMAIN_EXPERIMENTIO_H_
...
struct TimerDescription {
    ...
    QObject *startObject;    /// The object that will signal the start of the
                             /// timer.
    const char *startSignal; /// The signal that will start the timer.
    QObject
        *endObject; /// The object on which a member will be called on timeout.
    const char
        *endMember; /// The member (signal/slot) that will be called on timeout.
};
...
class ExperimentIo : public QObject
{
    ...
    Q_PROPERTY(
        bool responseEnabled READ responseEnabled WRITE setResponseEnabled)
    Q_PROPERTY(bool allEnabled READ allEnabled WRITE setAllEnabled)
    Q_PROPERTY(bool paused READ isPaused WRITE setPaused)
    Q_PROPERTY(
        int stimulusTimeout READ stimulusTimeout WRITE setStimulusTimeout)
    Q_PROPERTY(
        int interTrialTimeout READ interTrialTimeout WRITE setInterTrialTimeout)
...
Q_SIGNALS:
    ...
    void stimulusPlayed();
    void
    responseGiven(const ScreenResult *); // FIXME this should not be a pointer
    void feedbackDone();
    void timeout();
