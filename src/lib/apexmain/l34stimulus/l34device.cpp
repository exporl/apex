...
#error OBSOLETE
    ...
    qCDebug(APEX_RS, "Initializing %s", qPrintable(qsdevice));
...
void L34Device::SetSequence  ( const DataBlockMatrix* ac_pSequence )
{
    ...
    if (nSeq==0 && nPar==0) {
        qCDebug(APEX_RS, "L34:  not playing, playmatrix empty");
        m_bReady=false;
        return;
    }
...
void L34Device::AddInput( const DataBlock& ac_DataBlock )
{
    ...
    qCDebug(APEX_RS, "** Time for creating sendbuffer: %s", qPrintable(QString::number(time.elapsed())));
...
#endif
    qCDebug(APEX_RS, "** Time in m_pDevice->send(): %u", time.elapsed());
...
void L34Device::PlayAll( void )
{
    ...
    if (! m_bReady) {
        qCDebug(APEX_RS, "L34Device: not ready, not starting play");
        return;
    }
    ...
    if(  m_bPlaying == false ) {
        ...
        qCDebug(APEX_RS, "Sending %s", qPrintable(startcommand));
        ...
        if ( m_iTriggerType== data::TRIGGER_IN ) {
            ...
            if (! status.contains(csStatusStreaming) && !waitUntil( csStatusStreaming, 5000 )) {
                ...
                qCDebug(APEX_RS, "Unexpected status of device %d after sending start command: %s", m_nDeviceNr, qPrintable (status));
            } else {
                m_bPlaying=true;
            }
        }
...
{
#ifdef DEBUGL34
    qCDebug(APEX_RS, "L34-" + QString::number(m_nDeviceNr) + ": Wait until " + STATE1.c_str() + " or " + STATE2.c_str() + " timeout " + QString::number(timeout) );
#endif
    Q_ASSERT(m_pDevice);
...
#ifdef DEBUGL34
    qCDebug(APEX_RS, QString("Device %1: ").arg(m_nDeviceNr) + data.c_str());
#endif
    ...
    while ( ! data.contains( STATE1 ) && ! data.contains( STATE2 )) {
        if ( data.contains( "error" ) ) {
            ...
            if ( data.contains( "parity" ) ) {
                qCDebug(APEX_RS, "FIXME: warn user that the trigger switch is in the wrong direction");
            }

            throw ApexStringException( QString("L34: error string received ") +data );
        }
        qCDebug(APEX_RS, "%s", qPrintable(data));
        ...
        //      qCDebug(APEX_RS, "Polling again...");

        m_pDevice->send( "<status/>" );
        //      qCDebug(APEX_RS, "Receiving");
        data = m_pDevice->receive( );
        //      qCDebug(APEX_RS, "End receive");
#ifdef DEBUGL34
        qCDebug(APEX_RS, QString("Device %1: ").arg(m_nDeviceNr) + data.c_str());
#endif
    }
...
bool L34Device::AllDone( )
{
    ...
    qCDebug(APEX_RS, "L34Device::AllDone( )");
    ...
    if (m_stimulusLength) {         // try to not poll while we're sure the stimulus hasn't finished (polling is extremely blocking)
        ...
        if (sleeptime>0) {
            qCDebug(APEX_RS, "Waiting %d ms for stimulation to end (total stimulus duration=%d ms)",(int) sleeptime, (int) m_stimulusLength/1000);
            appcore::IThread::sf_Sleep(sleeptime ); // 10ms safety margin
            qCDebug(APEX_RS, "End wait");
        } else {
    ...
    if (!wResult) {
        if (m_iTriggerType==data::TRIGGER_IN) {
            qCDebug(APEX_RS, "Timeout waiting for 'Stopped' status\nTrigger did probably not occur.");
    ...
    qCDebug(APEX_RS, "leaving L34Device::AllDone( )");
...
{
//    qCDebug(APEX_RS, "L34Device::SetMap, map size: %s", qPrintable(QString::number((unsigned)p_map->size())));
    Q_ASSERT(p_map);
    m_pMap = p_map;
}
...
void L34Device::SendPowerup(  )
{
    ...
    qCDebug(APEX_RS, "Number of powerup frames: %s", qPrintable(QString::number(m_nPowerupCount)));
...
#ifdef DEBUGL34
    qCDebug(APEX_RS, "** Time in SendPowerup: "+  QString::number(time.elapsed()));
    showStatus();
#endif
...
bool L34Device::SetParameter ( const QString& type, const int channel, const QVariant& value )
{
    ...
    qCDebug(APEX_RS, "L34Device::SetParameter %s=%s", qPrintable(type), qPrintable(value.toString()));
    if (type=="volume") {
        ...
        qCDebug(APEX_RS, "L34Device: set volume to %s", qPrintable(QString::number(m_volume)));
        if (m_volume<0)
