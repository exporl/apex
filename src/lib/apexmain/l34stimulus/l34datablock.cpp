...
{
    if (bIsXMLData) {
//        qCDebug(APEX_RS, "We don't map");
        return m_XMLData;
    }
    if (volume!=mappedForVolume) {
//        qCDebug(APEX_RS, "Volume differs, doing mapping");
        ConvertToMappedXMLData(pMap, volume);
    } else {
//        qCDebug(APEX_RS, "Volume the same, NOT mapping again");
    }
...
//     qCDebug(APEX_RS, "Entering ConvertToMappedXMLData");
//     qCDebug(APEX_RS, "File extension is %s", qPrintable(file_extension));
    ...
    Q_ASSERT(pMap);
//    qCDebug(APEX_RS, "Map size: %s", qPrintable(QString::number((unsigned)pMap->size())));
...
//                qCDebug(APEX_RS, "t=%d, c=%d, m=%f, cl=%d", t, c, prevStim.magnitude, cl);
            }
        ...
        qCDebug(APEX_RS, "Unknown mapping extension");
    }

    qCDebug(APEX_RS, "** Time in ConvertToMappedXMLData: %s",
...
{
//    qCDebug(APEX_RS, "Parsing file %s", qPrintable(pFilename));

    if (pFilename.toLower().endsWith(".xml")) {
//        qCDebug(APEX_RS, "Parsing XML stimulus file\n");

        QFile file( pFilename );
        if (!file.open( QIODevice::ReadOnly)) {
            qCDebug(APEX_RS, "Can't open file for reading");
            return false;
        }
        ...
        qCDebug(APEX_RS, "Parsing %s stimulus file: %s", qPrintable(file_extension),
        ...
        qCDebug(APEX_RS, "Loaded %i stimuli", m_Data.size());
