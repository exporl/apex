...
class PlayMatrixCreator
{
    public:
        ...
        /**
          ...
          * This method removes all duplicates, creating a new DataBlock with a _n postfix,
        ...
        /**
          ...
          * A matrix is valid when all datablocks on one row are for the same device.
          * @param a_pMatrix the matrix to check
          * @param a_DataBlocks the DataBlock map used by the experiment
          * @param a_sError string receiving an error if return value is false
          * @return true if check succeeds
          */
        static bool sf_bCheckMatrixValid(const PlayMatrix* a_pMatrix, const tDataBlockMap& a_DataBlocks, QString& a_sError);

        /**
          * Create a matrix containing only datablocks for the given device.
          * Nodes containing datablocks for other devices will be set to an empty string.
          * @param a_pMatrix the matrix to check
          * @param a_DataBlocks the DataBlock map used by the experiment
          * @param ac_sDevice the device to create a matrix for
          * @return new'd PlayMatrix
          */
        static PlayMatrix* sf_pCreateSubMatrix(const PlayMatrix* a_pMatrix, const tDataBlockMap& a_DataBlocks, const QString& ac_sDevice);

        /**
          * Shrink a matrix.
          * The MatrixStorage class doesn't use dynamic resizing,
          * so all methods creating a matrix start with a 16*16 sized one containing all empty strings.
          ...
          */
        static PlayMatrix* sf_pShrinkedMatrix(const PlayMatrix* ac_pSrc, const unsigned ac_nSim, const unsigned ac_nSeq);
        ...
        /**
          ...
          */
        static DataBlockMatrix* sf_pConvert(const PlayMatrix* ac_pSrc, const tDataBlockMap& a_DataBlocks);
