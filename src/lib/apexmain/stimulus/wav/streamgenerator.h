  ...
  namespace stimulus
  {
    ...
    protected:
        /**
          ...
          */
      StreamGenerator( const unsigned ac_nChan, const unsigned ac_nBufferSize, const unsigned long ac_lFs = 0L ) :
    ...
    public:
      /**
        * Constructor.
        */
      SineGenerator( const unsigned ac_nChan, const unsigned ac_nBufferSize, const unsigned long ac_lFs ) :
    ...
    public:
        /**
          * Constructor.
          * @param ac_pInput the stream to loop, can be 0 on construction and set later using setinputstream
          * @param ac_lFs the samplerate
          * @param ac_bRandom if true, starts at a random position ( 0 < ac_pInput.mf_lTotalSamples )
          */
        DataLoopGenerator( PositionableInputStream* const ac_pInput, const int nChannels, const unsigned bufferSize, const unsigned long ac_lFs, const bool ac_bRandom, double jump);
    ...
    public:
      /**
        * Constructor.
        */
      NoiseGenerator( const unsigned ac_nChan,
      ...
      const Stream& Read()
      {
        for( unsigned i = 0 ; i < mc_nChannels ; ++i )
        {
          ...
          {
            // [Tom] maak normaal verdeeld: (cfr Numerical Recipes in C, Press)
    ...
    public:
      /**
        * Constructor.
        */
      SinglePulseGenerator( const unsigned ac_nChan,
                            const unsigned ac_nBufferSize,
                            const unsigned long ac_nFs     ) :
          StreamGenerator( ac_nChan, ac_nBufferSize, ac_nFs ),
        m_Buf( ac_nChan, ac_nBufferSize, true ),
      ...
      {
        assert( ac_nToGenerate < mc_nBufferSize );  //this should be enough, it's called 'single pulse' after all
...
#endif //#ifndef STREAMGENERATOR_H
