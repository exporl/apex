...
namespace
{
  const unsigned sc_nMaxNumItems = 16;       //initial size of working buf
  const QString  cDBlTag = "datablock";      //tags we use
  const QString  cSeqTag = "sequential";
  const QString  cSimTag = "simultaneous";

  QString f_sAppendRow( const QString& ac_sID, const int nRow ) //use int, saves us a conversion
  {
...
PlayMatrix* PlayMatrixCreator::createMatrix(
                                            ...
                                            data::ConnectionsData& a_Connections )
{
  ...
  for( unsigned i = 0 ; i < cnRows ; ++i )
  {
    for( unsigned j = 0 ; j < cnCols ; ++j )
    {
      const QString sCur = (*a_pMatrix) ( i, j ); // No reference here, string gets changed!
      if (sCur.isEmpty())
              continue;
      if( i > 0 )
      {
        ...
        if( it != search.end() && (*it).second != i )
        {
          ...
          if( a_DataBlocks.value(sCopy) == 0 )   //check if no copy was made already for a previous stim
          {
            ...
            for( tConnections::size_type i = 0 ; i < curcs.size() ; ++i )
            {*/
            for ( data::ConnectionsData::const_iterator it=a_Connections.begin();
...
void PlayMatrixCreator::parseDatablocks(const QString& ac_sPrevTag,
                                       const data::StimulusDatablocksContainer &data,
...
PlayMatrix* PlayMatrixCreator::sf_pShrinkedMatrix( const PlayMatrix* ac_pSrc, const unsigned ac_nSim, const unsigned ac_nSeq )
{
...
bool PlayMatrixCreator::sf_bCheckMatrixValid( const PlayMatrix* a_pMatrix, const tDataBlockMap& a_DataBlocks, QString& a_sError )
{
  const unsigned cnRows = a_pMatrix->mf_nGetChannelCount();
  const unsigned cnCols = a_pMatrix->mf_nGetBufferSize();
  ...
  for( unsigned i = 0 ; i < cnRows ; ++i )
  {
    ...
    const QString sDevice =
            a_DataBlocks.value(
            a_pMatrix->operator() ( i, 0 ) )->GetDevice();
...
PlayMatrix* PlayMatrixCreator::sf_pCreateSubMatrix( const PlayMatrix* a_pMatrix,
        const tDataBlockMap& a_DataBlocks, const QString& ac_sDevice )
{
  const unsigned cnRows = a_pMatrix->mf_nGetChannelCount();
  const unsigned cnCols = a_pMatrix->mf_nGetBufferSize();
  ...
  /*for( unsigned i = 0 ; i < cnRows ; ++i )
  {
      if ( a_pMatrix->operator() ( i, 0 ).isEmpty())
          continue;
    if( ac_sDevice == a_DataBlocks.value( a_pMatrix->operator() ( i, 0 ) )->GetDevice() )
    {
      unsigned nCurCols = 0;
      for( unsigned j = 0 ; j < cnCols ; ++j )
      {
...
DataBlockMatrix* PlayMatrixCreator::sf_pConvert( const PlayMatrix* ac_pSrc, const tDataBlockMap& a_DataBlocks )
{
