#ifndef L34STIMULUS_h
#define L34STIMULUS_h
...
public:
            ...
            currentLevel ( pCurrentLevel )
    {
        ...
        if (!isQuantized(pPeriod))
            qDebug("Warning: period not quantized: %1.10f",pPeriod);
        if (!isQuantized(pPhaseWidth))
            qDebug("Warning: period not quantized: %1.4f",pPeriod);
    ...
    bool isQuantized(float n) const
    {
        ...
        if (std::abs(cycles/5 - n) > 1e-4) {            // 23 bits are used for the base
            //qDebug("Difference: %f",cycles/5-n);
            return false;
        } else
            return true;
    }
