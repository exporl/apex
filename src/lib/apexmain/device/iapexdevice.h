...
class IApexDevice
{
    protected:
         ...
         * @param a_pParameters ptr to the initial (aka default) parameters, must be valid, won't be deleted
         */
    ...
    public:
        ...
        /**
         * Restore all parameters to their default (as specified in the config) value.
         */
        virtual void RestoreParameters();

        /**
         * Reset the device to its initial state and set all internal parameters to built in
         * default values
        ...
        **************************************************************************** */
        enum mt_eInfoType
        {
            mc_eClipping,       //!< query clipping, uses tUnsignedBoolMapCIt* for the void* pointer
            mc_eOutOfRange,     //!< query if the last parameter that was set was in range, use QString* for the pointer
            mc_eCustom = 1000
        };
