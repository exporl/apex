...
/**
  ...
  */
class AxMouseFlash : public QAxWidget
{
public:
...
class FlashWidgetPrivate : public QObject
{
    Q_OBJECT
public:
...
FlashWidgetPrivate::FlashWidgetPrivate(FlashWidget *p)
    : p(p), shortcut(NULL), enabled(false)
{
}
...
FlashWidget::FlashWidget(QWidget *parent)
    : QWidget(parent), d(new FlashWidgetPrivate(this))
{
...
bool FlashWidget::isFinished() const
{
    ...
    /*qCDebug(APEX_RS) << "Framenum=" <<
       d->axwidget->property("FrameNum").toInt()
...
void FlashWidget::setPalette(const QPalette &p)
{
    ...
    d->axwidget->setProperty(
        "BGColor", p.color(QPalette::Active, QPalette::Window).name());
}
