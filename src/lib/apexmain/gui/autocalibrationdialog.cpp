...
AutoCalibrationDialogPrivate::AutoCalibrationDialogPrivate(
    AutoCalibrationDialog *pub)
    : p(pub), ignoreHardware(0), enabled(true), calibrating(false)
{
    ui.setupUi(pub);

    calibrateAll = ui.buttonBox->addButton(tr("Calibrate Selected"),
                                           QDialogButtonBox::ActionRole);
    stop = ui.buttonBox->addButton(tr("Stop"), QDialogButtonBox::ActionRole);
    advanced =
        ui.buttonBox->addButton(tr("Advanced.."), QDialogButtonBox::ActionRole);
    ...
    connect(calibrateAll, SIGNAL(clicked()), pub,
            SIGNAL(calibrateAllParameters()));
    connect(stop, SIGNAL(clicked()), pub, SIGNAL(stopAutoCalibration()));
    // Make sure the dialog doesn't hide when receiving the accepted/rejected
    // signal
    ...
    connect(calibrateManually, SIGNAL(clicked()), pub,
            SIGNAL(calibrateManually()));
    connect(advanced, SIGNAL(toggled(bool)), pub, SLOT(showAdvanced(bool)));
    connect(ui.typeCbox, SIGNAL(currentIndexChanged(QString)), pub,
            SLOT(slmTypeChanged(QString)));
    connect(ui.freqCbox, SIGNAL(currentIndexChanged(QString)), pub,
            SLOT(slmFreqWeighChanged(QString)));
    connect(ui.timeCbox, SIGNAL(currentIndexChanged(QString)), pub,
            SLOT(slmTimeWeighChanged(QString)));
    connect(ui.percTxt, SIGNAL(textChanged(QString)), pub,
            SLOT(slmPercChanged(QString)));
    connect(ui.timeTxt, SIGNAL(textChanged(QString)), pub,
            SLOT(slmTimeChanged(QString)));
    connect(ui.hardware, SIGNAL(currentIndexChanged(int)), this,
            SLOT(on_hardware_currentIndexChanged(int)));
    connect(ui.amplitudeCbox, SIGNAL(stateChanged(int)), pub,
            SLOT(enableUserDefinedCalibrationAmplitude(int)));
    ui.advanced->hide();
...
void AutoCalibrationDialogPrivate::updateWidgets()
{
    ...
    ui.amplitudeTxt->setEnabled(enabled && !calibrating &&
    ...
    ui.buttonBox->button(QDialogButtonBox::Ok)
        ->setEnabled(enabled && !calibrating);
}

void AutoCalibrationDialogPrivate::on_hardware_currentIndexChanged(int index)
{
    if (ignoreHardware)
        return;
...
void AutoCalibrationDialogPrivate::setParameters(
    const QList<QStringList> &values)
{
    ...
    for (int i = 0; i < values.size(); ++i) {
        QTreeWidgetItem *item =
            new QTreeWidgetItem((QTreeWidget *)0, values.at(i));
...
void AutoCalibrationDialog::updateParameter(const QString &parameter,
                                            const QString &value,
                                            const QString &difference)
{
    QList<QTreeWidgetItem *> items =
        d->ui.parameterTree->findItems(parameter, Qt::MatchFixedString);
    items.at(0)->setData(2, 0, QVariant(value));
    items.at(0)->setData(3, 0, QVariant(difference));
    ((QCheckBox *)d->ui.parameterTree->itemWidget(
         items.at(0), d->ui.parameterTree->columnCount() - 1))
...
void AutoCalibrationDialog::newMeasurement()
{
    for (int i = 0; i < d->ui.parameterTree->topLevelItemCount(); ++i) {
        if (d->ui.parameterTree->topLevelItem(i)->foreground(2).color() ==
            QColor(255, 255, 255)) {
            d->ui.parameterTree->topLevelItem(i)->setBackground(
                2, QBrush(QColor(255, 255, 255)));
            d->ui.parameterTree->topLevelItem(i)->setForeground(
                2, QBrush(QColor(0, 0, 0)));
            d->ui.parameterTree->topLevelItem(i)->setData(
                2, 0, QVariant("-")); // TODO
        }
    }
}
...
AutoCalibrationDialog::AutoCalibrationDialog(QWidget *parent)
    : QDialog(parent), d(new AutoCalibrationDialogPrivate(this))
{
...
void AutoCalibrationDialog::setCurrentParameterCalibrated(
...
QStringList AutoCalibrationDialog::getSelectedParametersToSave()
{
    ...
    for (int i = 0; i < d->ui.parameterTree->topLevelItemCount(); ++i) {
        if (((QCheckBox *)d->ui.parameterTree->itemWidget(
                 ...
                 d->ui.parameterTree->columnCount() - 1))
                ->checkState() == Qt::Checked) {
            checkedParameters
                << d->ui.parameterTree->topLevelItem(i)->data(0, 0).toString();
        }
    }
...
void AutoCalibrationDialog::enableUserDefinedCalibrationAmplitude(
    int checkState)
{
