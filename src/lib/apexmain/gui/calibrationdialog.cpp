      ...
      autoCalibrateEnabled(false)
{
    ...
    start = ui.buttonBox->addButton(tr("Start"), QDialogButtonBox::ActionRole);
    ...
    correct = buttons->addButton(tr("Correct Output Parameter"),
                                 QDialogButtonBox::ActionRole);
    autoCalibrateSingle =
        buttons->addButton(tr("Autocalibrate"), QDialogButtonBox::ActionRole);
    ...
    connect(apply, SIGNAL(clicked()), pub, SIGNAL(saveCalibration()));
    connect(correct, SIGNAL(clicked()), this, SLOT(on_correct_clicked()));
    connect(autoCalibrateSingle, SIGNAL(clicked()), this,
            SLOT(on_autoCalibrateSingle_clicked()));
    connect(start, SIGNAL(clicked()), pub, SIGNAL(startOutput()));
    connect(stop, SIGNAL(clicked()), pub, SIGNAL(stopOutput()));
    connect(advanced, SIGNAL(toggled(bool)), pub, SLOT(showAdvanced(bool)));
    connect(autoCalibrateAll, SIGNAL(clicked()), pub,
            SIGNAL(autoCalibrateAll()));
    connect(ui.outputSlider, SIGNAL(valueChanged(int)), this,
            SLOT(on_outputSlider_valueChanged(int)));
    connect(ui.output, SIGNAL(textChanged(QString)), this,
            SLOT(on_output_textChanged()));
    connect(ui.target, SIGNAL(textChanged(QString)), this,
            SLOT(on_target_textChanged()));
    connect(ui.measured, SIGNAL(textChanged(QString)), this,
            SLOT(on_measured_textChanged()));
    connect(ui.hardware, SIGNAL(currentIndexChanged(int)), this,
            SLOT(on_hardware_currentIndexChanged(int)));
    connect(ui.stimulus, SIGNAL(currentIndexChanged(QString)), this,
            SLOT(on_stimulus_currentIndexChanged(QString)));
    connect(ui.parameter,
            SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)),
            this, SLOT(on_parameter_currentIndexChanged(QListWidgetItem *)));
    connect(ui.loop, SIGNAL(stateChanged(int)), this,
            SLOT(on_loop_stateChanged(int)));
    connect(ui.muted, SIGNAL(itemSelectionChanged()), this,
            SLOT(on_muted_itemSelectionChanged()));
...
void CalibrationDialogPrivate::updateWidgets()
{
    ...
    apply->setEnabled(enabled && measured.isEmpty() &&
                      !targetAmplitude().isEmpty() &&
                      !outputParameter().isEmpty());
    ...
    ui.targetExperimentLabel->setText(
        tr("Target SPL during experiment (%1)").arg(unit));
...
void CalibrationDialogPrivate::on_outputSlider_valueChanged(int value)
{
    ...
    ++ignoreInput;
    setOutputParameter(
        QString::number(value * (maximum - minimum) / 1000 + minimum));
    --ignoreInput;
}
...
void CalibrationDialogPrivate::on_hardware_currentIndexChanged(int index)
{
    if (ignoreHardware)
        return;
...
CalibrationDialog::CalibrationDialog(QWidget *parent)
    : QDialog(parent), d(new CalibrationDialogPrivate(this))
{
...
void CalibrationDialog::setParameterName(const QString &value)
{
    const QList<QListWidgetItem *> items =
        d->ui.parameter->findItems(value, Qt::MatchExactly);
    if (items.isEmpty())
