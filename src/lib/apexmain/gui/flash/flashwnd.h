...
/***
  ** [ stijn 13/08/2008 ] - removed all unused arguments
  **                      - removed DX support
  **                      - removed LoadMovie from @c OnAfterShowingContent
  **/
...
#import "macromed\\flash\\flash6.ocx" named_guids
...
OLECONTAINER(BOOL)::Create(GUID clsid, DWORD dwExStyle, DWORD dwStyle, HWND hWndParent, HINSTANCE hInst)
{
	...
	wcs.hInstance = hInst;
	wcs.lpszClassName = "ApexOleControl";
	RegisterClassEx(&wcs);
	...
	m_hWnd = CreateWindowEx(dwExStyle,
		"ApexOleControl", "ApexOleControlWindow",
		dwStyle,
...
OLECONTAINER(void)::Draw(HDC hdcDraw, const RECT* /*rcDraw*/, BOOL /*bErase*/)
{
	...
	if (lpO && lpV)
	{
		...
		else //in 8/16/24 bit screen depth UpdateLayeredWindow produces wrong results - we use underlaying DC to paint to
		{
			...
			BitBlt(m_hdcBack, 0, 0, rTotal.right, rTotal.bottom, hdcParent, 0, 0, SRCCOPY);
	...
	UpdateLayeredWindow(hwnd, NULL, &p, &sz, m_hdcBack, &p2, 0, &bf, m_iBPP == 32 ? ULW_ALPHA : ULW_OPAQUE);
}


//IStorage

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::CreateStream(const WCHAR *, DWORD , DWORD , DWORD , IStream **)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::OpenStream(const WCHAR * , void *, DWORD , DWORD , IStream **)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::CreateStorage(const WCHAR *, DWORD , DWORD , DWORD , IStorage **)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::OpenStorage(const WCHAR * , IStorage * , DWORD , SNB , DWORD , IStorage **)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::CopyTo(DWORD , IID const *, SNB ,IStorage *)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::MoveElementTo(const OLECHAR *,IStorage * , const OLECHAR *, DWORD )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::Commit(DWORD )
{
	NOTIMPLEMENTED;
}
...
OLECONTAINER(HRESULT STDMETHODCALLTYPE)::EnumElements(DWORD , void * , DWORD , IEnumSTATSTG ** )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::DestroyElement(const OLECHAR *)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::RenameElement(const WCHAR *, const WCHAR *)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetElementTimes(const WCHAR *, FILETIME const *, FILETIME const *, FILETIME const *)
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetClass(REFCLSID )
{
	return(S_OK);
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetStateBits(DWORD , DWORD )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::Stat(STATSTG * , DWORD )
{
	NOTIMPLEMENTED;
}
...
OLECONTAINER(HRESULT STDMETHODCALLTYPE)::GetBorder(LPRECT )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::RequestBorderSpace(LPCBORDERWIDTHS )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetBorderSpace(LPCBORDERWIDTHS )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetActiveObject(IOleInPlaceActiveObject *, LPCOLESTR )
{
	return(S_OK);
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::InsertMenus(HMENU , LPOLEMENUGROUPWIDTHS )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetMenu(HMENU , HOLEMENU , HWND )
{
	return(S_OK);
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::RemoveMenus(HMENU )
{
	NOTIMPLEMENTED;
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::SetStatusText(LPCOLESTR )
{
	return(S_OK);
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::EnableModeless(BOOL )
{
	return(S_OK);
}

OLECONTAINER(HRESULT STDMETHODCALLTYPE)::TranslateAccelerator(LPMSG , WORD )
{
	NOTIMPLEMENTED;
}
...
//IDispatch
OLECONTAINER(HRESULT STDMETHODCALLTYPE)::GetTypeInfoCount(UINT __RPC_FAR *)
{
	NOTIMPLEMENTED;
}
...
class CFlashWnd :
  ...
  //, public ShockwaveFlashObjects::IServiceProvider [stijn] not found
{
public:
	long m_lVersion;
