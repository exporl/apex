...
IApexMixerCallback::IApexMixerCallback(const unsigned ac_nMaxNumInputs,
                                       const unsigned ac_nNumOutputs,
                                     ...
                                     const unsigned ac_nThisInChannel)
{
    ...
    if (nFree != -1) {
        ...
        mc_pMixer->mp_SetMatrixNode((unsigned)nFree, ac_nThisInChannel,
                                    mv_dGain);
...
void IApexMixerCallback::mp_RemoveInput(StreamCallback *const ac_pItem,
                                        const unsigned ac_nItemOutChannel,
                                        const unsigned ac_nThisInChannel)
{
    int nIndex = sf_nFindInArray(ac_pItem, m_Inputs);
    while (nIndex != -1) {
        //[ stijn 28/02/2009 ] when device parameters are not reset before
        // loading a stimulus,
        // all gains in the matrix stay where they are; this value is however
        // not necessarily the same as mv_dGain:
        // gain only gets set when using mp_SetGain( dGain ), not when using
        // mp_SetGain( dGain, nChannel ).
        // So the line mc_pMixer->mf_dGetMatrixNode( (unsigned) nIndex,
        // ac_nThisInChannel ) == mv_dGain might return false,
        // event though the gain at that position was set earlier. The purpose
        // of the line is only to figure out if it is in use,
        // so it's ok if we just check if mc_pMixer->mf_dGetMatrixNode(
        // (unsigned) nIndex, ac_nThisInChannel ) != sc_dMixerMin :
        // if that is the case, the position in the mixer is in use, else it's
        // not.
        // if( mc_pMixer->mf_dGetMatrixNode( (unsigned) nIndex,
        // ac_nThisInChannel ) == mv_dGain )
        if (mc_pMixer->mf_dGetMatrixNode((unsigned)nIndex, ac_nThisInChannel) !=
            sc_dMixerMin) {
                                    ...
                                    const unsigned ac_nChannel)
{
    ...
    for (unsigned i = 0; i < nIChan; ++i) {
        if (mc_pMixer->mf_dGetMatrixNode(i, ac_nChannel) != sc_dMixerMin) {
            ...
            // qCDebug(APEX_RS, "Not setting channel to %f dB, it is muted",
            // ac_dGainIndB);
        }
    }
#ifdef PRINTCONNECTIONS
...
{
    ConnectItem::mc_pResult =
        &mc_pMixer->mf_DoProcessing(*ConnectItem::mc_pResult);
}

/**********************************************************************************************************************/

ApexOutputCallback::ApexOutputCallback(OutputStream *const ac_pOutput,
                                       const unsigned ac_nMaxNumInputs,
                                       const bool ac_bDeleteOutput)
    : IApexMixerCallback(ac_nMaxNumInputs, ac_pOutput->mf_nGetNumChannels(),
                         ac_pOutput->mf_nGetBufferSize()),
...
ApexProcessorCallback::ApexProcessorCallback(IStreamProcessor *const ac_pProc,
                                             const unsigned ac_nMaxNumInputs)
    : IApexMixerCallback(ac_nMaxNumInputs, ac_pProc->mf_nGetNumInputChannels(),
                         ac_pProc->mf_nGetBufferSize()),
      mc_pProc(ac_pProc)
{
}
...
{
    ConnectItem::mc_pResult =
        &mc_pProc->mf_DoProcessing(*ConnectItem::mc_pResult);
}
