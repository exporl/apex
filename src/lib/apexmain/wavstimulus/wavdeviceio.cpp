...
public:
     ...
     */
    WavDeviceBufferThread(IEofCheck* a_pCheck, BufferedProcessing* a_pBuffer, unsigned blockSize ) :
                                                          ...
                                                          const bool ac_bTryDefaultBufferSize, QString& a_sErr )
{
    ...
    try {
        //create driver
        qCDebug(APEX_RS, "Card name: %s, Driver name: %s", qPrintable(card), qPrintable(useDriver) );
        ...
        qCDebug(APEX_RS, "num of sample rates=%u", unsigned(info.m_SampleRates.size()));
        ...
        if (ac_Config.bufferSize()!=-1 && !info.mf_bCanBufferSize(ac_Config.bufferSize()))
            qCDebug(APEX_RS, "Warning: requested soundcard buffer size (%d) not suppored, to the default: %d",
...
void WavDeviceIO::mp_InitIO()
{
    ...
    m_pConnMan->mp_RegisterItem(mc_Config.id().toLatin1().data(), m_pMainOutput);
    ...
    m_pBufferThread = new WavDeviceBufferThread(m_pEofCheck, m_pBuffer, m_blockSize);
...
void WavDeviceIO::mp_AddConnectItem(PositionableInputStream* a_pItem, const QString& ac_sID, const bool ac_bAddToEofCheck)
{
...
void WavDeviceIO::mp_AddConnection( const tConnection& ac_Connection, const bool /*ac_bMixToFile */)
{
    if (ac_Connection.m_nFromChannel >= 0 && ac_Connection.m_nToChannel >= 0) {
        m_pConnMan->mp_Connect( ac_Connection.m_sFromID.toLatin1().data(), ac_Connection.m_sToID.toLatin1().data(),
...
{
    qCDebug(APEX_RS, "Wavdevice set gain of channel %d to %f", ac_nChannel, ac_dGain);
...
void WavDeviceIO::addPortaudioDevicesToGui(SoundcardsDialog *soundcardsDialog, unsigned sampleRate)
{
    ...
    for (PaHostApiIndex hostApiIndex = 0 ; hostApiIndex < numberOfHostApis ; hostApiIndex++) {
        QString hostApiName(QString::fromLocal8Bit(Pa_GetHostApiInfo(hostApiIndex)->name));
        numberOfDevices = Pa_GetHostApiInfo(hostApiIndex)->deviceCount;
        for (int hostApiDeviceIndex = 0 ; hostApiDeviceIndex < numberOfDevices ; hostApiDeviceIndex++) {
            deviceIndex = Pa_HostApiDeviceIndexToDeviceIndex(hostApiIndex,hostApiDeviceIndex);
            ...
            // Check if any of the most frequently used sample rates are supported, if it is, the device is suitable
            // or if sampleRate is not 0, check if that sampleRate is supported
            if (sampleRate != 0 && deviceIndex != paNoDevice && deviceInfo && deviceInfo->maxOutputChannels > 0 && Pa_IsFormatSupported(0, &out, sampleRate) == paFormatIsSupported) {
                soundcardsDialog->registerDevice(hostApiName, deviceIndex, deviceInfo->name, deviceInfo->maxOutputChannels);
            } else if (deviceIndex != paNoDevice && deviceInfo && deviceInfo->maxOutputChannels > 0
                       && (Pa_IsFormatSupported(0, &out, 32000) == paFormatIsSupported
                           || Pa_IsFormatSupported(0, &out, 44100) == paFormatIsSupported
                           || Pa_IsFormatSupported(0, &out, 48000) == paFormatIsSupported
                           || Pa_IsFormatSupported(0, &out, 96000) == paFormatIsSupported)) {
                soundcardsDialog->registerDevice(hostApiName, deviceIndex, deviceInfo->name, deviceInfo->maxOutputChannels);
            }
        }
    }
...
int WavDeviceIO::deviceNameToPortaudioId(const QString &hostName, const QString &devName)
{
    ...
    for (hostNum = 0; hostNum < hostCnt; hostNum++) {
        ...
        if (hinfo && hinfo->name == hostName) {
            for (PaDeviceIndex hostDevice = 0; hostDevice < hinfo->deviceCount; hostDevice++) {
                ...
                if (dinfo && dinfo->name == devName && dinfo->maxOutputChannels > 0 ) {
...
{
    m_pConnMan->mp_UnregisterItem(mc_Config.id());
    ZeroizingDeleter()(m_pCard);
