...
void sf_ShowCardsForDriver(const gt_eDeviceType ac_tType)
{
    ...
    if (nCards) {
        std::cout << fSndEnumToString<std::string>(ac_tType)
                  << " cards: " << std::endl;
...
void ShowSoundCardsPortAudio()
{
    if (!Pa_Initialize()) {
        ...
        for (PaDeviceIndex deviceIndex = 0; deviceIndex < numberOfDevices;
             deviceIndex++) {
            ...
            //                if ( deviceIndex ==
            //                streamapp::PortAudioInitter::sf_pInstance()->mf_nGetInputDeviceID("default")
            //                )
            //                    std::cout << " (default input device!)";
            //                if ( deviceIndex ==
            //                streamapp::PortAudioInitter::sf_pInstance()->mf_nGetOutputDeviceID("default")
            //                )
            //                    std::cout << " (default output device!)";
            std::cout << std::endl;
            std::cout << "\t DeviceID=" << deviceIndex
                      << "\t Hostapi: " + hostApiName.toStdString()
                      << std::endl;

            std::cout << "Default portaudio device: "
                      << Pa_GetDefaultOutputDevice() << std::endl;

            // TODO: remove (copied from portaudiowrapper.cpp
            PaHostApiTypeId preferredHostApiTypeIds[] = {
                paWASAPI, paWDMKS, paDirectSound,  paMME,
                paASIO,   paALSA,  paInDevelopment};
            ...
            for (PaHostApiIndex preferredHostApiIndex = 0;
                ...
                for (PaHostApiIndex installedHostApiIndex = 0;
                    ...
                    if (preferredHostApiTypeIds[preferredHostApiIndex] ==
                        ...
                        preferred = Pa_GetHostApiInfo(installedHostApiIndex)
                                        ->defaultInputDevice;
                }
            }

            std::cout << "Default device for preferred HostAPI: " << preferred
                      << std::endl;
            // END
        }
        std::cout << "--------------------------" << std::endl;
        ...
        for (PaHostApiIndex preferredHostApiIndex = 0;
             ...
             preferredHostApiIndex++) {
            PaHostApiIndex hostApiIndex = Pa_HostApiTypeIdToHostApiIndex(
                preferredHostApiTypeIds[preferredHostApiIndex]);
            if (hostApiIndex < 0) {
                continue;
            }
            const PaHostApiInfo *hinfo = Pa_GetHostApiInfo(hostApiIndex);
            std::cout << "HOSTAPI " << hostApiIndex << " " << hinfo->name
                      << "'s devices: " << std::endl;
            for (PaDeviceIndex hostDevice = 0; hostDevice < hinfo->deviceCount;
                 hostDevice++) {
                PaDeviceIndex deviceNum = Pa_HostApiDeviceIndexToDeviceIndex(
                    hostApiIndex, hostDevice);
                const PaDeviceInfo *dinfo = Pa_GetDeviceInfo(deviceNum);
                if (dinfo) {
                    std::cout << "device " << deviceNum << " " << dinfo->name
                              << " belonging  to " << hinfo->name
                              << " with default sample rate "
                              << dinfo->defaultSampleRate << " outputchannels "
                              << dinfo->maxOutputChannels << std::endl;
                }
                ...
                const unsigned long RatesToTry[] = {8000,  12000, 16000,
                                                    24000, 32000, 44100,
                                                    48000, 96000, 192000};
                ...
                if (std::find(a_SampleRates.begin(), a_SampleRates.end(),
                              dinfo->defaultSampleRate) !=
                    a_SampleRates.end()) {
                    std::cout
                        << "!!! PaDeviceInfo's default sample rate is not "
                           "reported as supported by Pa_IsFormatSupported"
                        << std::endl;
                    std::cout << "Supported formats: ";
                    for (std::vector<unsigned long>::const_iterator i =
                             a_SampleRates.begin();
                         i != a_SampleRates.end(); ++i) {
                        std::cout << *i << "  ";
                    }
                    std::cout << std::endl;
                }
            ...
            std::cout << std::endl
                      << "host api default output device " << devindex << " "
                      << Pa_GetDeviceInfo(devindex)->name << std::endl;
        }
        std::cout << "------------" << std::endl;
        std::cout << "default output device over entire system "
                  << Pa_GetDefaultOutputDevice() << " with name "
                  << Pa_GetDeviceInfo(Pa_GetDefaultOutputDevice())->name
                  << std::endl;
        std::cout << "default host api " << Pa_GetDefaultHostApi()
                  << " with name "
