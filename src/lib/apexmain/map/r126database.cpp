...
namespace r126{
  ...
  {    //SCREEN
    ...
    {
     /* gui::SimpleDialogBox* p = new gui::SimpleDialogBox( QMessageBox::Critical,
        "Error", ac_sError );
      p->exec(); */
    }
    ...
    {
      /*gui::SimpleDialogBox* p = new gui::SimpleDialogBox( QMessageBox::Information,
        "", ac_sMessage );
      p->exec(); */
    }
  }
  ...
  R126DataBase::R126DataBase() :
    m_pConnectionSettings( new R126DataBaseSettings() )
  {
  ...
  void R126DataBase::mf_SetDataBaseSettings( const eR126VersionNumber ac_eVersion, const QString& ac_sServer )
  {
    ...
    if( !sm_bPatientsRecorded )
    {
      ...
      try 
      {
        ...
        connection->Open(sConn,L"",L"",ADODB::adConnectUnspecified);
          ...
          //Old SELECT PatientGUID, Surname, Firstname, Gender, DateOfBirth from Patient order by Patient.Surname",
        PatientRecordSet->Open("SELECT Patient.PatientGUID, Patient.Surname, Patient.Firstname, Patient.Gender, Patient.DateOfBirth, Implant.DateOfImplantation, Implant.ImplantType from Patient, Implant where Implant.PatientGUID = Patient.PatientGUID order by Patient.Surname",
        ...
        while( !PatientRecordSet->ADOEOF )
        {
          ...
          var = PatientRecordSet->Fields->GetItem(L"PatientGUID")->GetValue();
          pRec->m_sGUID = QString( static_cast<char*>(_bstr_t(var.bstrVal)) );

          var = PatientRecordSet->Fields->GetItem(L"DateOfBirth")->GetValue();
          ...
          var = PatientRecordSet->Fields->GetItem(L"DateOfImplantation")->GetValue();
          datetemp = var.date;
          pRec->m_sImplantDate = QString( datetemp.Format().GetBuffer() );

          var = PatientRecordSet->Fields->GetItem(L"ImplantType")->GetValue();
  ...
  std::vector<R126NucleusMAPRecord*>& R126DataBase::mf_FillNucleusMapsInfo( const QString& ac_sPatientGUID )
  {
    ...
    try 
    {
      ...
      QString sqlstring = "SELECT * from MAP where PatientGUID = '" + ac_sPatientGUID + "' order by CreationDateTime";

      MAPRecordSet->Open((_variant_t) sqlstring,
        connection.GetInterfacePtr(),
        ADODB::adOpenForwardOnly,ADODB::adLockReadOnly,
        ADODB::adCmdText);
      ...
      while(!MAPRecordSet->ADOEOF)
      {
        ...
        var = MAPRecordSet->Fields->GetItem(L"CreationDateTime")->GetValue();
        COleDateTime datetemp(var.date);
  ...
  R126NucleusMAPRecord* R126DataBase::mf_pGetNucleusMap( const QString& ac_sMapGUID )
  {
    R126NucleusMAPRecord* pRec = new R126NucleusMAPRecord();
    ...
    try 
    {
      ...
      QString sqlstring = "SELECT MAP.MAPNumber, MAP.CreationDateTime, MAP.MAPGUID"
        ...
        "from MAP, Patient "
        "where Patient.PatientGUID=MAP.PatientGUID and MAP.MAPGUID='" + ac_sMapGUID + "'";
      ...
      sqlstring = "SELECT * from PhysicalChannel where PhysicalChannel.MAPGUID='"
        + ac_sMapGUID + "' order by ChannelNumber DESC";
      // !!! R126 orders channels from base to apex (here we turn around this ordering !!)

      ChannelRecordSet->Open((_variant_t) sqlstring,
        connection.GetInterfacePtr(),
        ADODB::adOpenForwardOnly,ADODB::adLockReadOnly,
        ADODB::adCmdText);
      ...
      while( !ChannelRecordSet->ADOEOF )
      {
        ...
        var = ChannelRecordSet->Fields->GetItem(L"StimulationMode")->GetValue();
        if( i == 0 )
          pRec->m_sStimulationMode = QString( static_cast<char*>(_bstr_t(var.bstrVal)) );
        else if( pRec->m_sStimulationMode != static_cast<char*>(_bstr_t(var.bstrVal)) )
          throw( 104 );

        var = ChannelRecordSet->Fields->GetItem(L"ChannelEnabled")->GetValue();
