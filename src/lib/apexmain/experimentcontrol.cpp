...
class ExperimentControlPrivate : public QObject
{
    ...
    public Q_SLOTS:
        ...
        void debug() {qCDebug(APEX_RS) << "debug slot called";}
                                      ...
                                      stopped(false)
{
    ...
    connect(this, SIGNAL(errorMessage(QString, QString)), qq, SIGNAL(errorMessage(QString, QString)));
...
void ExperimentControlPrivate::setupStates()
{
    ...
    QHistoryState* resume = new QHistoryState(QHistoryState::DeepHistory, running);
    ...
    connect(waitForStart, SIGNAL(entered()), io, SLOT(disableSelectSoundcard()));
...
void ExperimentControlPrivate::setupRunningState(QState* running)
{
    ...
    runningTrial->addTransition(runningTrial, SIGNAL(finished()), checkingPause);
    ...
    interTrial->addTransition(this, SIGNAL(interTrialScreenDone()), runningTrial);
...
void ExperimentControlPrivate::setupRunningTrialState(QState* runningTrial)
{
    ...
    runningTrial->addTransition(io, SIGNAL(repeatTrialRequest()), restartingTrial);

    // [Tom] changed, because when stop is clicked, no results should be collected for the last trial
    ...
    runningScreen->addTransition(runningScreen, SIGNAL(finished()), showingScreen);
...
void ExperimentControlPrivate::setupResponseState(QState* response)
{
    ...
    connect(waitingForResponse, SIGNAL(entered()),
            this, SLOT(startResponseTimer()));
    connect(this, SIGNAL(responseGiven()),
            this, SLOT(stopResponseTimer()));

    checkingResponseNeeded->addTransition(this, SIGNAL(needResponse()), waitingForResponse);
    checkingResponseNeeded->addTransition(this, SIGNAL(dontNeedResponse()), responseDone);
    connect(checkingResponseNeeded, SIGNAL(entered()),
            this, SLOT(checkResponseNeededDuringStimuli()));

    // [Tom] Removed property setting: property is unset when leaving state, but this leads
    ...
    connect(waitingForResponse, SIGNAL(entered()),
            io, SLOT(enableResponseStopPause()));

    waitingForResponse->addTransition(io, SIGNAL(pauseRequest()), postponingPause);
    waitingForResponse->addTransition(this, SIGNAL(responseGiven()), showingFeedback);
...
void ExperimentControlPrivate::setupStimuliState (QState* stimuli)
{
    ...
    //the state after all stimuli have been played. check whether response is needed
    ...
    playingStimuli->addTransition(playingStimuli, SIGNAL(finished()), afterStimuli);
    setupPlayingStimuliState(playingStimuli);

    connect(afterStimuli, SIGNAL(entered()), this, SLOT(checkResponseNeededAfterStimuli()));
}
...
void ExperimentControlPrivate::setupPlayingStimuliState(QState* playingStimuli)
{
    ...
    playingStimulus->addTransition(io, SIGNAL(stimulusPlayed()), waitAfterStimulus);
    ...
    waitAfterStimulus->addTransition(this, SIGNAL(stimulusTimeoutDone()), playingStimulus);
    connect(waitAfterStimulus, SIGNAL(entered()), this, SLOT(waitAfterStimulus()));
}

void ExperimentControlPrivate::loadExperiment(ExperimentRunDelegate* runDelegate)
{
    rd = runDelegate;
    QString error;
    ...
    else
    {
        ...
        crashed->addTransition(this->api,
                               SIGNAL(stoppedWithError(QString,QString)),
                               runningDone);
    }
}
...
{
    //this happens after the last trial: we enter the trial state for a last time
    //and, although the trialsDone signal has been emitted, we still seem to enter
...
{
    Q_ASSERT(currentTrial.isValid() && currentScreen < currentTrial.screenCount());
    ...
    {
        QString stimulus = currentTrial.stimulus(currentScreen, currentStimulus);
        ...
        if (rd->GetData().screensData()->hasShowCurrentEnabled())
        {
            ...
            io->showFeedback(rundelegates::ScreenElementRunDelegate::HighlightFeedback,
...
void ExperimentControlPrivate::showFeedback()
{
    ...
    if ((lastResponse.overrideCorrectFalse==false && rd->GetData().screensData()->hasFeedbackEnabled())
...
void ExperimentControlPrivate::animateTimeout(int msec, const char* property, const char* signal)
{
...
void ExperimentControlPrivate::processResponse(const apex::ScreenResult* result)
{
    ...
    for(stimulusParamsT::const_iterator it = result->GetStimulusParameters().begin(); it != result->GetStimulusParameters().end(); ++it) {
...
void ExperimentControlPrivate::showMessageBox(const QString title, const QString text) const {
