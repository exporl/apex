...
void ProceduresWriter::fillParametersElement(const ProcedureData &data,
                                             QDomElement *theElement)
{
    ...
    if (n > 0) {
        theElement->appendChild(
            XmlUtils::createTextElement(&doc, "presentations", n));
    }

    //<skip>
    theElement->appendChild(
        XmlUtils::createTextElement(&doc, "skip", data.skip()));

    //<order>
    QString order = getOrderString(data.order());
    theElement->appendChild(XmlUtils::createTextElement(&doc, "order", order));

    //<defaultstandard>
    QString ds = data.defaultStandard();
    if (!ds.isEmpty()) {
        theElement->appendChild(
            XmlUtils::createTextElement(&doc, "defaultstandard", ds));
    }

    //<uniquestandard>
    theElement->appendChild(XmlUtils::createTextElement(&doc, "uniquestandard",
                                                        data.uniqueStandard()));
    ...
    if (n > 0)
        theElement->appendChild(
            XmlUtils::createTextElement(&doc, "pause_between_stimuli", n));

    //<time_before_first_trial>
    theElement->appendChild(XmlUtils::createTextElement(
        &doc, "time_before_first_trial", data.timeBeforeFirstStimulus()));

    //<input_during_stimulus>
    theElement->appendChild(XmlUtils::createTextElement(
        &doc, "input_during_stimulus", data.inputDuringStimulus()));

    // add type specific parameters
    switch (data.type()) {
    case ProcedureData::AdaptiveType:

        finishAsAdaptive(*dynamic_cast<const AdaptiveProcedureData *>(&data),
                         theElement);
        break;
    ...
    case ProcedureData::PluginType:
        ...
        finishAsPlugin(*dynamic_cast<const ScriptProcedureData *>(&data),
                       theElement);
        break;
...
void ProceduresWriter::fillCorrector(const data::CorrectorData &correctorData,
                                        ...
                                        QDomElement *toFinish)
{
    ...
    // nDown
    toFinish->appendChild(
        XmlUtils::createTextElement(&doc, "nDown", data.nDown()));
    // adapt_parameter
    Q_FOREACH (QString adap, data.adaptingParameters()) {
        toFinish->appendChild(
            XmlUtils::createTextElement(&doc, "adapt_parameter", adap));
    }

    // start_value
    toFinish->appendChild(
        XmlUtils::createTextElement(&doc, "start_value", data.startValue()));
    // stop_after_type
    toFinish->appendChild(XmlUtils::createTextElement(
        &doc, "stop_after_type", data.stopAfterTypeString()));
    // stop_after
    toFinish->appendChild(
        XmlUtils::createTextElement(&doc, "stop_after", data.stopAfter()));
    // min_value
    if (data.hasMinValue()) {
        toFinish->appendChild(
            XmlUtils::createTextElement(&doc, "min_value", data.minValue()));
    }
    // max_value
    if (data.hasMaxValue()) {
        toFinish->appendChild(
            XmlUtils::createTextElement(&doc, "max_value", data.maxValue()));
    }

    // TODO rev_for_mean
    // larger_is_easier
    toFinish->appendChild(XmlUtils::createTextElement(
        &doc, "larger_is_easier",
        ApexTools::boolToString(data.largerIsEasier())));
    // repeat_first_until_correct
    if (data.repeatFirstUntilCorrect()) {
        toFinish->appendChild(XmlUtils::createTextElement(
            &doc, "repeat_first_until_correct", "true"));
    }

    // stepsizes
    QDomElement stepsizes = doc.createElement(QSL("stepsizes"));
    stepsizes.appendChild(XmlUtils::createTextElement(
        &doc, "change_after", data.changeStepsizeAfterString()));
    ...
    for (it = upStepsizesMap.begin(); it != upStepsizesMap.end(); it++) {
        if (downStepsizesMap.contains(it.key())) {
            ...
            auto stepsize =
                createStepsizeElement(&doc, it.key(), it.value(), "up");
            stepsizes.appendChild(stepsize);
        }
    }
    ...
    for (it = downStepsizesMap.begin(); it != downStepsizesMap.end(); it++) {
        if (!upStepsizesMap.contains(it.key())) {
            auto stepsize =
                createStepsizeElement(&doc, it.key(), it.value(), "down");
            stepsizes.appendChild(stepsize);
        }
    }
...
QDomElement ProceduresWriter::createStepsizeElement(QDomDocument *doc,
                                                    int begin, float size,
                                                    QString direction)
{
                                      ...
                                      QDomElement *toFinish)
{
    ...
    // script
    toFinish->appendChild(
        XmlUtils::createTextElement(&doc, "script", data.script()));
    // adjust_parameter
    QString adj = data.adjustParameter();
    if (!adj.isEmpty()) {
        toFinish->appendChild(
            XmlUtils::createTextElement(&doc, "adjust_parameter", adj));
    }
    ...
    for (it = paramList.begin(); it != paramList.end(); it++) {
        QDomElement parameter =
            XmlUtils::createTextElement(&doc, "parameter", it->value);
        parameter.setAttribute(QSL("name"), it->name);
        toFinish->appendChild(parameter);
    }
}

void ProceduresWriter::fillTrialsElement(const data::ProcedureData &data,
                                         QDomElement *theElement)
{
    QDomDocument doc = theElement->ownerDocument();
    // get the trials
    const data::tTrialList &trials = data.GetTrials();
    ...
    for (it = trials.begin(); it != trials.end(); it++) {
        ...
        if (!answer.isEmpty())
            trialElem.appendChild(
                XmlUtils::createTextElement(&doc, "answer", answer));

        //<answer_element>
        answer = trial->GetAnswerElement();
        if (!answer.isEmpty()) {
            trialElem.appendChild(
                XmlUtils::createTextElement(&doc, "answer_element", answer));
        }
    ...
    if (data.hasPluginTrials())
        theElement->appendChild(doc.createElement(QSL("plugintrials")));
}
