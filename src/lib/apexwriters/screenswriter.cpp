...
public:
    ...
    QDomElement getResult() { return result; }
...
void ElementToXMLVisitor::addShortcutChild(QDomElement de, const ScreenElement* e)
{
    Q_UNUSED(de);
    Q_UNUSED(e);
...
void ElementToXMLVisitor::addLayoutChildren(QDomElement de, const ScreenLayoutElement* sle)
{
...
void ElementToXMLVisitor::addFontSizeChild(QDomElement de, const ScreenElement* se)
{
    if (se->getFontSize() != -1)
    {
...
void ElementToXMLVisitor::addFeedbackChild(QDomElement de, bool overrideFeedback,
        const QString& highlight,
        const QString& positive,
        const QString& negative)
{
    if (overrideFeedback)
    {
        ...
        {
            QDomElement highlightElem = domDocument->createElement(QSL("highlight"));
            highlightElem.appendChild(
                    domDocument->createTextNode(highlight));
            feedbackElem.appendChild(highlightElem);
        }
        if (!positive.isEmpty())
        {
            QDomElement positiveElem = domDocument->createElement(QSL("positive"));
            positiveElem.appendChild(
                    domDocument->createTextNode(positive));
            feedbackElem.appendChild(positiveElem);
        }
        if (!negative.isEmpty())
        {
            QDomElement negativeElem = domDocument->createElement(QSL("negative"));
            negativeElem.appendChild(
                    domDocument->createTextNode(negative));
...
void ElementToXMLVisitor::visitParameterList(const ParameterListElement* e)
{
            ...
            i != parlist.end(); ++i)
    {
        ...
        parElem.appendChild(
                domDocument->createTextNode(pardata.id));
...
void ElementToXMLVisitor::visitPicture(const PictureElement* e)
{
    ...
    pathElem.appendChild(
            domDocument->createTextNode(e->getDefault()));
    result.appendChild(pathElem);
    addFeedbackChild(result, e->getOverrideFeedback(),
            e->getHighlight(), e->getPositive(), e->getNegative());
}
...
void ElementToXMLVisitor::visitPictureLabel(const PictureLabelElement* e)
{
    ...
    QDomElement pathElem = domDocument->createElement(QSL("file"));
    pathElem.appendChild(
            domDocument->createTextNode(e->getPicture()));
    result.appendChild(pathElem);
}
...
void ElementToXMLVisitor::visitTextEdit(const TextEditElement* e)
{
    ...
    textElem.appendChild(
            domDocument->createTextNode(e->getText()));
    ...
    if (!inputMask.isEmpty())
    {
        ...
        imElem.appendChild(
                domDocument->createTextNode(inputMask));
...
void ElementToXMLVisitor::visitParameterLabel(const ParameterLabelElement* e)
{
    ...
    QDomElement param = XmlUtils::createTextElement(domDocument,
            "parameter",
            paramData.id);//argh...
        ...
        const QStringList& screenList)
{
    ...
    if (screenList.isEmpty()) {
        ...
        Q_FOREACH (const QString &screen, screenList) {
            screens.appendChild(doc->importNode(XmlUtils::parseString(screen).documentElement(), true));
        }
    }
...
QDomElement ScreensWriter::addScreen(QDomDocument *doc, const data::Screen& data)
{
    ...
    if (!defaultAnswer.isEmpty()) {
        QDomElement defaultAnswerEl = doc->
            createElement(QSL("default_answer_element"));
        defaultAnswerEl.appendChild(
                doc->createTextNode(defaultAnswer));
...
QDomElement ScreensWriter::buttonGroupToXml(QDomDocument *doc, const ButtonGroup &g)
{
        ...
        const data::ScreensData &data)
{
    ...
    //feedback
    QDomElement feedback = XmlUtils::createTextElement(doc, "feedback",
        ...
        const data::ScreensData &data)
{
    ...
    if (data.hasInterTrialScreen()) {
        QDomElement its = XmlUtils::createTextElement(doc, "intertrialscreen",
                "true");
        its.setAttribute(QSL("length"), QString::number(data.interTrialLength()));
        ...
        const data::ScreensData &data)
{
    ...
    if (data.hasIntroScreen()) {
        QDomElement intro = XmlUtils::createTextElement(doc, "intro",
                data.introScreen());
    ...
    if (data.hasOutroScreen()) {
        QDomElement outro = XmlUtils::createTextElement(doc, "outro",
                data.outroScreen());
