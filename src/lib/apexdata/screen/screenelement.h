...
class APEXDATA_EXPORT ScreenElement
{
    ...
    public:
        ...
        void setBGColor(const QString& p) { bgcolor = p; };
        const QString& getBGColor() const { return bgcolor;} ;

        void setStyle(const QString s) { style = s;};
        QString getStyle() const { return style; };

        void setDisabled(bool p) { disabled = p; };
        bool getDisabled() const { return disabled; };
        void setFont(QString f) { font = f; };
        QString getFont() const { return font; };
