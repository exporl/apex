...
QString ScreenElement::getShortCut(const QString &action) const
{
    if (!shortcuts.contains(action))
        //    throw ApexStringException(tr("Could not find shortcut for action
        //    %1")
        //    .arg(action));
        return QString();
...
void ScreenElement::setShortCut(const QString &key, const QString action)
{
...
bool ScreenElement::operator==(const ScreenElement &other) const
{
    return elementType() == other.elementType() &&
           ...
           fontSize == other.fontSize && fgcolor == other.fgcolor &&
           bgcolor == other.bgcolor && style == other.style &&
           disabled == other.disabled && filePrefix == other.filePrefix &&
