...
@author Tom Francart,,,
*/
class APEXDATA_EXPORT TrialData : public QObject
{ // must be QObject for compatibility with QtScript
    Q_OBJECT
...
public slots: // have to be slots to be used in QtScript
    ...
    void SetAnswerElement(const QString &p_element)
    {
        m_answer_element = p_element;
    };
    ...
    const tStimulusList &GetStandards() const
    {
        return m_standards;
    };
    const tStimulusList::const_iterator GetStimulusIterator() const;
    const tStimulusList &GetStimuli() const
    {
        return m_stimuli;
    };
...
private:
    QString m_answer;
    QString
        m_answer_element; // screen element containing the answer to this trial
    QString m_screen;
