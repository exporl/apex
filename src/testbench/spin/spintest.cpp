...
#include "procedure/apexadaptiveprocedureparameters.cpp"
...
double totalNoiseGain(ExperimentData& d)
{
         ...
         it!=filters->end(); ++it)
    {
        ...
        if (id.startsWith(FILTER_TYPE_NOISE)) {
            qDebug() << "noise filter id" << id;
            qDebug() << "    gain:    " << it.value()->valueByType("gain").toDouble();
            qDebug() << "    basegain:" << it.value()->valueByType("basegain").toDouble();
            result+=it.value()->valueByType("gain").toDouble();
            result+=it.value()->valueByType("basegain").toDouble();
        } //else  qDebug("Not a noise filter: %s", qPrintable(id));
    }
    return result;
}
 ...
 void SpinTest::testTotalGain()
 {
     ...
     qDebug() << "Available speech materials: "
         <<configuration.speechmaterials().keys();
     QString targetSpeechmaterial( "LIST" );
     qDebug() << "Using speech material " << targetSpeechmaterial;
...
//      }
     ...
     qDebug() << "Available noise materials:" << configuration.noises().keys();
 ...
 //    qDebug() << expfile;
     ...
     std::auto_ptr<apex::data::ExperimentData> experiment(
     ...
     qDebug("speechgain=%f, noisegain=%f", speechgain, noisegain);
     qDebug("levels.speech=%f, levels.noise=%f", levels.speech, levels.noise);
...
void SpinTest::testAdaptiveWithoutNoise()
{
    ...
    apex::ExperimentParser expParser(expfile);
    std::auto_ptr<apex::data::ExperimentData> experiment(expParser.parse(false));
    ...
    QCOMPARE(static_cast<ApexAdaptiveProcedureParameters*>
            (experiment->procedureConfig()->GetParameters())->startValue(),
            60.0);
    QCOMPARE(static_cast<ApexAdaptiveProcedureParameters*>
