...
<xsl:stylesheet
 ...
 exclude-result-prefixes="xsd ppp html">
   ...
   <!-- Should point to the folder containing the diagram images. -->
   <xsl:param name="imagesBaseURL" select="''"/>

   <!-- The file name of the master schema html output file. This is the top
   of the tree. Just the name, not the path.-->
   <xsl:param name="masterSchemaOutputFileName" select="''"/>

   <!-- ******** Diagrams ******** -->

    <!-- yes if the diagram images should be referred in the generated documentation -->
    <xsl:param name="showDiagrams" select="'false'"/>

  <!-- yes if the logical diagram images should be referred in the generated documentation -->
  <xsl:param name="showLogicalDiagrams" select="'false'"/>

   <!-- diagram images extension -->
   <xsl:param name="diagramImagesExtension" select="'.png'"/>

   <!-- ******** Chunk support ******** -->

   <!--  yes if more files should be generated as output -->
   <xsl:param name="generateChunks" select="'false'"/>

   <!--  specifies the main schema documentation file name -->
   <xsl:param name="mainDocFile" select="''"/>

   <!--  specifies the main schema documentation base directory -->
   <xsl:param name="mainDocBase" select="''"/>

   <!--
      If enabled then the achors and links will be generated using the hashcode of the component
      identifier instead of using the identifier itself. This is useful when the schema componet
      names contain characters that are not directly supported by the browsers or by the file
      system.
   -->
   <xsl:param name="useHashcodes" select="'false'"/>

   <!--
      If the value is yes (default) and the documentation is generated with chunks the component
      description and name will be added to the generated chunk page title. To disable this set the value
      to 'no'.
   -->
   <xsl:param name="useComponentInTitle" select="'true'"/>
   <!--
     The current working directory
   -->
   <xsl:param name="cwd" select="''"/>
  <!--
      The name of the schema file. It will be used to add this in the title of the generated documentation.
   -->
  ...
  <!-- Schema components -->
  <xsl:variable name="components" select="'attribute attributeGroup group complexType element
    simpleType notation'"/>
  <!-- Characters we do not remove when generating image names -->
  <xsl:variable name="alphaNumeric"
    select="'.0123456789qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM'"/>
...
'searchIncludedSchemas' or 'searchImportedSchemas' is true.
      ...
      <!-- Check that mainDocFile is provided if generateChunks is yes. -->
      <xsl:if test="$mainDocFile=''
         and normalize-space(translate($generateChunks, 'TRUE', 'true'))='true' ">
         <xsl:call-template name="HandleError">
            <xsl:with-param name="isTerminating">true</xsl:with-param>
            <xsl:with-param name="errorMsg">'maindocFile' variable must be provided if generateChunks is yes.</xsl:with-param>
         </xsl:call-template>
      </xsl:if>
...
@import url(</xsl:text><xsl:value-of select="$externalCSSURL"/><xsl:text>);
            ...
            <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true' and not(normalize-space(translate($generateChunks, 'TRUE', 'true'))='true')">
            ...
            <!-- Add Javascript code for documentation popups -->
            <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true' ">
               <xsl:call-template name="PrintJSCode">
                  <xsl:with-param name="code">
                     <xsl:call-template name="DocumentationJSCode"/>
                  </xsl:with-param>
               </xsl:call-template>
            </xsl:if>
         ...
         <body>
            <!-- Title -->
            <h1><a name="top"><xsl:value-of select="$actualTitle"/>blabla</a></h1>

            <!-- Buttons for displaying printer-friendly version, and
                 expanding and collapsing all boxes -->
            <xsl:if test="not(normalize-space(translate($generateChunks, 'TRUE', 'true'))='true')">
            ...
            <xsl:call-template name="PrintAllSchemaDocs">
               <xsl:with-param name="masterSchemaOutputFileName" select="$masterSchemaOutputFileName"/>
            </xsl:call-template>
               ...
               <xsl:choose>
                  ...
                  <xsl:when test="normalize-space(translate($sortByComponent,'TRUE','true'))='true'">
                     ...
                     <xsl:if test="xsd:attribute or xsd:element">
                        ...
                        <xsl:apply-templates select="xsd:attribute | xsd:element  | xsd:element/xsd:key" mode="topSection">
                  ...
                  <xsl:otherwise>
                     ...
                     <xsl:apply-templates select="xsd:attribute | xsd:attributeGroup | xsd:complexType | xsd:element | xsd:group | xsd:notation | xsd:simpleType | xsd:element/xsd:key" mode="topSection"/>
                  </xsl:otherwise>
               </xsl:choose>
   ...
   <xsl:template name="Footer">
      <!-- Document Footer -->
      <p class="footer">
         <xsl:text>Generated by </xsl:text><a href="http://www.oxygenxml.com">&lt;oXygen/> XML Editor</a><xsl:text> using a modified version of </xsl:text>
         <a href="http://titanium.dstc.edu.au/xml/xs3p">xs3p</a>
         <xsl:text> that adds schema diagrams and chunking support.</xsl:text>
         <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true'">
   ...
   <xsl:template match="xsd:schema" mode="toc">
      <ul>
         ...
         <xsl:if test="xsd:redefine">
            <li>
               <xsl:choose>
                  <xsl:when test="normalize-space(translate($generateChunks, 'TRUE', 'true'))='true'">
   ...
   <xsl:template match="xsd:*[@name]" mode="toc">
      ...
      <xsl:variable name="link">
         <xsl:choose>
            <xsl:when test="normalize-space(translate($generateChunks, 'TRUE', 'true'))='true'">
               <xsl:value-of select="$mainDocFile"/><xsl:text>_</xsl:text>
               <xsl:value-of select="$componentID"/><xsl:text>.html</xsl:text>
            </xsl:when>
            <xsl:otherwise>
               <xsl:text>#</xsl:text>
     ...
     -->
   <xsl:template match="xsd:key" mode="topSection" priority="10">
      <xsl:if test="normalize-space(translate($generateChunks,'TRUE','true'))='true'">
         <xsl:variable name="componentID">
            <xsl:call-template name="GetComponentID">
               <xsl:with-param name="component" select="."/>
            </xsl:call-template>
         </xsl:variable>
         <xsl:apply-templates select="parent::*" mode="topSection">
            <xsl:with-param name="actualComponentID" select="$componentID"/>
         </xsl:apply-templates>
      </xsl:if>
   </xsl:template>


   <xsl:template name="DiagramsAndInfo">
      <!-- Show diagrams -->
      <xsl:if test="translate($showLogicalDiagrams, 'TRUE', 'true')='true' and local-name()='element'">
         <!-- Logical model diagram -->
         <xsl:call-template name="LogicalModelDiagram">
            <xsl:with-param name="component" select="."/>
         </xsl:call-template>
      </xsl:if>
      ...
      <!-- Diagram -->
      <xsl:if test="translate($showDiagrams, 'TRUE', 'true')='true'">
         <!-- Full model diagram -->
         <xsl:call-template name="FullModelDiagram">
            <xsl:with-param name="component" select="."/>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template match="xsd:*[@name]" mode="topSection">
      <xsl:param name="actualComponentID"/>
      <xsl:variable name="componentID">
         <xsl:call-template name="GetComponentID">
            <xsl:with-param name="component" select="."/>
         </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="commonContent">
      ...
      <xsl:if test="not(normalize-space(translate($generateChunks,'TRUE','true'))='true')">
         <xsl:call-template name="ComponentSectionHeader">
            ...
            <xsl:copy-of select="$commonContent"/>
      </xsl:if>
      <xsl:choose>
         <xsl:when test="normalize-space(translate($generateChunks, 'TRUE', 'true'))='true'">
            <!-- Handle chunks -->
               <xsl:variable name="chunkComponentID">
                 <xsl:choose>
                   <xsl:when test="$actualComponentID"><xsl:value-of select="$actualComponentID"/></xsl:when>
                   <xsl:otherwise><xsl:value-of select="$componentID"/></xsl:otherwise>
                 </xsl:choose>
               </xsl:variable>

            <xsl:variable name="href" select="concat($mainDocFile, '_', $chunkComponentID, '.html')"/>      <!-- [Tom] set filename of chunk -->
            <xsl:message>
    Writing: <xsl:value-of select="$href"/>
            </xsl:message>
               <xsl:result-document
                  href="{$href}"
                  omit-xml-declaration="yes">
                  <part>
                     <section><xsl:value-of select="@name"/></section>
                  <!--html-->
                     <!--head>
                        <title><xsl:value-of select="$actualTitle"/>
                           <xsl:if test="$schemaName!=''">
                              <xsl:text> - </xsl:text>
                              <xsl:value-of select="$schemaName"/>
                           </xsl:if>
                           <xsl:if test="$useComponentInTitle='true'">
                              <xsl:text> - </xsl:text>
                              <xsl:call-template name="GetComponentDescription">
                                 <xsl:with-param name="component" select="."/>
                              </xsl:call-template>
                     ...
                     <!--body-->
                        ...
                        <!-- Header -->
                        <!-- [Tom]
                        <xsl:call-template name="ComponentSectionHeaderChunked">
                           <xsl:with-param name="component" select="."/>
                        </xsl:call-template>
                        -->
                        <xsl:copy-of select="$commonContent"/>
                        <xsl:call-template name="DiagramsAndInfo"/>
                        <!--<xsl:call-template name="Footer"/>-->
                     <!--/body-->
                  <!--/html-->
                  </part>
...
<!--               <xsl:call-template name="write.chunk">
               <xsl:with-param name="filename" select="concat($mainDocBase, $mainDocFile, '_', $chunkComponentID, '.html')"/>
...
if (pc != null) {
   pc.style.display="block";
            ...
            <div id="globalControls" style="display:none">
              <xsl:if test="translate($showLogicalDiagrams, 'TRUE', 'true')='true'">
                <strong>Logical Diagram:</strong><br/>
                <span style="margin-left: 1em; white-space: nowrap">
                  <xsl:text>[ </xsl:text>
                  <a href="javascript:void(0)" onclick="expandAll(logicalDiagramBoxes)">Expand All</a>
                  <xsl:text> | </xsl:text>
                  <a href="javascript:void(0)" onclick="collapseAll(logicalDiagramBoxes)">Collapse All</a>
                  <xsl:text> ]</xsl:text>
                </span><br/><br/>
                </xsl:if>
               ...
               <xsl:if test="translate($showDiagrams, 'TRUE', 'true')='true'">
                  ...
                  <span style="margin-left: 1em; white-space: nowrap">
                     <xsl:text>[ </xsl:text>
                     <a href="javascript:void(0)" onclick="expandAll(diagramBoxes)">Expand All</a>
                     <xsl:text> | </xsl:text>
                     <a href="javascript:void(0)" onclick="collapseAll(diagramBoxes)">Collapse All</a>
                     <xsl:text> ]</xsl:text>
                  </span><br/><br/>
               </xsl:if>
...
if (gc != null) {
   ...
   <!--
     ...
     Param(s):
            component (Node) required
              ...
              [Tom]
     -->
   <xsl:template name="ComponentSectionHeaderChunked">
      <xsl:param name="component"/>
      ...
      <!--<h4><a href="{$mainDocFile}" class="details">[Table of contents]</a></h4>   [Tom] -->
   </xsl:template>
   ...
   <xsl:template name="DocumentJSCode">
     ...
     <!-- Get all IDs of diagram boxes and place them in an array. -->
     <xsl:text>/* IDs of diagram boxes */
</xsl:text>
     <xsl:text>var diagramBoxes = new Array(</xsl:text>
     <xsl:for-each select="/xsd:schema/xsd:*[@name]">
       <xsl:if test="position()!=1">
         <xsl:text>, </xsl:text>
       </xsl:if>
       ...
       <xsl:text>_diagram'</xsl:text>
     </xsl:for-each>
     <xsl:text>);
</xsl:text>

     <!-- Get all IDs of logical diagram boxes and place them in an array. -->
     <xsl:text>/* IDs of logical diagram boxes */
</xsl:text>
     <xsl:text>var logicalDiagramBoxes = new Array(</xsl:text>
     <xsl:for-each select="/xsd:schema/xsd:element[@name]">
       ...
       <xsl:text>_logicalDiagram'</xsl:text>
     </xsl:for-each>
     <xsl:text>);
</xsl:text>
...
function displayMode(isPrinterVersion) {
   ...
   <!--
      Prints the JS code that shows the documentation popup.
   -->
   <xsl:template name="DocumentationJSCode">
      <xsl:text>
...
.footer {
   ...
   <xsl:template name="Legend">
      ...
      <table class="properties">
         ...
         <tr>
            <th>
               <a title="Look up 'Abstract' in glossary" href="{$mainDocFile}#term_Abstract">Abstract</a>
            </th>
            <td>no</td>
         </tr>
      </table>
   ...
   <!-- Generates a row in the properties table, containing back references. -->
   <xsl:template name="GenerateUsedBy">
      <xsl:variable name="usedByContent">
         ...
         <!-- The target namespace prefix. The references are made using this prefix. -->
         <xsl:variable name="targetNamespace" select="/xsd:schema/@targetNamespace"/>
         <xsl:variable name="prefixForTargetNamespace" select="local-name(/xsd:schema/namespace::*[string(.) = $targetNamespace][1])"/>

         <!-- Selects all the references having the name and the type matchinng the current component.

            Debug:

            Searching for: <br/>
            <xsl:value-of select="$nameAttr"/> <br/>
            and component: <br/>
            <xsl:value-of select="$qName"/>
            and localname: <br/>
            <xsl:value-of select="$localName"/><br/>

            and prefix for target namespace: <br/>
            <xsl:value-of select="$prefixForTargetNamespace"/><br/>

         -->
         <xsl:variable name="refAttr">
            <xsl:choose>
               <xsl:when test="$prefixForTargetNamespace">
                  <xsl:value-of select="concat(concat($prefixForTargetNamespace,':'), $nameAttr)"/>
               </xsl:when>
               <xsl:otherwise>
         ...
         <xsl:choose>
            <xsl:when test="$localName =  'element'">
               <xsl:for-each select="//*[substring-after(name(),':') =  'element' and  @ref= $refAttr ]">
                  <xsl:call-template name="GenerateUsedByItem"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:when test="$localName =  'attribute'">
               <xsl:for-each select="//*[substring-after(name(),':') =  'attribute' and  @ref= $refAttr ]">
                  <xsl:call-template name="GenerateUsedByItem"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:when test="$localName =  'complexType'">
               <xsl:for-each select="//*[substring-after(name(),':') =  'element'  and  @type=$refAttr ]">
                  <xsl:call-template name="GenerateUsedByItem"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:when test="$localName =  'simpleType'">
               <xsl:for-each select="//*[(substring-after(name(),':') =  'element'  or substring-after(name(),':') =  'attribute' ) and  @type=$refAttr ]">
                  <xsl:call-template name="GenerateUsedByItem"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:when test="$localName =  'group'">
               <xsl:for-each select="//*[substring-after(name(),':') =  'group'  and  @ref=$refAttr ]">
                  <xsl:call-template name="GenerateUsedByItem"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:when test="$localName =  'attributeGroup'">
               <xsl:for-each select="//*[substring-after(name(),':') =  'attributeGroup'  and  @ref=$refAttr ]">
                  <xsl:call-template name="GenerateUsedByItem"/>
               </xsl:for-each>
            </xsl:when>

            <xsl:otherwise>
               <xsl:message>Unknown component name <xsl:value-of select="$localName"/></xsl:message>
            </xsl:otherwise>
         </xsl:choose>
      </xsl:variable>

      <xsl:if test="count($usedByContent/*) > 0">
         <tr>
            <th>Used by (from the same schema document)</th>
            <td><xsl:apply-templates select="$usedByContent" mode="copy"/>
            </td>
         </tr>
      </xsl:if>
   </xsl:template>

   <xsl:template name="GenerateUsedByItem">

      <xsl:variable name="topLevelComponent" select="if
        (./ancestor-or-self::*[last() - 1]/self::xsd:redefine) then
        ./ancestor-or-self::*[last() - 2] else ./ancestor-or-self::*[last() - 1]"/>


      <!-- Description: Element, Complex Type, Simple Type, etc... -->
      <xsl:call-template name="GetComponentDescription">
         <xsl:with-param name="component" select="$topLevelComponent"/>
      </xsl:call-template>

       <!-- The id of the component - eg. type_tip or element_name -->
      <xsl:variable name="componentID">
         <xsl:call-template name="GetComponentID">
            <xsl:with-param name="component" select="$topLevelComponent"/>
         </xsl:call-template>
      ...
      <xsl:variable name="href">
         <xsl:choose>
            <xsl:when test="normalize-space(translate($generateChunks, 'TRUE', 'true'))='true'">
               <xsl:value-of select="concat($mainDocFile, '_', $componentID, '.html')"/>
            </xsl:when>
            <xsl:otherwise>
      ...
      <b>
         <a href="{normalize-space($href)}">
            ...
            <xsl:value-of select="$topLevelComponent/@name"/>
         </a>
      </b>
   ...
   <xsl:template name="PrintAnnotation">
      ...
      <xsl:if test="$component/xsd:annotation/xsd:documentation">
         <tr>
            ...
            <!-- Dan: Added the documentation class so the cell may be customized
             through a CSS -->
            <td class="documentation">
               <xsl:for-each select="$component/xsd:annotation/xsd:documentation">
                  <xsl:if test="position()!=1"><br/><br/></xsl:if>

                  <!--Dan: This line bellow copies the documentation, escaping
                     the tags - no pretty print.. Changed to the copy-of, so the
                     eventual HTML tags are not escaped. -->
                  <!-- <xsl:apply-templates select="." mode="properties"/> -->
                  <xsl:copy-of select="*|text()" exclude-result-prefixes="xs" copy-namespaces="no"/>
               </xsl:for-each>
            </td>
         </tr>
      </xsl:if>
         ...
         <tr>
            ...
            <td>
               <xsl:for-each select="$component/xsd:annotation/xsd:appinfo">
                  <xsl:if test="position()!=1"><br/><br/></xsl:if>
                  <!-- Dan: I leave this unchanged because it pretty prints the
                     elements from the app info.-->
                  <xsl:apply-templates select="." mode="properties"/>
               </xsl:for-each>
            </td>
         </tr>
   ...
   <xsl:template name="PrintSimpleRestriction">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$restriction">
            ...
            <xsl:variable name="baseTypePrefix">
...
<!--
            <xsl:message>BT Name <xsl:value-of select="$baseTypeName"/></xsl:message>
            <xsl:message>BT NS <xsl:value-of select="$baseTypeNS"/></xsl:message>
-->
            <xsl:choose>
               ...
               <xsl:otherwise>
                  ...
                  <xsl:variable name="defLoc">
                     <xsl:variable name="baseTypeName">
                        <xsl:choose>
                           <xsl:when test="$baseTypePrefix">
                              <xsl:value-of select="concat($baseTypePrefix, ':', $baseTypeName)"/>
                           </xsl:when>
                           <xsl:otherwise>
                              <xsl:value-of select="$baseTypeName"/>
                           </xsl:otherwise>
                     ...
                     <xsl:message><xsl:value-of select="$baseTypeName"/></xsl:message>
                     <xsl:call-template name="FindComponent">
                        <xsl:with-param name="ref" select="normalize-space($baseTypeName)"/>
                        <xsl:with-param name="compType">simple type</xsl:with-param>
                     </xsl:call-template>
                  </xsl:variable>
                  <xsl:message>DEF LOC <xsl:value-of select="$defLoc"/></xsl:message>

                  <xsl:choose>
                     <!-- Simple type was found in current schema. -->
                     <xsl:when test="normalize-space($defLoc)='this'">
                        <xsl:variable name="stype" select="key('simpleType', $baseTypeName)"/>

                        <xsl:call-template name="PrintSimpleConstraints">
                           <xsl:with-param name="simpleContent" select="$stype"/>
                           <xsl:with-param name="typeList" select="concat($typeList, '*', $typeName, '+')"/>
                        </xsl:call-template>

                     </xsl:when>
                     <!-- Simple type was not found. -->
                     ...
                     <!-- Simple type was found in external schema. -->
                     <xsl:otherwise>
                        <xsl:variable name="stype" select="document($defLoc, /)/xsd:schema/xsd:simpleType[@name=$baseTypeName]"/>
                        <xsl:message>Found the simple type in the <xsl:value-of select="$defLoc"/> <xsl:value-of select="$stype/@name"/></xsl:message>
                        <xsl:call-template name="PrintSimpleConstraints">
                           <xsl:with-param name="simpleContent" select="$stype"/>
                           <xsl:with-param name="typeList" select="concat($typeList, '*', $typeName, '+')"/>
                        </xsl:call-template>
  ...
  <!--
     Prints out the full model diagram for a top-level
     schema component.
     Param(s):
            component (Node) required
              Top-level schema component
     -->
  <xsl:template name="FullModelDiagram">
    <xsl:param name="component"/>
    <!--
      We show the diagram only for schema components, that is not redefined components
      because we have no diagram for the redefined components.
    -->
    ...
    <xsl:call-template name="CollapseableBox">
      <xsl:with-param name="id" select="concat($componentID, '_diagram')"/>
      <xsl:with-param name="styleClass">sample</xsl:with-param>
      <xsl:with-param name="caption">Diagram</xsl:with-param>
      <xsl:with-param name="contents">
        <xsl:call-template name="showModelDiagram">
          <xsl:with-param name="component" select="$component"/>
          <xsl:with-param name="imagesBaseURL" select="$imagesBaseURL"/>
        </xsl:call-template>
      </xsl:with-param>
      <xsl:with-param name="isOpened">true</xsl:with-param>
    </xsl:call-template>
  ...
  <!--
     Prints out the Logical model diagram for a top-level
     schema element.
     Param(s):
            component (Node) required
              Top-level schema component
     -->
  <xsl:template name="LogicalModelDiagram">
    <xsl:param name="component"/>

    <!-- Applicable only for element definitions -->
    <xsl:if test="local-name($component)='element'">
      <xsl:variable name="componentID">
        <xsl:call-template name="GetComponentID">
          <xsl:with-param name="component" select="$component"/>
        </xsl:call-template>
      ...
      <xsl:call-template name="CollapseableBox">
        <xsl:with-param name="id" select="concat($componentID, '_logicalDiagram')"/>
        <xsl:with-param name="styleClass">sample</xsl:with-param>
        <xsl:with-param name="caption">Logical Diagram</xsl:with-param>
        <xsl:with-param name="contents">
          <xsl:call-template name="showModelDiagram">
            <xsl:with-param name="component" select="$component"/>
            <xsl:with-param name="imagesBaseURL" select="$imagesBaseURL"/>
   ...
   <xsl:template match="xsd:attributeGroup[@ref]" mode="sample">
      ...
      <xsl:choose>
         <xsl:when test="contains($parentGroups, concat('*', normalize-space($attrGrpName), '+'))
           and not(parent::xsd:attributeGroup[@name=$attrGrpName and parent::xsd:redefine])">
         ...
         <xsl:otherwise>
           ...
           <xsl:variable name="defLoc">
             <xsl:choose>
               <xsl:when test="parent::xsd:attributeGroup[@name=$attrGrpName and parent::xsd:redefine]">
                 <xsl:call-template name="FindComponentInSchema">
                   <xsl:with-param name="name" select="@ref"/>
                   <xsl:with-param name="compType">attribute group</xsl:with-param>
                   <xsl:with-param name="schema"
                     select="document(parent::xsd:attributeGroup/parent::xsd:redefine/@schemaLocation)/xsd:schema"></xsl:with-param>
                   <xsl:with-param name="schemaFileLoc" select="parent::xsd:attributeGroup/parent::xsd:redefine/@schemaLocation"></xsl:with-param>
                 </xsl:call-template>
               </xsl:when>
               <xsl:otherwise>
                 <xsl:call-template name="FindComponent">
...
<xsl:text> </xsl:text-->
   ...
   <xsl:template match="xsd:unique | xsd:key | xsd:keyref" mode="sample"/>      <!-- [Tom] -->

   <xsl:template match="xsd:unique | xsd:key | xsd:keyref" mode="sampleREMOVED">
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="schemaLoc">this</xsl:param>
   ...
   <!--
     ...
     Param(s):
            component (Node) required
                Schema component
  -->
   <xsl:template name="PrintSampleDocumentationJavaScript">
      <xsl:param name="component"/>
      ...
      <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true' and $component and $component/xsd:annotation/xsd:documentation">
         <xsl:variable name="documentation">
            <xsl:call-template name="ExtractDocumentation">
               <xsl:with-param name="component" select="$component"/>
            </xsl:call-template>
         </xsl:variable>
   ...
   <xsl:template name="ExtractDocumentation">
      <xsl:param name="component"/>
      <xsl:for-each select="$component/xsd:annotation/xsd:documentation">
   ...
   <!--
      Prints out  the documentation drectly in the output. No need for clicking on a button to open
   ...
   <xsl:template name="PrintSampleDocumentation">
      ...
      <xsl:if test="$component and $component/xsd:annotation/xsd:documentation">
         ...
         <xsl:text> </xsl:text>
         <!--p class="documentation"><xsl:value-of select="$documentation"/></p-->
   ...
   <xsl:template name="PrintSampleSimpleElement">
      ...
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="schemaLoc">this</xsl:param>
     <xsl:param name="typeList" select="''"/>
      ...
      <!--div style="margin-left: {$margin}em">
         ...
         <xsl:text>></xsl:text-->
         <element>
            <name><xsl:copy-of select="$elemTag"/></name>
...
<element>
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <name><xsl:copy-of select="$tag"/></name>
   ...
   <xsl:template name="PrintSampleTypeContent">
      ...
      <!--xsl:if test="$addBR='true'"><br/></xsl:if  [Tom] -->

      PrintSampleTypeContent
...
PrintSampleSimpleRestriction
   ...
   <xsl:template name="PrintGlossaryTermRef">
      ...
      <xsl:choose>
         <xsl:when test="$code !='' and normalize-space(translate($printGlossary,'TRUE','true'))='true'">
            <a title="Look up '{$term}' in glossary" href="{$mainDocFile}#{concat($TERM_PREFIX, $code)}">
               <xsl:choose>
                  <xsl:when test="$term!=''">
         ...
         <xsl:choose>
            <xsl:when test="/xsd:schema/namespace::*[local-name(.)=normalize-space($prefix)]">
               <xsl:variable name="link">
                  ...
                  <xsl:if test="normalize-space($schemaLoc)='this' and normalize-space(translate($generateChunks, 'TRUE', 'true'))='true'">
                     <xsl:value-of select="$mainDocFile"/>
                  </xsl:if>
   ...
   <xsl:template name="PrintCompName">
      ...
      ref:<xsl:value-of select="$name"/>
    </xsl:template>


      <xsl:template name="PrintCompNameINVALID">
      <xsl:param name="baseFile"/>
      <xsl:param name="name"/>
      <xsl:param name="compType"/>
      <xsl:param name="schemaLoc">this</xsl:param>
    ...
    <xsl:variable name="anchor">
      <xsl:choose>
        <xsl:when test="normalize-space(translate($useHashcodes,'TRUE', 'true'))='true' ">
          <xsl:value-of select="'#'"/>
          <xsl:call-template name="GetHashcode">
            <xsl:with-param name="value"
              select="concat(normalize-space($compPrefix),normalize-space($name))"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:variable name="link">
         <xsl:choose>
            ...
            <!-- There exists a link to the schema component's
                 documentation. -->
            <xsl:otherwise>
               <xsl:if test="string-length(normalize-space($baseURI))=0 and normalize-space(translate($generateChunks, 'TRUE', 'true'))='true'">
                  <xsl:value-of select="$mainDocFile"/>
               </xsl:if>
               <!-- Base URI -->
            ...
            <xsl:otherwise>
               ...
               <xsl:choose>
                  <xsl:when test="normalize-space(translate($useHashcodes,'TRUE', 'true'))='true' ">
                     ...
                     <xsl:call-template name="GetHashcode">
                        <xsl:with-param name="value" select="concat(normalize-space($compPrefix),normalize-space($name))"/>
                     </xsl:call-template>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:value-of select="concat('_',normalize-space($compPrefix),normalize-space($name))"/>
                  </xsl:otherwise>
               </xsl:choose>
   ...
   <xsl:template name="FindComponent">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$refNamespace=/xsd:schema/@targetNamespace">
            <xsl:call-template name="FindComponentInSchema">
               <xsl:with-param name="name" select="$refName"/>
               <xsl:with-param name="compType" select="$compType"/>
         ...
         <xsl:when test="normalize-space(translate($searchImportedSchemas, 'TRUE', 'true'))='true'">
            <xsl:variable name="refNS" select="/xsd:schema/namespace::*[local-name(.)=normalize-space($refPrefix)][1]"/>
   ...
   <xsl:template name="GetComponentDescription">
      ...
      <xsl:choose>
        ...
        <xsl:when test="local-name($component)='key'">
...
Unknown schema component, <xsl:value-of select="local-name($component)"/>.
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:choose>
               ...
               <xsl:when test="normalize-space(translate($useHashcodes,'TRUE', 'true'))='true' ">
                  <xsl:call-template name="GetHashcode">
                     <xsl:with-param name="value" select="concat($componentPrefix,$component/@name)"/>
                  </xsl:call-template>
               </xsl:when>
               <xsl:otherwise>
   ...
   <xsl:template name="GetHashcode">
      <xsl:param name="value" select="''"/>
      <xsl:value-of select="'h'"/>
      <xsl:variable name="t" select="s:new(string($value))" xmlns:s="java.lang.String"/>
      <xsl:value-of select="s:hashCode($t)" xmlns:s="java.lang.String"/>
   </xsl:template>
...
Unknown schema component, <xsl:value-of select="local-name($component)"/>.
   ...
   <xsl:template name="GetThisPrefix">
      <xsl:if test="/xsd:schema/@targetNamespace">
         <xsl:value-of select="local-name(/xsd:schema/namespace::*[normalize-space(.)=normalize-space(/xsd:schema/@targetNamespace)][1])"/>
      </xsl:if>
   </xsl:template>
   ...
   <xsl:template name="GetXSDPrefix">
      <xsl:value-of select="local-name(/xsd:schema/namespace::*[normalize-space(.)=$XSD_NS][1])"/>
   </xsl:template>
...
// --&gt;
  ...
  <!--  diagram templates -->
  <xsl:template name="showModelDiagram">
    <xsl:param name="component"/>
    <xsl:param name="imagesBaseURL" select="''"/>
    <!-- Parameter if 1 then generate the full diagram, otherwise, the logical diagram. -->
    ...
    <xsl:variable name="name">
       <xsl:choose>
          <!-- The anchors are generated with the type prefiexes if the component is a type -->
          <xsl:when test="local-name($component) = 'complexType'">
             <xsl:value-of select="$CTYPE_PREFIX"/>
          </xsl:when>
          <xsl:when test="local-name($component) = 'simpleType'">
          ...
          <xsl:otherwise>
             <!-- or directly with the local name -->
             <xsl:value-of select="local-name($component)"/>
             <xsl:text>_</xsl:text>
          </xsl:otherwise>
       </xsl:choose>
      ...
      <xsl:if test="/*/@targetNamespace">
        <xsl:text>_</xsl:text>
        <xsl:value-of select="translate(/*/@targetNamespace, translate(/*/@targetNamespace,
          $alphaNumeric, '') , '')"/>
      </xsl:if>
    </xsl:variable>

     <!-- TODO - possible refactor ... The name of the image, with no extension. -->
     <xsl:variable name="rawImageName1">
        <xsl:choose>
           <xsl:when test="normalize-space(translate($useHashcodes,'TRUE', 'true'))='true' ">
              <xsl:call-template name="GetHashcode">
                 <xsl:with-param name="value">
                    <xsl:choose>
                       <xsl:when test="$full = 0">
                          <!-- Logical - add logical in front ! TODO! -->
                          <xsl:value-of select="concat('logical_', $name)"/>
                       </xsl:when>
                       <xsl:otherwise>
                          <xsl:value-of select="$name"/>
                       </xsl:otherwise>
     ...
     <xsl:variable name="rawImageName" select="normalize-space($rawImageName1)"/>


     <xsl:if test="contains($components, local-name($component))">

          <xsl:variable name="rawImagePath">
             <xsl:variable name="base">
                <!-- This is relative. -->
                <xsl:if test="string-length($imagesBaseURL)&gt;0">
                   <xsl:value-of select="$imagesBaseURL"/>
                   <xsl:if test="substring($imagesBaseURL, string-length($imagesBaseURL))!='/'">
                      <xsl:text>/</xsl:text>
                   </xsl:if>
                </xsl:if>
             </xsl:variable>
              <xsl:value-of select="concat($base, $rawImageName)"/>
          </xsl:variable>

          <img usemap="#{$rawImageName}">
           <xsl:attribute name="src">
              <xsl:value-of select="$rawImagePath"/><xsl:value-of select="$diagramImagesExtension"/>
           </xsl:attribute>
         </img>
         <!-- map -->
        <!-- The map files are in the directory containing the images.
           This directory is relative to the html output directory. -->
        <xsl:variable name="fullPath" select="concat($mainDocBase, '/', $rawImagePath, '.map')"/>
        <xsl:variable name="correctedFullPath" select="translate($fullPath, '\', '/')"/>
        <!--
        <xsl:message>The corrected full path is:  <xsl:value-of select="$correctedFullPath"/></xsl:message>
        -->
        <xsl:variable name="correctedFullPath">
           <xsl:choose>
              <xsl:when test="not(starts-with($correctedFullPath, '/')) and string-length(substring-before($correctedFullPath,':')) = 1">
                 <!-- Is not prefixed with slash and is an absolute path. This is the windows case.
                    The drive letter is the first.. -->
                 <xsl:value-of select="concat('file:/', $correctedFullPath)"/>
              </xsl:when>
              <xsl:otherwise>
                 <!-- An absolute path already prefixed with / -->
                 <xsl:value-of select="concat('file:', $correctedFullPath)"/>
              </xsl:otherwise>
           </xsl:choose>
        </xsl:variable>

        <xsl:variable name="mapRef" select="normalize-space($correctedFullPath)"/>
         <xsl:comment>
            Map ref: <xsl:value-of select="$mapRef"/>
         </xsl:comment>
        <!--
        <xsl:message>Map ref: <xsl:value-of select="$mapRef"/></xsl:message>
        -->
       <xsl:if test="doc-available($mapRef)">
          <xsl:apply-templates select="document($mapRef)/*" mode="copy"/>
       </xsl:if>
     </xsl:if>
  </xsl:template>
   ...
   <!--
   <xsl:template name="PrintAllSchemaDocsRecursive">
       // The current document.
      <xsl:param name="doc"/>
      // The schema, exactly how it appears in the schemaLocation attribute. Can be relative.
       // The links file contains this relative expression.

      <xsl:param name="schemaLocation"/>
      // URIs of the already loaded documents. Skips recursivity.
      <xsl:param name="docUris" select="''"/>
      ...
      <xsl:for-each select="$doc//xsd:include | $doc//xsd:import | $doc//xsd:redefine">
         <xsl:variable name="refDoc" select="document(@schemaLocation)"/>
         <xsl:variable name="docUri" select="document-uri($refDoc)"/>
         <xsl:choose>
            <xsl:when test="contains($docUris, $docUri)">
               // The document was already processed.
               // The docUris contains <xsl:value-of select="$docUri"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:call-template name="PrintAllSchemaDocsRecursive">
                  <xsl:with-param name="doc" select="$refDoc"/>
                  <xsl:with-param name="docUris" select="concat($docUris, ' : ' , $docUri)"/>
   ...
   -->
   <xsl:template name="PrintAllSchemaDocs">
      <xsl:param name="masterSchemaOutputFileName"/>
      <xsl:param name="masterSchemaURL"/>
      <xsl:if test="$masterSchemaOutputFileName and $masterSchemaOutputFileName != $mainDocFile">
         <h4>Master schema documentation file:
            <a href="{$masterSchemaOutputFileName}"><xsl:value-of select="$masterSchemaOutputFileName"/></a>
         </h4>
      </xsl:if>
   </xsl:template>
