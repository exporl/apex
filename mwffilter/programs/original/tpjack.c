...
int callback_process (jack_nframes_t nframes, void *args);
...
int TPAudio_start(int * samplingRate, int blockSize)
{
    ...
    TPJack_jackHandle = jack_client_open(JACK_CLIENT_NAME, JackNullOption, NULL);
...
*/
int callback_process (jack_nframes_t nframes, void *args __attribute__((unused)))
{
 ...
 */
void jack_shutdown (void *args __attribute__((unused)))
{
    printf("\nJACK shutdow NOW!\n");
    exit (1);
}
