...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"  
    ...
    <xsl:template match="apex:results">
        ...
        <xsl:text>&#10;&#10;[Stimulus/response/correctorvalue/rove]&#10;</xsl:text>
        ...
        <xsl:text>&#10;</xsl:text>
        <xsl:call-template name="getroves"/>
    ...
    <xsl:template name="getresponses">
        <xsl:apply-templates select="trial/screenresult/element[@id=$answer_element]"/>
    </xsl:template>
    
    <xsl:template name="getanswers">
        <xsl:apply-templates select="trial/corrector/currentanswer" mode="answers"/>
    </xsl:template>
    ...
    <xsl:template match="trial/corrector/currentanswer" mode="answers">
        <xsl:value-of select="."/>
        <xsl:text>&#x9;</xsl:text>
    </xsl:template>
    ...
    <xsl:template name="getroves">
        <xsl:apply-templates select="trial/randomgenerators/randomgenerator[@id='roving']"/>
    </xsl:template>
    
    <xsl:template match="trial/randomgenerators/randomgenerator[@id='roving']">
        <xsl:value-of select="."/>
        <xsl:text>&#x9;</xsl:text>
    </xsl:template>
