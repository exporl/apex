...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"  
    ...
    <xsl:template match="apex:results">
        <xsl:text>Results of Gottinger speech test from file</xsl:text> <xsl:value-of select="$experiment_file"/> 
        <xsl:text>&#10;&#10;</xsl:text>
        <xsl:text>Presentation level: </xsl:text><xsl:value-of select="general[1]/interactive[1]/entry[1]/@new_value"/><xsl:text> dBSPL</xsl:text>
