...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"  
    ...
    <xsl:template match="trial/procedure/reversal">
        <!--xsl:variable name="stimname" select="../stimulus"/-->
        <!-- get previous trial-->
        
        <xsl:variable name="stimname" select="../../preceding-sibling::trial[1]/procedure/stimulus"/>
        
        <xsl:value-of select="document($experiment_file)//apex:apex/stimuli/stimulus[@id=string($stimname)]/fixedParameters/parameter[@id='ild']"/>
            <xsl:text>&#x9; </xsl:text>        
    </xsl:template>
    ...
    <xsl:template name="getlabels">
        <xsl:apply-templates select="trial/procedure/stimulus" mode="parameter"/>
    </xsl:template>
    <xsl:template name="getlabel" match="trial/procedure/stimulus" mode="parameter">
        <xsl:if test="not(../../@id='VIRTUAL_TRIAL')">
            <xsl:variable name="trialname" select="."/>
            <!--xsl:value-of select="."/-->

            <xsl:value-of select="document($experiment_file)//apex:apex/stimuli/stimulus[@id=string($trialname)]/fixedParameters/parameter[@id='ild']"/>
            <xsl:text>&#x9;</xsl:text>
        </xsl:if>
    </xsl:template>
