...
function run_apex
{
    echo  $1 --record --deterministic --autoanswer --autostart --autosaveresults --exitafter $2
#    run_silent
    xvfb-run -a -e "$2.xvfb-error-log" $1 --record --deterministic --autoanswer --autostart --autosaveresults --exitafter $2 >"$2.out.log" 2>&1
    #return $?
}
...
function do_tests
{
    ...
    ORIGINAL_TEST_FILE=$APEX_ROOT/examples/$1
    ...
    cp $APEX_ROOT/examples/$EXPERIMENT_DIR/*.js $TEST_DIR
...
#    for EXPERIMENT in ${CMD_LINE[@]:2}
	...
	ARR=(${FILE//;/ })
	if [ ${#ArrayName[@]} -eq 2 ]; then 
	    SKIP_RESULTS=1
	else
	    SKIP_RESULTS=0
	fi
        do_tests $FILE $SKIP_RESULTS
