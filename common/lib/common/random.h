...
#ifndef _EXPORL_COMMON_LIB_COMMON_RANDOM_H_
#define _EXPORL_COMMON_LIB_COMMON_RANDOM_H_
...
public:
    ...
    /** Determines whether future instantiations of random generators without an
     * explicitely specified seed get a seed based on the current time or just
     * 0.
     *
     * @param deterministic if true, the seed will be 0, otherwise time-based
      */
     ...
     * each value has the same likelihood (1/<code>n</code>).
     *
...
private:
    DECLARE_PRIVATE(Random)

protected:
    DECLARE_PRIVATE_DATA(Random)
};
