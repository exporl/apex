...
/* exported genericRunner, main_experiment */
/* global api, main_experiment: true, setExperimentParameters, setMainParameters */
...
function genericRunner() {
    ...
    function loadMain(showForm) {
        ...
        if ( showForm && main_experiment.form ) {
            ...
            parametersForm.innerHTML += '<input type="submit" value="Go!" onclick="loadMain(0); return false;" />';
        } else {
            ...
            for ( var elementID = 0 ; elementID < form_response.length ; elementID++ ) {
                var element = form_response[elementID];
    ...
    function doExperiment(experiment) {
        ...
        api.runExperiment({experimentfilePath: experiment.file, resultfilePath: experiment.resultfile, expressions: experiment.expressions});
    }
